...
**The proportion bar is NOT pixel-proportional at the extremes — decision 15's required note.** A minimum rendered width of 6 px is enforced for every non-zero category, because the two smallest corpus categories are `out-to-in` at 2.3% and `in-to-out` at 3.1%, which at 320 CSS px on a ~296 px bar are ~7-9 px: a label cannot go inside them (DESIGN.md:303's 11 px type floor) and a sub-pixel segment would vanish outright. The consequence, stated here rather than left implicit: at those extremes a segment's rendered width overstates its share, so **the bar is a shape, not a measurement** — the labelled value list beneath it, not the bar, is the surface that states the numbers. (Recorded at code review: the consequence had been written into a code comment in `MovementToReceiveSection.tsx` but never into this record, which is where decision 15 asked for it.)
...
Adversarial code review, 2026-08-03 — three parallel layers (Blind Hunter, Edge Case Hunter,
Acceptance Auditor) over the 2.9-scoped diff `32fc131..3cf4237` (19 files, +4591/-21). The range is
this story's own commit, deliberately not `dd3dfc3..HEAD`, which would have swept in Story 1.15's
entire `pipeline/precompute/` package. Every finding below was re-verified against the shipped tree
before triage; three were dropped as noise.

Two layers converged independently on four defects: the false log caption, the `?? 0` minute
fabrication, the section-level `viz.offers.zero` string, and the dead `TileLeader` import.

**The Acceptance Auditor cleared all 23 ruled decisions and both ACs as implemented**, with the
departures ruled below. No file on the "Do not touch" list was touched; `PitchPanel.tsx` is
strictly additive (+19/-0); Task 8's nine ledger filings are all present.

### Decisions ruled

All four were ruled by Juan on 2026-08-03 and converted to patches; the rulings are recorded inline
so they are not re-litigated.

- [x] [Review][Decision] **RULED — ratify declared reading #1 as an amendment to decision 5, and delete `byType`.** The chip stays a bare total. Decision 19 deliberately refuses to distinguish `forced-turnover` from `possession-regain` visually, so a chip enumerating them beside that legend would re-introduce exactly the distinction the map does not draw; the coherence argument beats decision 5's literal text. `defensiveFigureCount` reduces to the total and the dead per-type loop goes with it. *Original finding:* **The `metaLine` chip enumerates nothing, where decision 5 requires it to enumerate the types present** — the shipped chip is a bare total (`DefensiveActionsSection.tsx:150-154`), disclosed in the Dev Record as declared reading #1. It is a departure from decision 5's literal text, not an implementation of it. Consequence: `defensiveFigureCount(...).byType` is dead in the render path — referenced nowhere outside its own module and tests — and its inner loop rebuilds four template-literal keys per marker per render (~776 discarded allocations at corpus density) to recover an enum code `PitchMarker` had discarded.
- [x] [Review][Decision] **RULED — extend the FD-1 whole-column gate to all three columns.** Add `anyPlayerName(rows)` and `anyMinute(rows)` beside the shipped `anyContestType`, so each of Jugador, Minuto and Tipo de disputa appears only when some row carries a value. This is decision 20's own reasoning applied consistently rather than as a one-off for `contestType`. Today's fixtures populate player and minute on 100% of rows, so nothing visibly changes until the 2.19 cutover. *Original finding:* **The Jugador and Minuto columns are unconditional, shipping two full columns of em dashes on corpus data** — `DefensiveActionsSection.tsx:186-187, 221-222`. `playerName` and `at` have NO CARRIER AT ALL (`defensive-actions-model.ts:29-30`), so on real data these two columns are in exactly the condition decision 20 removes the contest column for, and no `anyPlayerName` / `anyMinute` equivalent existed.
- [x] [Review][Decision] **RULED — accept the degradation, correct the false premise, file the consequence.** The collapsed announcement is honest: it states exactly the absence that is real, and inventing a positional disambiguator is a UX call this story does not have (and would repeat the naming drift the ledger already routes for a rename). Task 3.3's justifying comment must be corrected — the premise "these events **do** carry a real clock in the contract" is refuted by the same module's `minuteLabelOf` — and the announcement collapse filed to the ledger against Story 1.16, which decides whether these fields ever get a carrier. *Original finding:* **Every defensive marker's accessible name collapses to one identical sentence per action type on corpus data** — `defensive-actions-model.ts:191-196`. With `subjectName` and `minuteLabel` both null (pinned by the module's own `CORPUS_SHAPED_EVENT` test), `markerName` yields "Acción defensiva de jugador desconocido, minuto desconocido, Recuperación de balón" for **all ~97 markers per side**, so a keyboard reader roving the tabindex hears the same sentence ~97 times with nothing distinguishing one triangle from another.
- [x] [Review][Decision] **RULED — add a runtime partition guard and correct the inverted docblock.** `movementSplit` compares `sum(six categories)` against `sum(totalOffers)` and exposes a mismatch flag; when it trips, the component renders the labelled value list **without** the bar. That is decision 14's declared fallback in its minimal honest form — cheap, and it makes a partition break visible instead of silent. The docblock's safety argument is backwards and must be rewritten to say so. *Original finding:* **`movementSplit.total` and `offersSummary.offersMade` are two independent derivations of one quantity, on adjacent sections of one page** — `receiving-model.ts:298-323` sums the six categories while `offersSummary` sums `totalOffers`. The docblock at `:307-315` argues that taking `total` from the six is safer because "a bar whose segments do not fill it is the visible failure; a silently mis-scaled bar is not" — but taking it from the six is what *guarantees* the bar always fills, i.e. it selects the silent option, not the visible one. Decision 14's fallback was not implemented and there was no runtime check anywhere.

### Patches

Four patches (marked ⟨from D#⟩) follow from the rulings above; the remaining sixteen were triaged
directly.

- [x] [Review][Patch] ⟨from D1⟩ **Reduce `defensiveFigureCount` to the total and delete `byType`** — with declared reading #1 ratified, the per-type breakdown has no consumer. Remove the `byType` field, its inner marker×code loop and the `defensiveActionKey` round-trip, and update the tests that assert on it. [app/src/viz/defensive-actions-model.ts:327-346]
- [x] [Review][Patch] ⟨from D2⟩ **Add `anyPlayerName` / `anyMinute` and gate their columns** — mirror the shipped `anyContestType` predicate and its header/cell gating for the Jugador and Minuto columns; pin all three with a corpus-shaped test in which every column disappears. [app/src/viz/defensive-actions-model.ts:316-318]
- [x] [Review][Patch] ⟨from D3⟩ **Correct Task 3.3's false premise and file the announcement collapse** — rewrite the comment that claims these events carry a real clock, and append a ledger entry routing the ~97-identical-announcements consequence to Story 1.16 by name. [app/src/viz/defensive-actions-model.ts:180-190]
- [x] [Review][Patch] ⟨from D4⟩ **Add the partition mismatch guard and rewrite the docblock** — have `splitTeam` carry both derivations, expose a mismatch flag on `MovementTeamSplit`, suppress the bar (keeping the value list) when it trips, and replace the inverted safety argument with the real one. Cover the mismatch branch with a constructed input, since no fixture can produce it. [app/src/viz/receiving-model.ts:294-326]
- [x] [Review][Patch] **`sprint-status.yaml` marks Story 1.9 `done` and the appended note denies the co-commit that the story file discloses** — this commit flips `1-9-domains-e-f-extraction-goalkeeping-set-plays: review` → `done`, while the story file discloses that deletion as `review` → `in-progress`, and Story 1.9's own last commit is titled "IN PROGRESS — Decision 3 open". Separately the note appended by this commit reads "pipeline/** never touched, so NO in-flight 1.9 / 1.15 lines are carried and there is no co-commit", contradicting the story file's own "this commit DOES carry in-flight lines" disclosure. A reader of the board gets the false statement; only a reader of the story file gets the true one. [_bmad-output/implementation-artifacts/sprint-status.yaml:53,230]
- [x] [Review][Patch] **The defensive log caption asserts an ordering the table does not have on real data** — `viz.table.caption` is literally "Ordenado por minuto.", but `orderByMinute` sorts clock-less rows last, so with every `at` absent the shipped order is home-block-then-away-block in artifact order, above a Minuto column that is 100% em dashes. The justifying comment ("these rows carry a real clock in the contract") is contradicted by the model's own `minuteLabelOf`. Both sibling receiving tables minted their own caption keys for exactly this reason. [app/src/components/DefensiveActionsSection.tsx:205-214]
- [x] [Review][Patch] **`DefensiveLogRow.minute` / `.stoppageMinute` fabricate minute 0 for every clock-less row** — `event.at?.minute ?? 0` while `minuteLabel` stays null and the row sorts last. Neither field is in Task 3.6's specified row shape, neither is rendered, and the section's own comment names "the row array below" as Story 2.11's sort plug-in point — so a future `aria-sort` reading `.minute` orders all corpus rows as minute 0, silently. This is the same `?? 0` defect the story routes to 2.11 as a known-open item, re-introduced. Make it `number | null`. [app/src/viz/defensive-actions-model.ts:295-296]
- [x] [Review][Patch] **The offers zero-state renders a per-team sentence at section level, where no team is in scope** — when both teams are at zero, `isZero` short-circuits both figures and renders one `viz.offers.zero` = "El informe no registra ofrecimientos para **este equipo**." The referent was just removed from the render. `MovementToReceiveSection.tsx:169-173` gets this right by keeping its zero line inside each team's figure. Two sections, one fact, two treatments, one wrong. [app/src/components/OffersToReceiveSection.tsx:296-303]
- [x] [Review][Patch] **A team with no player rows renders a positive false claim plus a leader glyph against a non-existent value** — `isZero` is false when the *other* team has offers, so the empty team's figure asserts "0 ofrecimientos, 0 recibidos" — the report recorded zero — when the truth is that no rows exist for that team at all. `resolveLeader(390, 0)` then awards the present team the accent, the ▲ and the spoken "líder". `playerCount: 0`, the one signal separating the two states, appears only in the totals table behind the disclosure. The model explicitly tests this state ("survives a team with no rows at all"); the component does not distinguish it. [app/src/components/OffersToReceiveSection.tsx:168-192]
- [x] [Review][Patch] **The movement bar drops zero-count segments while the value list keeps all six, breaking the only link between them** — the bar filters `category.count > 0`; the `<dl>` renders all six in frozen order. All segments are painted the same accent and the bar carries no in-segment labels, so decision 15's stated mechanism — categories carried by order — is the sole mapping, and it shifts by one for every zero category. The frozen order puts the two smallest corpus categories (`out-to-in` 2.3%, `in-to-out` 3.1%) in the middle, where a shift is least detectable. [app/src/components/MovementToReceiveSection.tsx:195-209]
- [x] [Review][Patch] **`x` and `y` are the only unguarded fields in a module that guards every other one** — `at`, `playerName` and `contestType` are all read defensively because bundles arrive as `as`-cast unvalidated JSON, with the docblock noting a throw "would take all eleven Tactical sections down". `x`/`y` are copied verbatim into `formatDecimal(row.x, locale, 2)`, which calls `assertFinite` and throws — and the log table is inside the lazily-mounted disclosure, so that throw fires when a reader opens "Ver los datos", i.e. the deferred-throw the eager-build convention exists to prevent. Guard at model entry and fail loud on load, as `resolveSide` already does. [app/src/viz/defensive-actions-model.ts:297-298]
- [x] [Review][Patch] **`barNote` describes a bar that is not rendered** — "Cada barra reparte los ofrecimientos del equipo entre los seis tipos de desmarque." renders unconditionally at the top of the section while both team blocks take the `isZero` branch and draw no bar at all. Asymmetric with `OffersToReceiveSection`, which does have a section-level zero branch. [app/src/components/MovementToReceiveSection.tsx:308]
- [x] [Review][Patch] **Dead code ships in both locales and in one component, and no gate catches either class** — `viz.offers.players`, `viz.offers.playersOne`, `viz.movement.shareLabel`, `viz.movement.countLabel` and `viz.movement.typeLabel` are added to `es.ts` and `en.ts` and referenced nowhere (the headers use the pre-existing `viz.table.players`; the value list uses `offerMovementKey`). `OffersToReceiveSection.tsx:10` imports `type { TileLeader }` and never uses it. `eslint . --max-warnings 0` exits 0 and `tsconfig.json` sets neither `noUnusedLocals` nor `noUnusedParameters`, so this rot has no gate at all. [app/src/components/OffersToReceiveSection.tsx:10]
- [x] [Review][Patch] **`marker-model.test.ts` documents a compile-time guarantee it does not provide** — the comment states "the two lines below stop compiling the day someone adds a diamond without a surface to put it on." They do not: `const shipped: MarkerShape[] = [...]` stays assignable however many members the union gains, and `toHaveLength(6)` asserts the length of a hand-written literal. Adding `"diamond-hollow"` leaves this test green. The real check is `Record<MarkerShape, true>` or a `satisfies` over a keyed object. [app/src/viz/marker-model.test.ts:108-117]
- [x] [Review][Patch] **The roving-order contract is untested** — the test named "orders each side by minute (the roving order, UX-DR16)", whose comment says "the underlying order must be non-decreasing by minute", asserts only `minutes.every((minute) => minute !== null)`. Nothing checks the ordering that both the roving tabindex and the log's default sort depend on. (`minuteLabel` is a formatted string like "45+2′" and cannot be compared directly — assert on the row-level numeric field.) [app/src/viz/defensive-actions-model.test.ts:153-162]
- [x] [Review][Patch] **The "never an encoding table" test asserts nothing related to its claim** — `expect(typeof asRecord.length).toBe("number")` and `expect(Array.isArray(DEFENSIVE_ACTION_TYPES)).toBe(true)` are both true for any array and neither bears on the property claimed (that the module exports no encoding table). [app/src/viz/defensive-actions-model.test.ts:94-105]
- [x] [Review][Patch] **The decision-4 override test cannot detect the regression it exists for** — it compares `tactical.empty.receivingHeadline` (a full sentence, "Sin datos por jugador para este partido.") against `tactical.empty.headlineBefore` (a fragment, "Sin datos de") that `useEmptyHeadline` composes with a section title. The inequality is trivially true and would stay true if `receivingHeadline` were set to the composed generic headline — the actual defect decision 4 guards against. Compare against the composed string. [app/src/lib/i18n.test.ts:189-198]
- [x] [Review][Patch] **`PossessionContestType` is hand-copied into the i18n suite while its two siblings are imported** — the describe block's docblock says the suite is "Driven by the frozen ordered lists the models export, so a contract enum change is caught here as well as at compile time", but `CONTEST_TYPES` is a local literal. A seventh contest code would need two files edited to be caught, and the label-resolution loop would simply never visit it. Export `POSSESSION_CONTEST_TYPES` from `defensive-actions-model.ts` alongside `DEFENSIVE_ACTION_TYPES`. [app/src/lib/i18n.test.ts:180-187]
- [x] [Review][Patch] **Decision 16's compile-error guarantee does not hold for the two arrays, and their docblocks claim it does** — `OFFER_MOVEMENT_PROPERTY` is a `Record<OfferMovementType, …>` and does fail closed, but `DEFENSIVE_ACTION_TYPES` and `OFFER_MOVEMENT_TYPES` are `readonly T[]`: widening either contract enum compiles silently. The i18n exhaustiveness tests compare locale keys against the same arrays, so they do not catch it either. Derive the arrays from a `Record<T, true>` keyed object so the union stays the source of truth. [app/src/viz/defensive-actions-model.ts:50-58]
- [x] [Review][Patch] **Decision 15's required Dev-Record note landed in a code comment instead** — the decision ends "Enforce a minimum rendered segment width … **and note in the Dev Record** that the bar is therefore not pixel-proportional at the extremes." The minimum is enforced and the consequence is written, but into `MovementToReceiveSection.tsx:70-73`; the Completion Notes contain no such statement. Move it into the Dev Agent Record. [app/src/components/MovementToReceiveSection.tsx:70-73]

### Deferred

- [x] [Review][Defer] **The full pitch has no goal furniture at the end where every marker lands** [app/src/viz/pitch-geometry.ts:266-326] — deferred, pre-existing. `pitchMarkings` draws `penaltyArea`, `sixYardBox`, `penaltySpot`, `penaltyArc` and `goal` unconditionally at the attacked end only; the `isFullPitch` branch adds just the halfway line, centre circle and centre spot. Defensive actions concentrate in the acting team's own half (x 8.3–64.3), so effectively the entire marker population sits in a bare striped rectangle with no geometry to read position against — a sharper consequence than for Story 2.8's pass networks, whose nodes span 20–80. **Ruled by decision 9 ("yes, mirror it") with implementation explicitly routed onward and filed by Task 8.4.** The section's own docblock is accurate — it claims only the three things that do arrive.
- [x] [Review][Defer] **Clustering collapses at corpus marker density** [app/src/viz/marker-layout.ts:115-180] — deferred, pre-existing. `clusterMarkers` is single-link at `MIN_HIT_PX = 44`; at 97–153 markers per side (against the fixtures' 30–59) the transitive chain merges most of the pitch, the dialog path dominates at every width below 1920, and `PitchPanel.tsx:376-379` explicitly budgets for "at most ~120 markers per panel". **Measured by Task 9.1 and already filed to the ledger**, including the 320 px single-cluster collapse at 153 markers. One layer's estimate of a single cluster at ≥md rests on a mean-spacing argument that contradicts the dev's own measurement at 527/768 px (8–9 clusters); the filed measurement is the one to trust.
- [x] [Review][Defer] **Cluster popover copy over-claims at high density** [app/src/components/PitchPanel.tsx:966-972] — deferred, pre-existing. "Punto con N eventos" and "Eventos en este punto" describe a point, but a pitch-spanning cluster is anchored at the arithmetic centroid of its members — typically a location where no event occurred. `PitchPanel`'s own copy, not introduced here; same ledger entry as the density item above.
- [x] [Review][Defer] **Markers and rows are rebuilt on every render, forcing a full re-cluster** [app/src/components/DefensiveActionsSection.tsx:129-140] — deferred, pre-existing. No `useMemo`, and the `sides` array literal is fresh each render, so `PitchPanel`'s extent memo and `PitchFigure`'s layout memo both invalidate on every `TacticalLayer` state change, re-running an O(n²) cluster pass plus a Delaunay build twice. `ShotMapsSection` and `PassNetworksSection` do the same — a house-wide convention, not a defect introduced by this story, though this is the first consumer sitting near the documented ceiling.

### Dismissed as noise

- Three new private `DataTable` copies bringing the total to five — explicitly ruled by Task 6.6: "a fourth (and fifth, sixth) private copy of the `DataTable` helper is the **current convention**… Do not refactor `DataTable` out of `ShotMapsSection`."
- An out-of-union enum code leaking `enums.defensiveAction.<code>` in production — `defensiveActionKey` follows the shipped `shotOutcomeKey` / `crossDeliveryKey` pattern that decision 16 mandates; not introduced here.
- `block` / `possession-contest` events not filtered out of `defensiveMarkers` — decision 5 deliberately keeps all four codes labelled, and those two families carry no coordinates because they are aggregate panels never emitted as events at all.

| 2026-08-03 | Code review: 4 decisions ruled, 20 patches applied, 4 items deferred, 3 dismissed. Chip ruled total-only and `byType` deleted; FD-1 whole-column gate extended to player and minute; the Task 3.3 clock premise corrected and the announcement collapse filed to 1.16; decision 14's partition fallback implemented. Suite 439/19 -> 447/19, full chain green. Status -> done. |
