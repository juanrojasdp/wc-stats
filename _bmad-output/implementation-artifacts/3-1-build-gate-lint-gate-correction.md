...
Three adversarial layers (Blind Hunter, Edge Case Hunter, Acceptance Auditor). **Every finding below was re-verified in this session** by running the shipped gate and the pre-story gate (`f07116b`) over the same fixture trees, and `npx eslint` over throwaway probes — six agent findings were dismissed as unverifiable or out of scope, and two agent claims were corrected (see the dismissed note). **3 decision-needed, 9 patch, 5 defer, 6 dismissed.**

The headline: **three confirmed regressions in `linkHref` make the gate exit 0 on off-origin subresources that the pre-story gate caught at exit 1** — with all 23 tests green over them. AC2's must-stay-gated rels (`stylesheet`, `icon`, `preload`) are the ones that leak.

- [x] [Review][Decision→Patch] The `SITE_ORIGIN` allowance widens `fetch()`, `src`, `Worker` and XHR, not just `<link>` — a self-origin tracker now passes silently — `ALLOWED` is consulted by `allowed()` at every fetching position and by `mentions`/`anchors`, so adding `SITE_ORIGIN` (`assert-no-external-origins.mjs:150`) allow-lists far more than the canonical/preload/sitemap cases AC1 argues for. Verified: a tree containing `<script src="https://mundial-stats.juancr.dev/tracker.js">` plus `fetch("https://mundial-stats.juancr.dev/collect",{method:"POST"})` returns `2 text asset(s) in out/, 0 external subresources`, EXIT=0, **with no `MENTIONED` line at all**; the `f07116b` gate exits 1 naming both. Under AD-13 the deploy is a static export with no functions, so there is no self-origin endpoint to legitimately fetch — this is exactly the shape a proxied-analytics regression takes, and NFR-9 is what the file header calls "the one that matters most". The code is spec-sanctioned (`:124-127` says "in ANY position"), so this is a genuine ruling, not a bug: **(a)** keep it as written and accept the widening, or **(b)** restrict the `SITE_ORIGIN` entry to the `<link href>` position and the `mentions`/`anchors` reporting lines, leaving `fetch`/`src`/`Worker`/XHR gated on origin as before. Raised by Blind Hunter; missed by the other two layers.
- [x] [Review][Decision→Patch] The `key.value` arm makes correctly-localized code a build error — `eslint.config.mjs:202`. Verified live: `export const metadata = { openGraph: { "siteName": t("app.siteName") } }` reports `error Metadata strings must come from the locale layer` **at the key**, because a quoted key is itself a `Literal` child of the `Property`. AC4 asks only that a *bare Spanish literal* be an error. The Dev Agent Record records the effect honestly ("the quoted spelling is simply not writable in a metadata object"), so it is spec-sanctioned — but story 3.3 is the consumer, and the spec itself says 3.3 "authors nested object literals where the quoted spelling is entirely plausible". Under `--max-warnings 0` the only remedy is to unquote the key, which is the "gate that cries wolf" pressure this codebase documents repeatedly. **(a)** keep the documented trap, or **(b)** restrict the child matcher to the value position so the key is never reported. Raised by Blind Hunter + Acceptance Auditor.
- [x] [Review][Decision→Patch] AC4's two new selector arms have no durable regression test, and A3 bars the natural home for one — `app/src/lib/eslint-gate.test.ts` already exists and already pins this exact selector with three cases, but story 3.1 added zero and the file is **not** in the Ownership table, so extending it would be an A3/A4 ownership violation. Consequence: deleting `|alt|siteName` or the `[key.value=...]` arm from `eslint.config.mjs:202` is caught by nothing in `npm run test`, while AC1's and AC2's mechanisms both got durable gates. The only proof AC4 ever had was the Task 8.7 throwaway probe, explicitly deleted. **(a)** authorize extending `eslint-gate.test.ts` now as a review-sanctioned exception, or **(b)** hand it to 3.3, which owns the consuming code. Raised by Acceptance Auditor.
- [x] [Review][Patch] **Three confirmed AR-11 regressions in `linkHref`, one code block** [app/scripts/assert-no-external-origins.mjs:212-235] — every one verified as new-gate EXIT=0 / old-gate EXIT=1 on the same fixture. **(1) Whole-value anchoring.** `FETCH_HOST_ONLY = /^FETCH_HOST$/` (`:214`) tests the *entire* attribute value where the pre-3.1 pattern matched a *prefix*; `FETCH_HOST`'s tail class excludes space, quote, apostrophe, backtick, `)`, `<`, `>` and backslash, so any href containing one is dropped to `null`. Leaks: `href="https://cdn.evil.example.com/a b.css"`, `href=".../a(1).css"`, a backslash path separator (browsers normalize it to `/` and fetch), and `href=" https://cdn.evil.example.com/x.css"` — **a single leading or trailing space, which every browser strips before fetching**. Note the spec prescribes `^${FETCH_HOST}$` at Task 4.3, so spec and code agree and **the spec is wrong here**; Task 5.13 guards only against *widening*. **(2) The href reader matches `data-href`.** `\b` holds between `-` and `h`, and `.exec` takes the leftmost match, so `<link rel="stylesheet" data-href="/local.css" href="https://cdn.evil.example.com/z.css">` reads the relative decoy and returns `null`. Next.js already emits `data-precedence` on stylesheet links; any future `data-*href` switches the position off wholesale. **(3) The rel reader matches `data-rel`.** Same boundary, but an escape hatch rather than a miss: `<link data-rel="canonical" rel="stylesheet" href="https://cdn.evil.example.com/y.css">` manufactures a non-fetching rel and waves through a real third-party stylesheet — the deny-by-default policy advertised at `:189-199` defeated by an attribute name nobody reads. Fix: negative lookbehind on both attribute readers so a preceding `-`, word char or `:` disqualifies the match, and prefix-match `FETCH_HOST` against the **trimmed** href instead of full-matching it. Raised independently by all three layers.
- [x] [Review][Patch] `readSiteOrigin` takes the leftmost match, so a declaration inside the doc comment wins [app/scripts/assert-no-external-origins.mjs:91] — `^` under `/m` only requires column 0, which an unindented line inside the block comment satisfies. Verified against the real file: injecting `export const SITE_ORIGIN = "https://staging.example.dev";` into the doc block makes the gate read `https://staging.example.dev`. The drift test cannot catch it either — it counts occurrences of the *current* origin value, so a second declaration carrying a *different* value adds zero. The same hole admits a genuine second declaration later in the file. Fix: collect all matches and fail (exit 2) unless exactly one exists.
- [x] [Review][Patch] The scheme-exactness the comment promises does not exist for the rels that matter [app/scripts/assert-no-external-origins.mjs:143-145] — the comment states "An `http://` self-URL is NOT allow-listed. A canonical emitted over `http` is a real defect and must surface." `NON_FETCHING_RELS` short-circuits `linkHref` **before** `allowed()` is reached, so a canonical or alternate over `http` never consults the allow-list. Verified: `<link rel="canonical" href="http://mundial-stats.juancr.dev/players/">` gives EXIT=0; only the sibling `rel="preload"` form produces EXIT=1. Task 3.3 lists scheme-exactness among details "each with a red proof in Task 8" and this one got neither a proof nor a permanent case. Fix: correct the comment to state its actual scope (fetching rels only) — the rel exclusion is deliberately origin- and scheme-independent, so the code is right and the claim is not.
- [x] [Review][Patch] Make the `SITE_ORIGIN` regex tolerate a trailing carriage return [app/scripts/assert-no-external-origins.mjs:91] — under `/m`, `$` matches before the newline, so a `\r` between the semicolon and the newline breaks the match. Confirmed the repo has **no `.gitattributes` anywhere** and `core.autocrlf=false` locally, so the file is LF today — but a clone with `autocrlf=true` or a scripted rewrite (this repo has a documented history of scripted edits emitting CRLF) exits 2 on every build saying the declaration is missing when it is present and correct. `\s*$` costs nothing.
- [x] [Review][Patch] The extractor sentinel is `null`-only, so an `undefined` return misattributes the failure [app/scripts/assert-no-external-origins.mjs:366-367] — `if (url === null || allowed(url)) continue;` lets `undefined` through to `allowed(undefined)`, whose `.startsWith` throws inside the walk's `try`, so the build dies at exit 2 printing `could not walk <OUT>`. That is exactly the misattribution the `originOf` comment at `:324-338` records as a bug already fixed once ("A REPORTING-ONLY CODE PATH MUST NOT BE ABLE TO FAIL THE BUILD"). Fix: `url == null`.
- [x] [Review][Patch] The recorded rationale about computed keys is factually wrong [app/eslint.config.mjs, the `key.value` comment] — it states "A computed key stays unreachable by any static selector". Verified: `{ ["alt"]: "Texto" }` produces **two** errors (key Literal and value Literal). Only *identifier*-computed keys escape. Harmless in direction, but the next person reasoning about this selector reasons from a false premise.
- [x] [Review][Patch] File List insertion counts are wrong for two of five files — the File List claims `assert-no-external-origins.mjs` `(+148/-9)` and `eslint.config.mjs` `(+50/-4)`. `git diff --numstat f07116b 432dc29 -- app/` gives **141/7** and **46/4**; 148 and 50 are the combined changed-line totals mis-transcribed as insertions. The other three entries and the "9 to 21 cases", "no existing case deleted or weakened" and "exactly one definition" claims all check out.
- [x] [Review][Patch] Stale self-reference in the new comment [app/scripts/assert-no-external-origins.mjs:104] — it cites "this file's own recorded rule (`:254-265` below)" for the exit-2 convention. Post-change, `:254-265` is the `<image>/<use> href` entry; the exit-2 sites are `:382-386` and `:400-406`. The number is the pre-change one, copied from the spec without re-anchoring.
- [x] [Review][Patch] The two deferred-work entries this story declares it owns were never closed — `deferred-work.md:4408` and `:4421` both still read **Owner: Epic 3 story 3-1**, and `deferred-work.md` is absent from commit `432dc29`. No task required it, but the story's References cite both explicitly and the immediately preceding commits on `main` follow the convention (`d3c103c`: "L1553/L1886 closed in the ledger").
- [x] [Review][Defer] `imagesrcset` on `<link rel="preload" as="image">` is a fetching position nothing covers [app/scripts/assert-no-external-origins.mjs:249] — deferred, pre-existing. The `srcset` reader has no word boundary inside `imagesrcset` (`e` and `s` are both word chars) and `linkHref` reads only `href`. Verified EXIT=0 on **both** the new and the `f07116b` gate, so not a regression — but the rewrite's comment explicitly lists `preload` among rels that "stay gated", and for the preload form that actually drives LCP it is not gated at all.
- [x] [Review][Defer] A `>` inside a `<link>` attribute value truncates the tag [app/scripts/assert-no-external-origins.mjs:253] — deferred, pre-existing. The tag pattern cannot cross a `>`, so the match ends mid-tag and neither `href` nor `rel` is found. EXIT=0 on both gates, so not a regression (Edge Case Hunter claimed the old gate caught this; it does not). Worth recording because the rewrite made the tag boundary load-bearing for the **rel** read as well, doubling the blast radius of the same parse weakness.
- [x] [Review][Defer] The drift gate does not scan where the second copy will actually land [app/src/lib/site-origin.test.ts:48-56, 82] — deferred, matches Task 2.3 as prescribed but is narrower than AC1's "in the repository". `app/public/**` is never walked (it does not exist yet — **story 3.4's `public/robots.txt` carrying a `Sitemap:` line is the single most likely second copy**); repo-root `netlify.toml` is outside the scan entirely (confirmed present, carries no domain today); the check splits on the *verbatim full-origin string*, so a bare-host copy (the form used by an `images.domains` list, a CSP `connect-src`, a robots.txt or a Netlify redirect), a concatenated `"https://" + HOST`, or a case variant all count zero — and the origin gate's regex is case-insensitive, so a case variant drifts undetected on both sides. `SKIPPED_DIRECTORIES` at `:30` is also dead code: `walk` is only ever entered at `src`/`scripts`.
- [x] [Review][Defer] The ESLint metadata selector is defeated by hoisting the object one line [app/eslint.config.mjs:202] — deferred, pre-existing for the whole selector (it predates `alt`/`siteName`). Verified: `const OG = { alt: "Texto", siteName: "Mundial Stats" }; export const metadata = { openGraph: OG };` gives **0 errors**. The ancestor anchor is `VariableDeclarator[id.name="metadata"]` / `generateMetadata`, so a sibling const or a `buildMetadata()` helper in another module ships bare Spanish literals with the build green — the exact outcome the comment says these keys were added to prevent. Inline within `generateMetadata` is caught; the hoisted form is not.
- [x] [Review][Defer] Test-coverage gaps that leave stated invariants unpinned — deferred, low. The `NON_FETCHING_RELS` comment claims three consequences "each pinned by a test", but the **unknown-`rel`** one has no fixture (behaviour is correct — an invented rel gives EXIT=1 — the property is just asserted in prose); neither exit-2 branch in `readSiteOrigin` has a test, and its `catch` collapses any `readFile` failure into the same "could not find `export const SITE_ORIGIN`" message, pointing the operator at the constant rather than the real cause; and the ESLint value matcher misses `String.raw`-tagged templates. The `rel="alternate stylesheet"` and no-`rel` cases assert `status===1` plus a host `toContain` where that host also appears on the informational `MENTIONED` line — the `toContain` half is redundant rather than hollow, since `status===1` can only come from the fetching block (Blind Hunter's "prove nothing" framing overstated it).

**Dismissed (verified non-issues, for the record):** self-origin URLs carrying an explicit `:443` port or `user@` userinfo red-build (both reproduce, but Next emits neither form — speculative); Next's `opengraph-image.tsx` `export const alt` convention escapes the selector (real, but epics.md:1131/1186 specify story 3.3 authors `openGraph.images[].alt` and `openGraph.siteName` as object properties, which **are** covered); `applicationName`, `creator`, `publisher`, `keywords` and `authors[].name` remain ungated (out of AC4's scope, never claimed); an array-valued `alt` escapes (not a valid Next metadata shape — `alt` is a string); Edge Case Hunter's claim that the pre-story gate caught the `>`-in-attribute case (it does not — folded into the defer entry above); Blind Hunter's claim that the two REJECT tests "prove nothing" (overstated — `status===1` is load-bearing).

#### Resolution (2026-08-26) — all 12 patches applied, all 3 decisions ruled

The three decision items were put to Juan and all three were ruled the restrictive way:

1. **Self-origin allowance is now position-scoped.** `ALLOWED` was split into `NAMESPACE_ALLOWED`
   (w3.org, schema.org — never a request, any position) and a separate `SELF_ORIGIN` pattern granted
   only at the `<link href>` position and to the two informational lines (`mentions`, `anchors`) via
   `allowed(url, permitSelfOrigin)`. `src`, `srcset`, `poster`, CSS `url()`, `@import`, `fetch()`,
   `import()`, `importScripts()`, `Worker`, XHR, `EventSource` and `WebSocket` are gated on origin
   again, exactly as before story 3.1.
2. **The quoted-key eslint arm now constrains `value.type`** instead of matching any `Literal` child,
   and is its own `no-restricted-syntax` entry. `{ "siteName": t("app.siteName") }` is no longer an
   error; `{ "alt": "Texto" }` and `{ ["alt"]: "Texto" }` still are, and now report once rather than
   twice.
3. **`app/src/lib/eslint-gate.test.ts` was extended** under a review-granted A3/A4 ownership
   exception (the file was verified clean and untouched by any in-flight story). Six cases added,
   40 → 46.

**Code fixes applied:** the `linkHref` cluster (negative lookbehind `(?<![-\w:])` on both attribute
readers, and a `.trim()` + prefix match against `FETCH_HOST` instead of the full-value
`^FETCH_HOST$`); `readSiteOrigin` now collects every declaration and exits 2 unless exactly one
exists, with `\s*$` so a CRLF checkout does not fail it closed; the fetching loop uses `url == null`
so an extractor returning `undefined` cannot be misreported as a walk failure. Three comments that
claimed more than the code enforced were corrected (scheme-exactness scope, computed keys, the
stale `:254-265` self-reference).

**Regression cases added, and every one of them was RED before the fix** (each verified as exit 0 on
the patched-in defect and exit 1 on `f07116b`): six in `assert-no-external-origins.test.ts` (21 → 27)
covering the leading-space href, the space/parenthesis href, the `data-href` decoy, the `data-rel`
spoof, the self-origin tracker + `fetch()`, and the two-declarations exit-2 path; six in
`eslint-gate.test.ts` (40 → 46) covering `alt`, `siteName`, the quoted and computed key spellings,
and both `t()` non-firing directions. That parity gap is what let three false negatives ship green,
so it is now pinned rather than argued.

**Verified after the patches:** `npm run lint` exit 0, `npx tsc --noEmit` exit 0,
`assert-no-external-origins.test.ts` 27/27, `eslint-gate.test.ts` 46/46, `site-origin.test.ts` 2/2,
and a 14-case behavioural probe over the shipped gate (8 defects now caught at exit 1; 6
must-still-pass cases — self-origin canonical, alternate, preload, og:image, off-origin alternate,
SVG namespace — all still exit 0).

**Files this review added to the story's footprint**, beyond the original five:
`app/src/lib/eslint-gate.test.ts` (decision 3), plus `deferred-work.md`, where the two entries the
story owned are now closed.

---

**Ownership verified clean:** commit `432dc29` touches exactly the five owned `app/` paths plus the story file and `sprint-status.yaml`. No `ux-designs/**`, no `spec-sign-the-project.md` path. D20-b holds (`<meta content>` is not in `FETCHING_POSITIONS`, and the AC3 case pins the off-origin `og:image` out of the failure block). All 23 tests in the two owned suites pass on the current tree.
...
| `app/scripts/assert-no-external-origins.mjs` | MODIFY (+141/−7) | `readSiteOrigin()` regex-read with a loud exit 2; escaped, boundary-anchored `SITE_ORIGIN` entry in `ALLOWED`; `NON_FETCHING_RELS` + `linkHref()` extractor; the `FETCHING_POSITIONS` loop's optional fourth tuple element |
...
| `app/eslint.config.mjs` | MODIFY (+46/−4) | `alt` and `siteName` added to the metadata key regex, plus the `key.value` arm for the quoted-key spelling, and the recorded rationale |
