...
Adversarial code review 2026-08-05 over `f2ea99d..04f886e` (3 layers: Blind Hunter, Edge Case Hunter, Acceptance Auditor), scoped to this story's slice across both of its commits — `ab0a87f` (Phase 1) and `04f886e` (CS-2 + Phase 2) — with the interleaved `9adc567` (Story 2.11c review, `app/` only, zero file overlap) excluded. 45 raw findings deduped to 30; 2 dismissed on verification. The 104 emitted `data/matches/*.json` were reviewed as artifacts (rebuilt, re-validated, byte-compared) rather than as diff text. Every finding below was re-read against the code before rating and the reviewers' own severities were discarded.

**The ACs hold on the shipped artifacts.** Independently re-verified: 104/104 bundles schema-valid with zero violations at `schemaVersion 4`; all 11 root keys; the four Domain D tables plus `shootoutAttempts` `null` and never `[]` on 104/104; `knockoutScore` on all 104 including the 72 group ties with the `decidedBy` invariant holding; byte-identical re-runs matching the committed tree (17,871,730 bytes, zero CRLF, no volatile keys); no numeric leaf over its declared `x-decimals`; budget maximum 14,242 gzip-9 bytes (m082), 2.85%, with the gate driven red by construction. D1–D4 all implemented as ruled, D3 with its "re-derive 16/16 and fail loud" condition met. What follows is what sits underneath that.

#### Decisions ruled by Juan (2026-08-05)

**All four ruled as recommended. Each becomes a patch below; the ruling is recorded here so it is not re-litigated.**

1. **CS-2's non-atomic commit — DISCLOSE AND MOVE ON.** The history stays as it is on `main`. The Dev Agent Record and the ledger gain an explicit entry naming the deviation, why it happened and what it cost, so the record stops being silent about a ruled constraint it broke. *Rejected: rewriting `04f886e` into two commits — the schema and the two mappers it unblocks are genuinely entangled, and rewriting pushed history to recover a separation whose purpose (reviewability of the bump in isolation) is already served by the CS-2 spec.*
2. **The U+FFFD in `match-bundle.schema.json:504` — FIX NOW as a byte correction.** A `$comment` carries no semantics, so this is not a change request and takes no version bump; both type trees are regenerated because a description-level edit trips `check:types`. *Rejected: riding the next change-set — it leaves corruption sitting in the canonical artifact set for an unknown interval.*
3. **The emitter's bare-subscript spine reads — ADD THE ENTRY-POINT GUARD.** Required spine keys are asserted at load and raise a typed error naming the missing path. This makes `deferred-work.md`'s claim true rather than correcting it downward, and it is the resolution Task 11.2 names first. *Rejected: correcting the ledger sentence and re-filing only — it leaves an untyped `KeyError` reachable through the CLI for the exact input class the filing described.*
4. **Zero-row `events.shots` / `events.passNetworkEdges` — RAISE.** A match with zero shots or zero pass edges is an extraction defect, not a fact about football, so it is assert-on-unknown per AD-8 exactly like every other missing source. *Rejected: emitting `null` (it would claim the report does not carry a page that demonstrably exists on 208/208) and keeping `[]` (the reading AC 1's binding block warns is semantically different and unfalsifiable downstream).*

#### The findings behind those rulings

- [x] [Review][Decision→Patch] **Change-set CS-2 did not land as its own atomic commit — it rode this story's emission commit** — Task 0 (*"the prerequisite, and NOT part of this story's commit"*), Task 0.1 (*"its own spec file and its own atomic commit, exactly as CS-1 was"*), Task 11.4 (*"It does not ride this story's commit"*) and the PREREQUISITE section all require separation, and AD-14 is the reason. `04f886e` carries CS-2's 20 contract/fixture/app files **and** the 104 `data/matches/*.json` artifacts, both unblocked mappers in `emit.py`, `identity.py`, and three test modules. The CS-2 spec's own checklist ticks *"All in ONE commit"* — true, but that commit is not CS-2 alone, which is what the ruling forbade. Everything CS-2 owed was executed (six declarations, both type trees, seven fixtures, `check:types` green in `contract/` and `app/`); what is missing is the separation and its disclosure. History is already on `main`. Options: disclose the deviation in the Dev Agent Record and the ledger and move on; or rewrite the two commits.
- [x] [Review][Decision→Patch] **A U+FFFD replacement character is committed into `contract/match-bundle.schema.json:504`** — `ShapeByPhase`'s `$comment` reads *"No aggregation was implied by the data � m001 home in-possession lineHeight is 19 / 39 / 54"*: a mangled em dash, the repo's known PowerShell UTF-8 round-trip hazard. Verified it is the only one in the change — 1 occurrence in that file, 0 in `team-profile.schema.json`, `contract/README.md`, both generated trees, `emit.py`, `serialize.py`, `budget.py`, `pipeline/README.md`, `deferred-work.md`, `cs-2-change-set-spec.md`, and 0 CRLF anywhere. It is now baked into `schemaVersion: 4`, and `/contract` is read-only outside a change-set. Options: fix it now as a non-semantic byte correction (a `$comment` edit trips `check:types`, so it needs both type trees regenerated but no version bump — the same carve-out the fixtures-only rule uses); or ride the next change-set.
- [x] [Review][Decision→Patch] **The emitter reads the spine by bare subscript, and the ledger's claim that this is guarded is false** — `deferred-work.md:1789-1792` states the routed `domain_e_checks` filing *"is in fact present in a stronger form: `check_total` asserts every emitted object's key set against its `$def`, so a record staged by an older checkout surfaces as a typed `UnmappedFieldError` … rather than a bare `KeyError`."* `check_total` only ever inspects the dict a mapper has **already built**; every mapper reads the spine by bare subscript first (`build_key_statistics` `block["home"][s]`, `_lineup_entry` `entry["player_id"]`, `build_shots` `row["time_raw"]`). Reproduced: deleting `domains.key_statistics.home.second_balls` from one spine file makes `build_bundle` raise `KeyError('second_balls')` — untyped, and through the CLI an uncaught traceback. Task 11.2 offered exactly two sanctioned resolutions and the choice is yours: add the record-shape guard at the emitter's entry point, or correct the claim and re-file with a named successor.
- [x] [Review][Decision→Patch] **`events.shots` and `events.passNetworkEdges` emit `[]` for a zero-row page, collapsing the `null`-vs-`[]` distinction their five siblings declare** — `emit.py:1147` and `:1157`. `build_shots`/`build_pass_network_edges` always return a list; a spine whose shots or pass-network page staged zero rows would emit `[]` = *"zero events occurred"*, not `null` = *"the report does not carry that data"*. Neither has `minItems`, so it validates. Unreachable on today's corpus (0 of 208 innings empty) and wholly ungated. The other five keys each carry a declared `null` with a reason; these two carry no absence check at all. Options: raise (assert-on-unknown, matching AD-8); emit `null`; or record `[]` as the deliberate reading.

#### Patches

- [x] [Review][Patch] **`tacticalIdentity`'s 20 percentages and `shapeByPhase`'s 9 metres are rounded to 0 places instead of their declared 1** [pipeline/precompute/emit.py:1273] — `_KEY_TO_DEF` binds `"home"`/`"away"` to `Count` for `TeamScore`, `round_bundle` (`:1215`) passes a parent's binding down to any key it does not recognise, and none of `phasesInPossession.*` (8), `phasesOutOfPossession.*` (9), `defensiveBlockDistribution.*` (3) or `shapeByPhase.*.*.{lineHeight,teamLength,teamWidth}` is in the table — so all 29 inherit 0. Reproduced: `lineHeight 19.5 → 20`, `buildUpUnopposed 44.4 → 44`. `Percentage`/`Metres` are `type: number`, so a truncated bundle validates clean. Nine goalkeeping count keys (`totalInvolvements`, `involvements`, `attemptsFaced`, `totalInterventions`, `lineBreaks`, `crossesFacedAttempted`, `complete`, `incomplete`, plus every member of `byInterventionType`/`deliveryTypesFaced`) have no binding at all and are never coerced to `int`. No corpus value is wrong today — 0 of 4,160 phase values and 0 of 3,744 metre values are fractional — but this is landmine 3 armed rather than disarmed, for the one domain CS-2 was created to unblock. Visible from the other side: the regenerated fixture emits `19.0` where the bundle built from the same staged panel in the same commit emits `19`.
- [x] [Review][Patch] **Both precision tests derive their expectation from the emitter's own key map, so neither can see the defect above** [pipeline/tests/test_emit_bundles.py:686] — `:686` and `:1198` both open with `by_key = emit.precision_by_key(decimals)` and then assert against it: the implementation checked against itself, which is exactly the `emit(x) == emit(x)` failure mode Task 10.6 names. Compounding it, `_numeric_leaves` (`:665-675`) collects `float` only, and `round_to_precision(x, 0)` returns an `int`, so every wrongly-0-placed leaf is invisible. Task 10.5 asks for the expectation to be **derived from the schema**: resolve each instance path through `$ref` to its `$def` independently of `_KEY_TO_DEF`. Related and cheap: `precision_by_key` (`emit.py:1285`) silently drops any `$def` name missing from `decimals_map` (`if name in decimals`) with no assertion that every value in the table resolved.
- [x] [Review][Patch] **A short or empty spine deletes committed bundles before the count check runs** [pipeline/precompute/emit.py:1404] — the stale sweep unlinks every file not in `keep` immediately after the write loop, while `main` checks `count == 0` and `--expect-matches` only after `emit_bundles` returns (`:1458-1464`). Reproduced twice independently: an empty `matches/` dir against a populated `data/` prints `FAIL: no bundle was emitted`, exits 1, and leaves `data/matches/` **empty**; a 50-match spine with `--expect-matches 104` writes 50 and deletes the other 54 before reporting the miss. A truncated spine, a `precompute.run` that stopped early or a mistyped `--spine-dir` therefore destroys the committed corpus and leaves `check_committed_data` pinning a partial namespace — landmine 14 verbatim, and the opposite of `emit_bundles`' own docstring (*"a breach anywhere leaves `data/matches/` untouched rather than partial"*). Both existing tests stage into a fresh `tmp_path/data`, so neither observes the deletion.
- [x] [Review][Patch] **`--dry-run --expect-matches N` always reports FAIL, and `--dry-run` over a zero-match spine reports PASS** [pipeline/precompute/emit.py:1461] — `emit_bundles` returns `[]` under `dry_run` (`:1391`), and `main` guards the zero-count FAIL with `not args.dry_run` but not the `--expect-matches` comparison. Verified: `--expect-matches 104 --dry-run` validates and measures all 104 successfully, then prints `FAIL: emitted 0 bundle(s), expected 104`, exit 1. The pre-write validation run a reviewer would naturally reach for can never pass, and the vacuous-pass rule the code cites is skipped on the branch where it is easiest to hit. Both flags are in Task 2.5's table; no test exercises the pair.
- [x] [Review][Patch] **A corrupt spine file exits 1 with a traceback instead of the documented 2** [pipeline/precompute/emit.py:1337] — `load_spine` uses `json.loads` on `entities.json` and every match file; `main` catches `PipelineError` → 1 and `OSError` → 2 (`:1445-1451`), and `json.JSONDecodeError` is a `ValueError`. Reproduced by truncating one spine file: raw traceback on stderr, exit 1. `decimals_map`'s vacuity `AssertionError` escapes the same way. Task 10.6a asks for exit 2 on an unreadable spine, and `errors.py:12-14` reserves 1 for a dataset finding — this reports a broken harness as a finding. The existing test covers only the absent-directory case, which happens to raise `OSError`.
- [x] [Review][Patch] **`metadata.lineups` has no duplicate-id assertion, though Task 4.6a and the ledger both say it does** [pipeline/precompute/emit.py:269] — Task 4.6a: *"Raise on a duplicate within one bundle's `players`, and within `metadata.lineups` too"*; `deferred-work.md:1680-1683` reports the 2.11b filing **DISCHARGED**, *"Asserted at the emitter for both `players[]` and `metadata.lineups` … Closed."* `emit.py` has exactly one such check, in `build_players` (`:797-801`); `_lineup` maps `starters`/`substitutes` with no uniqueness check. A spine repeating a `player_id` between the two groups emits a schema-valid bundle with duplicate lineup entries and duplicate React keys. The corpus assertion at `test_emit_bundles.py:1140-1143` is green because today's data is clean; it is not the emitter guard both documents claim.
- [x] [Review][Patch] **The shipped `emit.py` module docstring still declares the module blocked, in the commit that unblocks it** [pipeline/precompute/emit.py:26] — the `BLOCKED-PENDING-CS-2` banner ends *"Until it lands, `build_bundle` raises `EmitError` naming them."* CS-2 landed here and `build_bundle` (`:1288`) emits both mappers. `BLOCKED_ON_CS2` (`:68`) is now referenced by nothing despite its comment claiming the CLI reports the block with it; `build_bundle_partial` (`:1318`) is a self-described deprecated alias six live tests still call; and `test_emit_bundles.py:835-841` repeats the same false premise to justify a `monkeypatch` that is no longer needed. The first thing a reader of the emitter sees is wrong about what the emitter does — and Task 11.6 makes that docstring the place a reviewer reads the module's reasoning.
- [x] [Review][Patch] **`test_set_play_counts_are_internally_consistent` still asserts the relation CS-2 just documented as corpus-false** [pipeline/tests/test_fixtures.py:906] — it asserts `direct == directOnTarget + directOffTarget` and its docstring still reads *"Free kicks are NESTED"*, while CS-2 rewrote `FreeKickCounts`' description to say that relation *"is CORPUS-FALSE — it holds on 0 of 208 team-innings"* and `build_set_plays` deliberately does not assert it. The test stays green only because it globs `data/fixtures/`, whose hand-authored values satisfy it. When the fixtures are regenerated from real data (1.18/1.19) it turns red for a reason that is not a defect — precisely the outcome Task 4.5b and landmine 9 spend paragraphs avoiding elsewhere. Re-scope it to the four corpus-true relations and correct the docstring.
- [x] [Review][Patch] **The single-candidate penalty join skips the elapsed check the multi-candidate branch enforces** [pipeline/precompute/emit.py:330] — `shot_elapsed` is computed at `:329` and then discarded when a scorer has exactly one lineup goal, so a penalty-delivery goal shot at 70' whose scorer's only lineup goal is at 30' flags the 30' goal `penalty: true` and raises nothing; the ≥2-candidate path raises on the same disagreement. D3's ruling is *"fails loud on any unmatched penalty-goal shot"*. Measured over the corpus: of the 16 penalty goals, 11 take the single-candidate branch and **11/11 agree exactly** on elapsed count, so asserting equality here closes the gap without touching a single emitted value.
- [x] [Review][Patch] **`build_bundle` failures abort on the first offending match, against the "collect, then raise" policy** [pipeline/precompute/emit.py:1359] — `emit_bundles` collects schema violations and budget breaches (`sorted(...)[:10]`, `"; "`, `" …"`) but lets `EmitError`/`UnmappedFieldError` propagate immediately. The Failure & validation policy names *"budget, unmapped fields, validation"* as the three classes to gather, *"Aborting on the first turns one run into ten."* Two of the three are collected.
- [x] [Review][Patch] **Two spine files carrying the same `match_id` inflate the reported count and pass `--expect-matches`** [pipeline/precompute/emit.py:1394] — `written` appends per built entry, not per distinct path, and `load_spine` keys only on filename. Reproduced: `m001-….json` copied to `zz-duplicate.json` with `--expect-matches 2` prints `bundles: 2`, `EMIT RESULT: PASS`, exit 0, while `data/matches/` holds **one** file — the second write silently overwrote the first. The count that exists to prove 104 bundles were emitted proves nothing about distinctness.
- [x] [Review][Patch] **The goal-prevention fixture test reads a key CS-2 deleted, and an unguarded field CS-2 made nullable** [pipeline/tests/test_fixtures.py:933] — both assertion messages interpolate `keeper['playerId']` while iterating `bundle["goalkeeping"]`, whose items are now `TeamGoalkeeping` (verified keys: `teamId, goalkeepers, totalInvolvements, involvementTimeline, distribution, goalPrevention, aerialControl`). Python evaluates the message only on failure, so the moment either denominator invariant actually breaks the test dies with `KeyError: 'playerId'` instead of naming the bundle and the relation. The same block sums `prevention["byBodyType"]` unguarded (`:935`) on a field CS-2 made nullable and which is `null` on 208/208 real bundles — the identical unguarded-nullable class Task 9 swept six instances of, newly reintroduced by CS-2 and not swept.
- [x] [Review][Patch] **`goalkeeping-model.ts`'s two docblocks now assert the opposite of the contract CS-2 shipped** [app/src/viz/goalkeeping-model.ts:592] — the involvement-series docblock still states the sample is *"a BARE `Minute`, 0-120, WITH NO STOPPAGE FIELD"* and *"Filed to the successor change-set as a BLOCKER"*, while the code it documents now reads `sample.at.minute`. The tick docblock (`:676`) rules its interim behaviour explicitly **because** *"Both mechanisms read `row.at.stoppageMinute`, WHICH THIS CONTRACT DOES NOT CARRY"* — it does now, so `involvementTicks`' minute-dedupe (`:684`) is an interim whose condition has expired, and `deferred-work.md:1939-1943`'s *"Closed, and 2.19 owes no App change for it"* is right for the index-keyed series and over-claimed for the ticks. Measured: 2,506 of 21,764 emitted samples carry a non-null `stoppageMinute` and every one collides with another sample on the same `minute`. Correct the docblocks and the ledger claim here; re-opening the tick model is 2.19's.
- [x] [Review][Patch] **A shoot-out prose with equal scores is accepted whenever the named winner is the away team** [pipeline/precompute/emit.py:531] — the guard is `(home_goals > away_goals) != winner_is_home`; with `home == away` the left side is `False`, so an away-named winner passes. Reproduced: `_parse_shootout("(Paraguay win 3-3 on Penalties)", …)` with Paraguay away returns `{'home': 3, 'away': 3}` and emits `decidedBy: "shootout"` with a winner, while the home-named form correctly raises. The assertion exists to prove the home-away reading rather than assume it; a tie is a state no shoot-out can end in and must raise from either side.
- [x] [Review][Patch] **`decidedBy` is derived from the momentum clock alone, with an independent discriminator sitting unused** [pipeline/precompute/emit.py:437] — `went_to_et` comes only from `periods_played` (`:150-154`, `max(sample.minute) > 90`). If a match's momentum series were truncated at 90 while the tie was actually decided in extra time with no shoot-out prose, `decidedBy` reads `regulation`, `scoreAfterET` is `null` and `scoreAfter90` under-counts — and nothing catches it, because `tally(120)` still equals the cover score and a winner still exists. `metadata.goals` already carries real stamps, and regulation stoppage stamps as `{90, N}`, so any goal at `minute > 90` proves extra time. Cross-check the two clocks and fail loud on disagreement (the corpus is clean: 95 regulation / 5 extra-time / 4 shootout).
- [x] [Review][Patch] **Task 3.3's key-set equality assertion is not applied to every emitted object** [pipeline/precompute/emit.py] — built without `check_total`: `metadata.score`, `TeamSetPlays.freeKicks` (`FreeKickCounts`), `cornersByDeliveryType`, `cornersByDeliveryStyle`, `GoalPrevention.byInterventionType`, and the three `TeamScore` objects inside `knockoutScore`. Low blast radius — each is a literal dict and jsonschema still enforces `required` + `additionalProperties: false` — but the numbered sub-binding says *every* emitted object, and that totality is the whole point of preferring key-set equality over the no-underscore check.
- [x] [Review][Patch] **Nothing pins the committed `data/matches/` against what the emitter produces** [pipeline/tests/test_emit_bundles.py] — every corpus test builds from `work/spine/`; `test_the_committed_data_baseline_is_populated_and_every_id_in_it_is_pinned` counts 104 files and checks pinning, nothing more. The property holds today — all 104 rebuilt bundles are byte-identical to the committed tree, 17,871,730 bytes — but the artifacts are committed under AD-13 and drift between the tree and the emitter is the failure mode that matters for a consumer reading `/data`. A rebuild-and-compare test, skipping when the spine is absent, is the same shape as the other corpus tests.
- [x] [Review][Patch] **`momentum: null` is unreachable, and one momentum-less match takes all 104 down** [pipeline/precompute/emit.py:1181] — `build_momentum` returns `None` for an empty series, but `build_bundle` evaluates `metadata` first and `periods_played` (`:150-154`) raises `EmitError` on the same input. The contract models `momentum: null` as a legal state with a shipped App empty state; emission can never produce it. The raise is defensible — the period structure genuinely depends on that clock, and D4 could not decompose a shot without it — but the dead branch and the corpus-level abort should say so rather than read as an oversight.
- [x] [Review][Patch] **Dead code left by the retired `#pressing` metre presentation** [app/src/components/PressingSection.tsx:8] — the `formatDecimal` import (`:8`) and the module constants `UNIT_SEPARATOR` (`:90`) and `ACCENT_CLASS` (`:98`) have no remaining reference after `metreBlock` was deleted. `eslint --max-warnings 0` and `tsc --noEmit` are both green (Next's config does not flag unused module-scope consts and `noUnusedLocals` is off), so the green-build proof cannot see it. Two neighbouring comments are now false: `PressingSection.tsx:171` (*"the rate tables and the metre table … so the three can never drift apart"* — there are two) and `TacticalLayer.tsx:280` (*"the four press rates plus the blocks plus the metres"*).
- [x] [Review][Patch] **The Dev Agent Record reports two different corpus budget maxima with neither marked superseded** [1-16-…md:712] — Phase 1's Verification block says *"11,784 gzip-9 bytes (m082), 2.36%"*; Phase 2 says *"14,242 … 2.85%"*. Re-measured over the committed tree: **14,242** for `m082-belgium-senegal`, 2.85%. Phase 1's figure measured bundles that were missing two root keys and describes nothing that exists; it reads as a live verification result.
- [x] [Review][Patch] **The regression tally offered as proof that no module was skipped is off by one** [1-16-…md:614] — *"1,481 collected … 1,371 + 110 new = 1,481 — so no module was skipped or double-counted."* Re-collected at HEAD: **1,482**. The arithmetic omits the one non-parametrized test this story added to `test_fixtures.py` (`test_a_null_node_table_does_not_silently_skip_a_real_dangling_edge`, Task 9.2's constructed pin), so 1,371 + 110 + 1 = 1,482. The sum exists precisely to substitute for a single un-chunked run; off by one it substitutes for nothing. The suite itself is healthy — `test_emit_bundles` 95 passed, `test_emit_serialize` 15 passed, `test_fixtures` + `test_contract_schemas` + `test_precompute_spine` 233 passed / 1 skipped.

#### Deferred

- [x] [Review][Defer] **An `OSError` mid-write leaves `data/matches/` partially populated with no rollback** [pipeline/precompute/emit.py:1394] — deferred, needs a staged-directory swap
- [x] [Review][Defer] **`i18n.test.ts`'s 27-caption accessibility inventory still lists a caption no section renders** [app/src/lib/i18n.test.ts:1456] — deferred, `app/` surface work owned by 2.19
- [x] [Review][Defer] **`goalkeeping-model.ts` synthesizes a joined `playerId` and a hand-composed display name for the two-keeper innings** [app/src/viz/goalkeeping-model.ts:111] — deferred, `app/` surface work owned by 2.19

#### Verification of the patched state

**Full `pipeline/tests` suite: 1,501 collected — 1,500 passed, 1 skipped, 0 failed.** Run in
four chunks (393 + 392 + 451+1skip + 264 = 1,501), every one of the 43 modules executed. The
tally now reconciles exactly and is stated as a check rather than an argument: 1,482 at review
start + the 19 tests this review added = 1,501.

**App chain green in the shared tree:** `tsc --noEmit`, `eslint --max-warnings 0`, and the
full app suite at **741 passed / 0 failed** across 26 files. Both `generate:types` re-run after
the `$comment` byte fix; neither produced a diff, and no version stamp moved — correct for a
non-semantic correction.

**Acceptance runs:** `emit --expect-matches 104` exit **0**; `emit --expect-matches 104
--dry-run` exit **0** (it reported FAIL by construction before this review); `run
--expect-records 104` exit **0**, still printing *"104 bundle(s), 89,358 id reference(s), all
pinned"*.

**The corpus changed, once, and only where the precision fix reaches:** 7,904
`tacticalIdentity` values across the 104 bundles now serialize at their declared 1 decimal
place (`19.0` in place of `19`), which is what `x-decimals: 1` always meant and what the
regenerated fixtures already carried. Nothing else in any bundle moved. Re-measured: 104
bundles, 17,887,538 bytes, byte-identical across two runs and equal to the committed tree,
zero CRLF, 104/104 LF-terminated; budget maximum 14,251 gzip-9 bytes (m082), 2.85%.

#### Dismissed on verification

- **"The disclosed CS-2 `app/` blast radius is 6 files; 7 changed."** A counting convention, not a discrepancy: the two regenerated type-tree files are one generated output and the two `.test.ts` files travel with their models. Nothing is hidden — every file is in the File List.
- **"Task 10.7's full suite was run in seven chunks rather than one background pass."** A documented departure, forced by an environment that kills a 45-minute invocation, disclosed rather than hidden, and every one of the 43 modules was executed. The reconciliation arithmetic it leaned on is filed as a patch above; the chunking itself is not a finding.
...
**DISCLOSURE ADDED IN CODE REVIEW: CS-2 did NOT land as its own atomic commit.** Task 0
("the prerequisite, and **NOT part of this story's commit**"), Task 0.1, Task 11.4 and the
PREREQUISITE section all require it to be separate, and AD-14 is the reason. Commit
`04f886e` carries CS-2's schema, fixture and `app/` files together with the 104 emitted
bundles, both unblocked mappers, `identity.py` and three test modules. Everything CS-2 owed
was executed and the CS-2 spec's own checklist ticks *"All in ONE commit"* truthfully —
that commit simply is not CS-2 alone. Juan ruled: disclose and move on, rather than rewrite
pushed history to recover a separation the CS-2 spec already provides for review purposes.
The paragraph below was written before that was noticed and its first clause is wrong.
...
> **RE-MEASURED AFTER THE CODE REVIEW's precision fix, which changed every bundle's bytes.**
> 7,904 `tacticalIdentity` values now serialize at their declared 1 place (`19.0`, not `19`),
> so: **104 bundles, 17,887,538 bytes**, byte-identical across two runs and matching the
> committed tree, zero CRLF, 104/104 LF-terminated. Corpus budget maximum **14,251 gzip-9
> bytes (m082), 2.85%**. `emit --expect-matches 104`, `emit --expect-matches 104 --dry-run`
> and `run --expect-records 104` all exit **0**, the last still reporting 104 bundle(s) /
> 89,358 id reference(s), all pinned.

**FULL REGRESSION SUITE: 1,480 passed, 1 skipped, 0 failed.** Run in seven chunks rather
than one pass: a single 45-minute invocation is killed in this environment, so it was
chunked and every one of the 43 test modules was executed. **CORRECTED IN CODE REVIEW: the
collected count is 1,482, not 1,481, and the reconciliation as written was off by one.**
1,371 pre-change + this story's 110 new tests (95 `test_emit_bundles` + 15
`test_emit_serialize`) omits the one non-parametrized test added to `test_fixtures.py`
(`test_a_null_node_table_does_not_silently_skip_a_real_dangling_edge`, Task 9.2's
constructed pin), so the sum is 1,371 + 110 + 1 = 1,482. The arithmetic exists precisely to
substitute for a single un-chunked run; off by one it substituted for nothing.
...
gzip-9 bytes (m082), 2.36% of the 500,000 ceiling — **SUPERSEDED: that figure measured
Phase 1 bundles, which were missing two root keys and no longer describe anything that
exists. The live figure is Phase 2's 14,242 gzip-9 bytes (m082), 2.85%**, re-measured
against the committed tree in code review. Byte-identity asserted on bytes, not parsed
dicts. Full regression suite re-run after all changes.
