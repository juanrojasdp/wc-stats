...
⚠️ **THE HEADINGS SENTENCE WAS AMENDED AT THIS STORY'S CODE REVIEW (2026-08-27, ruled by Juan).**
As written above it could not hold on `/players` and `/teams`: their section headings ARE the
tournament's team names, so rendering them with the shell requires the build-time read of the
409 KB index that this very table forbids on those two routes. The clause and the AD-11 ban were
in direct contradiction, and the ban won. `EXPERIENCE.md:417` now scopes the sentence to headings
that are **not artifact-derived**, naming `/players` and `/teams` as the exception. `/tournament`
and `/tops` are unaffected and still render their section headings with the shell — verified in
the export (`out/tournament/index.html` ships "Tabla de posiciones"; `out/tops/index.html` ships
its full teasers). The two index routes render their `<h1>` with the shell and everything
data-derived behind skeletons shaped like it.
...
Three parallel layers: **Blind Hunter** (adversarial, no spec), **Edge Case Hunter** (boundary walk,
no spec), **Acceptance Auditor** (AC 1–5, D1–D17, A1 against the spec). All layers completed; none
failed. 24 raw findings → **20 after dedup**, 2 dismissed. Every finding below was re-verified
against the working tree and the built `out/` export before it was rated; severities here are the
reviewer's, not the subagents'.

**Independently confirmed by this review, not taken from the Dev Agent Record:** the export is
**1,410 `index.html` / 1,411 documents / 1,408 sitemap `<loc>`**; the suite is **63 files / 1,546
tests / 0 skipped**, exit 0; all eleven `page.*` files are plain `.tsx` with no route group,
parallel route or private folder; all nine `available` flags are `true`; `id="leaders"` occurs in
**exactly one** document in the whole export (`out/tops/index.html`); `/` renders **zero** `<table>`
elements and eight badges in D1's ruled order, every href trailing-slashed with the slash before
`#`; and no new route file declares an `alternates` or `twitter` key. **AC 3, AC 4 and AC 5 are
satisfied. AC 1 and AC 2 are satisfied except where noted below.**

### Decisions needed

- [x] [Review][Decision] **`/players` and `/teams` ship a shell with no section headings or counts, against the State Patterns clause D5b quotes for them by name** [app/src/app/players/page.tsx:117-125, app/src/app/teams/page.tsx:95-99] — `medium`. D5b quotes the contract verbatim and binds it to these two routes: *"**Section headings and their counts render with the shell** so the surface's shape is readable before any data lands."* Measured in the export, the `<main>` of `out/players/index.html` contains the single word `Jugadores` and four unlabelled skeleton divs; `out/teams/index.html` contains `Equipos`. For contrast, `out/tournament/index.html` ships `El torneo` **and** `Tabla de posiciones`, and `out/tops/index.html` ships its full teaser content — both meet the clause. The tension is real and is not the dev's error: the 48 group names live in `tournament.json`, and D5b's own table bans a build-time read of that artifact on these two routes, so there is no way to render the headings with the shell without either inlining the 409 KB index (banned) or hardcoding 48 team names in the App (AD-5 forbids the App re-deriving pipeline order). The resolution was recorded only in a code comment (`players/page.tsx:117-122`), never filed as a deviation, so the contract clause is left silently unmet. **Options: (a) accept the shell as-is and file the deviation in `deferred-work.md` against UX-DR24/D5b; (b) render the section headings from a small build-time projection of team names only (~48 short strings, not the 1,248 players) — a third data path D5b does not currently allow, so it would need the ban amended; (c) re-rule the clause as not binding on index routes.** — **RULED 2026-08-27 (Juan, at review): RE-RULE THE CLAUSE.** The State Patterns row is not binding on an index route whose section headings are themselves artifact-derived. The clause was written for the Hub, where the headings (rounds, groups) are structural and known without reading the corpus; on `/players` the headings *are* the data, so the clause and AD-11's inlining ban cannot both hold and the ban is the one with teeth. **Now a patch:** amend `EXPERIENCE.md:417`'s *"Cold route load — `/tournament`, `/tops`, `/players`, `/teams`"* row to scope the "section headings and their counts render with the shell" sentence to routes whose headings are not artifact-derived, naming `/players` and `/teams` as the exception and stating why; then correct D5b in this story file to match, and replace the code comment at `players/page.tsx:117-122` with a citation of the amended clause rather than a private justification. **This re-opens a ruled contract decision, which D17 fences this story off from doing — it is permitted here only because the ruling authority made it at review**, the same route `3-10`'s two review decisions took (`3-10-...md:1438-1439`). Note the amendment is a contract edit, so it also trips `deferred-work.md:4821`'s note about `epics.md`'s UX-DR24 token list if that file is touched in the same pass.

- [x] [Review][Decision] **`epic-3` is still `in-progress`, so D15's first obligation is undischarged** [_bmad-output/implementation-artifacts/sprint-status.yaml:3488] — `low` as a defect, but a live obligation. D15 is normative and unconditional: *"1. Flip `epic-3: in-progress` → `done` at `sprint-status.yaml:3488`."* It has not been flipped. The Dev Agent Record argues (story file `:1488-1504`) that this repo closes a story in two steps — `review`, then `done` after a separate code-review run, citing `77d4d53` → `24906d4` as the most recent pair — and that flipping the epic on an unreviewed story would be wrong. **That reasoning is sound and this review is precisely the gate it names.** The obligation is recorded in two places in `sprint-status.yaml` and handed to whoever flips 3.9 to `done`. **Options: (a) flip `epic-3` → `done` now, as part of this review's status sync, and treat D15 as discharged; (b) leave it to a separate pass and amend D15 to describe the two-step close it actually collides with.** Note also that the record cites `epic-3-retrospective` at `:3892` while D15 cites `:3791`; `:3892` is the correct current line. **A6 stands either way: the Epic 3 retrospective is due and is a separate `bmad-retrospective` run.** — **RULED 2026-08-27 (Juan, at review): the two-step close is CORRECT and D15 is discharged on the `done` flip, not before.** This code review is the gate D15's premise required; the dev's correction stands rather than being a shortfall. **Now bookkeeping, not a patch:** when this story reaches `done`, flip `epic-3: in-progress` → `done` at `sprint-status.yaml:3488` in the same pass, bump `last_updated`, and leave `epic-3-retrospective: required` at `:3892` untouched so A6 stays owed. **The retrospective is NOT improvised here** (D15) — it is a separate `bmad-retrospective` run and is the last thing Epic 3 owes.

### Patches

- [x] [Review][Patch] **`/tops` ships zero `<h1>` — the only route on the site without one, and the file's own docblock claims the opposite** [app/src/app/tops/page.tsx:132-153] — `high`, found independently by all three layers. The page body mounts `SortAnnouncerProvider > LeaderboardsSection` and nothing else, and `LeaderboardsSection` leads with `<h2 id="leaders-title">` (`LeaderboardsSection.tsx:99`). Measured across the export: `out/tops/index.html` has **0** `<h1>`; `/`, `/tournament`, `/players`, `/teams`, `/about`, `/compare`, `/glossary` have exactly **1** each. The `<h2>` was correct while the section sat *under* the Hub's `<h1>` on `/`; re-siting it to its own address made it the document root, so `/tops` now has an outline starting at h2 with no page heading and a screen-reader user rotoring by heading finds no level-1. **This repo has already ruled on this exact defect:** `ComparePicker.tsx:177-188` records it being found and fixed at code review 2026-08-07 — *"left `/compare` the ONLY route in the app with no `<h1>` at all."* Compounding, `tops/page.tsx:29-31` asserts *"`es.ts` ships `leaderboards.title: "Líderes del torneo"` as this page's own `<h1>`"* — a false statement about the code in the same file — and `:128-131` justifies the `py-` container on the grounds the route "leads with its own `<h2>`", where D2 conditions `py-` on leading with an `<h1>`. The Dev Agent Record's own browser verification (story `:1344`) recorded `h2: "Líderes del torneo"` without flagging the absent h1. Fix: promote the section heading to `<h1>` (it is rendered once, on `/tops` only, and `HEADING_ID`/`aria-labelledby` are unaffected), or add a page-level `<h1>` in the shell as `/players` and `/teams` do — then correct both stale docblocks and re-point the assertion.

- [x] [Review][Patch] **Task 9.4 was not achieved for two of the four new routes: `everyRouteHtml()` never sweeps `/players` or `/teams`, and its non-vacuity floor passes by coincidence** [app/src/app/static-output.test.ts:852, :866-873] — `high`, found independently by all three layers. The static loop skips `players` and `teams` on the grounds that "the dynamic families are walked BELOW", but that walk (`:882-891`) only opens `${family}/${slug}/index.html` — `out/players/index.html` is a *file* at the family root, not a slug directory, so it is reached by neither half. **Reproduced against the real export:** the static list is exactly `/ | /404 | /404 | /_not-found | /about | /compare | /glossary | /tops | /tournament` — nine documents, with `/404` counted twice (`404.html` and the un-skipped `404/index.html`) and Next's internal `/_not-found` included, while `/players` and `/teams` are absent. The comment at `:866-873` states the floor is *"/, /404, /about, /glossary, /compare, /tournament, /tops, /players, /teams"* and asserts `>= 9`; **two of the nine it names are not in the set and two documents in the set are not named**, so the guard is satisfied by the double-count and cannot detect its own gap. The six cases titled *"on EVERY exported route"* (`:944`, `:970`, `:1017`, `:1092`, `:1121`) therefore do not cover two of the four routes this story minted — the precise regression the surrounding comment claims to have closed, re-introduced by the change that closed it. The dedicated non-vacuity case at `:924-942` passes too, because it asserts only that *some* route under each family was swept, which the 1,248 profile slugs satisfy. Fix: add the family-root `index.html` for each family that has one, exclude `/_not-found` and the duplicate `/404`, and make the floor name the documents it actually asserts.

- [x] [Review][Patch] **`/players` and `/teams` render their `<h1>` with a server `t()`, freezing it in Spanish while the body follows the language toggle** [app/src/app/players/page.tsx:125, app/src/app/teams/page.tsx:99] — `high`. Neither file carries `"use client"`; both import `t` from `@/lib/i18n` (`:4` in each), which defaults to `es`, so the heading is baked into the static HTML and the toggle cannot reach it. Every other `<h1>` in the app renders from a client component via `useT()` — `AboutContent`, `GlossaryContent`, `ComparePicker`, `MatchHero`, `PlayerHero`, `TeamHero`, `TournamentHubHeading`, and this story's own `LandingContent.tsx:95`. `TournamentHubHeading` (`TournamentHub.tsx:821`) exists for no other reason than to hold one `<h1>` on the client, and `/tournament` correctly delegates to it. **`LandingContent.tsx:35-41` names this exact trap** — *"A server-`t()` surface FREEZES SPANISH and ignores the language toggle … the one deferred item 6.2 closed under the title 'Home page body ignores the language toggle'"* — and the two sibling routes do it anyway. Nothing goes red because the ESLint i18n seam is scoped to `src/components/**`. Toggle to EN on `/players`: "Jugadores" sits above an otherwise fully English filter, group headings and table. Fix: mirror `TournamentHubHeading` with a small client heading component, or move the `<h1>` into the client Region.

- [x] [Review][Patch] **The `/` → `[]` artifact allow-list cannot detect the violation its docblocks say it catches** [app/src/app/static-output.test.ts:694, :763-771] — `medium`. The module-graph walk matches only `/fetchArtifact\s*<[^>]*>\s*\(/`, so a **build-time** read via `readTournament()` / `readLeaderboards()` from `@/lib/build-data` is invisible to it. Add either call to `app/src/app/page.tsx` and `reachable` stays `[]` — the case passes green while the 409 KB index is inlined into the landing document, which is exactly the AD-11 violation D5b calls out in red. **Proof the walk is blind to build-time reads is already in the same table:** `app/src/app/tops/page.tsx:124` calls `readLeaderboards()`, and the gate's expected set for `/tops` is still the single *runtime* path. Three separate docblocks (`page.tsx`, `LandingContent.tsx`, `static-output.test.ts`) assert this gate holds the AD-11 line; it holds only the runtime half. Task 11.8c drove it red by adding a *region fetch* — the runtime direction — so the build-time direction has never been exercised. Fix: extend the walk to `build-data` imports, or narrow the three docblocks to the scope the gate actually has.

- [x] [Review][Patch] **All 48 disclosure triggers announce a doubled command: "Ver los datos: Ver los jugadores de Argentina"** [app/src/components/PlayersIndexRegion.tsx:338, :351] — `medium`. `triggerName` is composed as `t("players.teamTrigger")` + `" "` + `group.teamName` — `"Ver los jugadores de Argentina"` — and passed as `ViewDataDisclosure`'s `panelTitle`, which at `ViewDataDisclosure.tsx:109` composes `accessibleName = t(labelKey) + ": " + panelTitle` with `labelKey` = `viz.viewData` = `"Ver los datos"`. The rendered `aria-label` is therefore **"Ver los datos: Ver los jugadores de Argentina"**, 48 times; EN is equally doubled (*"View data: View the players of Argentina"*). D4's ruled string is *"Ver los jugadores de Argentina"*. The prop's own docblock (`ViewDataDisclosure.tsx:56-62`) documents the intended shape as a bare title giving *"Ver los datos: Mapa de tiros"*, and `PlayersIndexRegion.tsx:331` quotes that composition rule immediately before violating it. The team is named either way, so this is a wording defect rather than an information one. Fix: pass `group.teamName` as `panelTitle`, yielding "Ver los datos: Argentina"; `players.teamTrigger` then becomes dead and should go with it.

- [x] [Review][Patch] **A schema-valid but shape-malformed payload lands in the retryable `error` state whose retry can never issue a network request** [app/src/components/PlayersIndexRegion.tsx:120-141, app/src/components/TeamsIndexRegion.tsx:78-93] — `medium`. Both regions dereference two levels into an `as`-cast payload *inside* `.then` — `setPlayers(payload.entities.players)` / `setTeams(payload.entities.teams)` — after validating only the `schemaVersion` scalar. If `entities` is absent the dereference throws a `TypeError`, which the adjacent `.catch(() => setStatus("error"))` swallows into the **retryable** branch. But `loadTournamentIndex()` clears its module-scope `pending` promise **only on rejection** (`lib/tournament-index.ts:97-103`), and here the promise *fulfilled* — so pressing *Reintentar* re-awaits the same cached bad payload, throws again, and returns to `error` **with no fetch at all**, permanently. This is new to this change: the shipped `TournamentHubRegion.tsx:118` does `setTournament(payload)` with no dereference, so it cannot throw. It is also the "button that provably does nothing" failure the `invalid` state was designed to avoid, arriving through the door beside it. Fix: validate `entities.players` / `entities.teams` before `setStatus("loaded")` and route a shape failure to `invalid`, not `error`.

- [x] [Review][Patch] **The landing badge grid renders from `NAV_DESTINATIONS` without consulting `available`** [app/src/components/LandingContent.tsx:90, :129-131] — `medium`. `byKey.get(key)` ignores `destination.available`. The entire availability apparatus this story leans on — the bijection gate, `SiteNav.test.tsx`'s render-to-flag binding, the export-level "none that is not available" case — protects the **nav only**. `nav-destinations.ts:137-141`'s new docblock explicitly requires the flag to stay usable (*"a route DELETED in future must be able to turn one of these back to `false`"*); the moment a future story does that, `SiteNav` correctly drops the entry, direction 2 goes green, and the landing page keeps shipping a full-card link to the deleted route — a 404 on the site's entry page, with no gate red. The silent-fallback shape makes both directions quiet: a missing key yields `null` and the badge simply disappears. Fix: source the grid from `availableDestinations()`, or filter on `available` and assert the badge set against it.

- [x] [Review][Patch] **The non-vacuity assertion added to `SiteNav.test.tsx` is a tautology, and adding it exceeded Task 8.5's "retitle only"** [app/src/components/SiteNav.test.tsx:350-353] — `medium`. `AVAILABLE` and `UNAVAILABLE` are defined at `:300-301` as complementary filters of `NAV_DESTINATIONS`, so `UNAVAILABLE.length + AVAILABLE.length === NAV_DESTINATIONS.length` holds unconditionally and the assertion **cannot fail**. The comment above it claims it "asserts the REASON the loop is empty"; it does not. D6 required either a real non-vacuity assertion **or** a comment stating what direction 2 still guards — this satisfies neither, while Task 8.5 said *"retitle only, do not re-shape the assertions."* The other two vacuity sites were done properly (`nav-destinations.test.ts:249-256`, `static-output.test.ts:1065-1069` carry real assertions), so this is the odd one of the three. Fix: assert `UNAVAILABLE.length === 0` with the standing-guard rationale in the message, or drop the assertion and keep the prose D6 allows.

- [x] [Review][Patch] **A group filtered to zero still offers a live disclosure that opens onto an empty table** [app/src/components/PlayersIndexRegion.tsx:328-360] — `medium`. `filterPlayerGroups` deliberately keeps empty groups — the structural promise D4 rules, and the right call — but `TeamGroup` has no `group.players.length === 0` branch, so any query matching fewer than all 48 teams (e.g. `"delgado"`, matching one player) still renders 47 sections each with a heading, "0 jugadores", and an operable *"Ver los jugadores de Brazil"* control that expands to a `<table>` with a `<caption>`, a `<thead>` and an empty `<tbody>`. The screen-reader control list keeps 48 triggers, 47 of which reveal nothing. D4 requires the **headings and counts** stay rendered; it does not require the control stay operable. Fix: keep the heading and count, drop or disable the trigger when the group's filtered count is 0.

- [x] [Review][Patch] **The Landing surface has no component test, and `static-output.test.ts` promises a block that does not exist** [app/src/app/static-output.test.ts:249-250] — `medium`. The comment states *"`/` is asserted separately, as the **Landing**, below."* There is no such block: the `describe` list is 404, authorship caption, index canonical markup, `/tournament`, `/glossary`, `/about`, `/tops`, footer, per-route artifact fetches, header search, chrome fetches. The only `/`-as-Landing assertion added anywhere is the source-level "reaches no artifact" case at `:763`. The story's own A3 ownership list (`:126-127`) declares `LandingContent.test.tsx` and `PlayersIndexRegion.test.tsx` as files this story owns; **neither exists**, nor do `FeatureBadge.test.tsx` or `TeamsIndexRegion.test.tsx`. The `/players` interaction gap **is** disclosed in `deferred-work.md`; the Landing gap is disclosed nowhere — not in the Completion Notes, the ledger, or the File List. So D1's ruled content — the lede, the eight badges, their order and hrefs, the single emphasised badge, the absence of a table, `FeatureBadge`'s own trailing-slash rule — is held only by the Dev Agent Record's one-off browser run. Meanwhile `composeRouteTitle`, a one-line template concat, received a 55-line suite; the test investment is inverted relative to risk. Fix: assert D1's badge set against the export (cheap, build-free, no jsdom needed) and either write `LandingContent.test.tsx` or correct the comment and file the gap.

- [x] [Review][Patch] **The `/players` count line reports a search result before any search happens, and the correct key was minted then bypassed** [app/src/components/PlayersIndexRegion.tsx:256-259] — `low`. `countSentence` unconditionally composes `players.filterResults` / `filterResultsOne` — *"jugadores **encontrados**"* — so on first paint, with an empty query, the polite live region reads *"1.248 jugadores encontrados"* about a search the reader never ran. `es.ts:2563-2564` mints `players.count: "jugadores"` / `players.countOne: "jugador"` with a comment naming their call site ("The count line under the `<h1>`") and **nothing imports them** — verified: zero call sites outside `locales/`. The `/teams` twins are wired correctly (`TeamsIndexRegion.tsx:173` uses both). Fix: use `players.count*` when the query is empty and `players.filterResults*` when it is not — which also retires the dead keys.

- [x] [Review][Patch] **The zero-result copy tells a reader to delete letters they never typed** [app/src/components/PlayersIndexRegion.tsx:296-308] — `low`. The branch is `filtered.total === 0` alone, with no test on `query`. With `status === "loaded"` and an empty `players` array, the surface renders *"Ningún nombre coincide con el filtro. / Borra letras para ver más jugadores."* above zero group headings, with an empty filter box. `LeaderboardsRegion.tsx:645-656` already carries the ruled fix for this exact shape (*"THREE STATES, NOT TWO … told a reader with an empty filter box to 'borrar letras para ver más filas'"*), so this reproduces a two-state bug the 2.13 review rejected. Not reachable on the shipped corpus of 1,248 — a robustness gap, not a live defect. Fix: branch on `query === ""` for a genuine empty-corpus message.

- [x] [Review][Patch] **A `position` outside the union yields a `NaN` comparator and a raw dictionary key in the cell, and the docblock's type-safety claim is false** [app/src/lib/players-index.ts:55-72, render at PlayersIndexRegion.tsx:388-395] — `low`. The runtime payload is `as`-cast, so `entry.position` is only *typed* as `Position`; any other string reaches both call sites. `POSITION_RANK[unknown]` is `undefined`, so `undefined - n` is `NaN`, the comparator returns `NaN`, and V8 leaves the group in an arbitrary order **silently** — no throw, so `TacticalErrorBoundary` never engages. Simultaneously `t("players.position.short.<unknown>")` logs a `console.error` (against the zero-console requirement) and renders the literal key into the cell and the `<abbr title>`. Separately, the docblock claims `POSITION_ORDER` is *"Declared as a const tuple so a `Position` added to the contract without a line here is a TYPE error"* — it is `readonly Position[]`, **not** a tuple (no `as const`), so widening the union produces no error here at all. The real guard is `POSITION_RANK`'s `Record<Position, number>` on the next line, which is undocumented. And `positionOrder()` is exported with **zero importers**, including its own test file, despite a docblock claiming two call sites. Fix: a rank fallback for unknown positions, correct the two false docblock claims, and delete or use the dead export.

- [x] [Review][Patch] **`composeTeamRecord` silently emits a malformed record for absent fields, bypassing the one guard built to make that loud** [app/src/lib/teams-index.ts:48] — `low`. `[record.played, record.won, record.drawn, record.lost].join(separator)` over an unvalidated payload yields `"-1-1-2"` for a missing `played` and `"---"` for four nulls — no throw, no boundary, no empty state, a plausible-looking wrong record shipped to the reader. This is the only numeric rendering path on the new surfaces that bypasses `formatInteger`'s `assertFinite` (`format.ts:32-36`), which exists precisely to make this case loud. Fix: route the four values through `assertFinite`, or validate the record shape at the fetch boundary alongside the `entities` fix above.

- [x] [Review][Patch] **`/teams` has no empty state and its caption asserts 48 rows regardless of what it holds** [app/src/components/TeamsIndexRegion.tsx:170-235] — `low`. There is no `rows.length === 0` branch at all: with `status === "loaded"` and an empty `teams` array the surface renders a header-only `<table>` with zero rows, a count line reading "0 selecciones", and `<caption class="sr-only">` reading *"Las 48 selecciones del torneo, por grupo"* (`es.ts:2651`) — a screen-reader user is told the table holds 48 rows while it holds none. The hardcoded 48 also misstates any corpus of a different size. Fix: an empty-state branch, and a caption that does not pin a count the data must match.

- [x] [Review][Patch] **Unmemoized regroup-and-resort of 1,248 players on every render** [app/src/components/PlayersIndexRegion.tsx:245-246] — `low`. `groupPlayersByTeam(players)` runs on every render — 48 `Array.sort` calls with an `Intl.Collator` comparator over ~1,248 entries — followed by `filterPlayerGroups`, which folds every name through NFD normalization again. Neither is wrapped in `useMemo` and the input is not debounced, so both run per keystroke. `/players` records the lowest Lighthouse median on the site (70). **Note:** the region's docblock at `:275-286` gives a sound, documented reason for not debouncing the *announcement* (it has no `SortAnnouncerProvider`, so the polite region coalesces on its own) — that reasoning is correct and this finding does not contest it; `LeaderboardsRegion`'s `setTimeout` at `:396`/`:421` is the sort announcer, not a filter debounce. The gap is compute, not announcements. Fix: `useMemo` on the grouping, keyed on `players`.

- [x] [Review][Patch] **`matches` can never receive `aria-current`, because `tournament` shadows its route** [app/src/lib/nav-destinations.ts:241-246] — `low`. `currentDestinationKey` exact-matches with `.find()`, and after the flips both `tournament` (`:165`) and `matches` (`:172`) declare `route: "/tournament/"`. `tournament` precedes `matches` in `RULED_ORDER`, so *Torneo* always wins and *Partidos* never receives `aria-current="page"` on any pathname on the site. This is arguably correct — D1 rules the containment deliberately (*"Badge 3 is contained by badge 2, and that is ruled, not accidental"*) and only one entry can be current for one URL — but the duplicate-route case is **new to this change** (both flags were `false` before), nothing in the nav tests covers it, and D6 warned about exactly this class of silently-changed behaviour. Fix: no behaviour change needed; document the shadowing at the `find()` and add a case pinning it, so a future reader does not "fix" it into a double-marked nav.

- [x] [Review][Patch] **A docblock describes a `prefetch={false}` prop that is not in the code** [app/src/components/PlayersIndexRegion.tsx:403-404] — `low`. The comment reads *"`prefetch={false}`: ~26 rows per group and 48 groups. The default would fire a prefetch for every link the reader scrolls past"* — but the element is a plain `<a href>`, there is no `Link` import and no `prefetch` attribute. The **choice** is deliberate and correctly documented in the sibling file (`TeamsIndexRegion.tsx:214-216`: *"`prefetch` is left off by using a plain anchor"*); only this comment misdescribes its own code. Fix: restate it the way `TeamsIndexRegion` does.

### Dismissed (2, not carried forward)

- **"The new route-shape guard re-arms the `_private` trap that code review disarmed"** (`nav-destinations.test.ts:347`). The scopes differ materially: `sitemap.ts`'s `assertPlainSegment` threw on any private **directory** during the build walk, which is what broke `next build` for `src/app/_components/`; the new test flags only a **`page.*` file** inside such a folder (`pageFiles` matches `/^page\.[a-z]+$/`). The realistic colocation case — `_components/Button.tsx` — does not fire. A residual nit stands (a private folder is unroutable, so it can never defeat `pageFor()`, making `_` in this filter protection-free), but the finding as framed is a false positive.
- **"Both index regions use bare `<a>` where the site uses `next/link`, giving two navigation behaviours to one destination."** The plain anchor is a deliberate, documented decision (`TeamsIndexRegion.tsx:214-216`) taken to avoid 48 simultaneous prefetches. The only real defect is the inaccurate comment in the sibling file, carried above as its own patch.

### Patches applied — 2026-08-27

All 20 applied and verified. Full chain green afterwards: `lint` → `typecheck` →
`assert:schema-version` → `next build` → `copy-data.mjs` → `assert:no-external-origins`, then
`npm test` at **63 files / 1,555 tests / 0 skipped** (up from 1,546 — nine new assertions, none
newly skipped). Export unchanged at **1,410 routes / 1,411 documents / 1,408 sitemap `<loc>`**, so
**AC 5 still holds**. `id="leaders"` still occurs exactly once site-wide, so **AC 1 still holds**.
Every one of the eight static routes now carries exactly one `<h1>`, which is the fix below.

**One finding was missed in the first triage and is recorded here rather than quietly folded in.**
Both no-spec layers reported that `/players` and `/teams` passed `headlineKey="hub.region.crashed"`
to their error boundaries — *"No pudimos mostrar los resultados y las posiciones."* on two pages
that show neither. `es.ts:812-815` records that key being minted precisely because the boundary's
default was "a false statement on a tournament-wide route (ruled D8)"; the same objection applies
one level out. It was dropped when the 24 raw findings were deduped to 20 and was applied during
the patch pass as a 20th item: `players.crashed*` and `teams.crashed*` are now route-scoped.

**A1 — the two new structural gates were driven RED before being trusted:**

```
# 1. the build-time AD-11 gate, with a real call added to app/src/app/page.tsx
$ npx vitest run src/app/static-output.test.ts -t "reads NOTHING at build time"
AssertionError: expected [ 'readTournament' ] to deeply equal []
+ Received
+   "readTournament",

# 2. the named-route membership check, with the index document moved aside
$ mv out/players/index.html out/players/index.html.bak
$ npx vitest run src/app/static-output.test.ts -t "found every exported route"
AssertionError: the static-route sweep did not find /players. Six cases titled "on EVERY
exported route" silently stop covering it — either the export did not run, the route was
deleted, or the discovery loop skips the directory it lives in.
```

Both were reverted and re-verified green. The Landing badge assertions were not driven red by a
rebuild; they are exact-equality checks against an eight-element list, so a walk that matched
nothing yields `[]` and fails — **the shape cannot pass vacuously**, which is the property the red
drive would have established.

**Worth recording, because it is the same class of defect this review found.** The first version of
the build-time detector went red on its own documentation: it matched the string
`readTournament()` inside `app/page.tsx`'s docblock — a sentence promising the route makes no such
call — and inside an `es.ts` comment. It then matched the function *declarations* in
`build-data.ts`, reporting `/tops` as reading the tournament index it never touches. Both are fixed
(comments stripped before matching, definition site reached but not counted) and both are recorded
in the code. A gate that reads prose as code is exactly the failure mode D11 warns about, and it
took two rounds to stop it here.

| # | finding | fix |
|---|---|---|
| 1 | `/tops` had no `<h1>` | `LeaderboardsSection`'s `<h2 id="leaders-title">` promoted to `<h1>`; visual weight deliberately unchanged (`type-headline`, not `type-display`) because D2 rules the move an address change, not a redesign. The id and `aria-labelledby` pairing untouched, so the three uniqueness assertions bind as before. Two stale docblocks corrected. |
| 2 | the every-route sweep skipped `/players` and `/teams`; the `>= 9` floor passed by double-counting `/404` and including `/_not-found` | family roots no longer skipped; `404/index.html` and `_not-found` excluded; the floor is now a **named** `STATIC_ROUTES` membership check, so a missing route fails by name instead of being masked by a duplicate |
| 3 | `/players` and `/teams` `<h1>` frozen in Spanish (server `t()`) | new client `PlayersIndexHeading` / `TeamsIndexHeading`, on `TournamentHubHeading`'s precedent; the heading now follows the language toggle like every other `<h1>` on the site |
| 4 | the `/` → `[]` artifact gate was blind to build-time reads that three docblocks credited it with catching | new `buildDataReadsReachableFrom` walk with its own non-vacuity probe; the two overclaiming docblocks narrowed to the scope they actually have |
| 5 | 48 triggers announced "Ver los datos: Ver los jugadores de Argentina" | `panelTitle` is the team name alone → "Ver los datos: Argentina"; the now-dead `players.teamTrigger` retired from both dictionaries |
| 6 | a shape-malformed payload landed in the retryable `error` state whose retry could never fetch | both regions shape-check `entities.*` before dereferencing and route a failure to `invalid` (no retry) rather than `error` |
| 7 | the landing badge grid ignored `available` | keyed off `availableDestinations()`; a retired route now drops from the grid as it already did from the nav |
| 8 | `SiteNav.test.tsx`'s new non-vacuity assertion was a tautology | replaced with `UNAVAILABLE.map(key) === []`, which actually goes red the day a destination is retired |
| 9 | filtered-to-zero groups still offered a control opening an empty table | the heading and count stay (D4's structural promise); the trigger is dropped when the group is empty |
| 10 | no test covered the Landing surface; the file promised a block that did not exist | new `exported / — the Landing surface` describe: badge set and order, trailing slashes, the single emphasised badge, no `aria-label`, no table, one `<h1>` |
| 11 | the count line said "encontrados" before any search | three states: `players.count*` unfiltered, `players.filterResults*` when filtering — wiring up the keys that were minted for this and never used |
| 12 | zero-result copy told readers to delete letters they never typed | new `players.empty*` pair for the empty-corpus case, on `LeaderboardsRegion`'s ruled three-state precedent |
| 13 | an out-of-union `position` produced a `NaN` comparator and rendered a raw key | `positionRank()` with a defined fallback (unknown sorts to the end, in name order); the false "const tuple" docblock corrected and the dead `positionOrder()` export removed |
| 14 | `composeTeamRecord` emitted `"-1-1-2"` / `"---"` silently | throws on a non-finite value, on `format.ts`'s `assertFinite` reasoning, so `/teams`'s boundary shows a crash panel instead of a plausible lie |
| 15 | `/teams` had no empty state and a caption hardcoding "48" | empty state added; caption no longer pins a count the markup cannot keep honest |
| 16 | 1,248 players regrouped and resorted on every keystroke | `useMemo` on both the grouping and the filter. The region's reasoning for *not* debouncing announcements was sound and is left intact — the gap was compute, not announcements |
| 17 | `matches` can never be `aria-current` because `tournament` shadows its route | ruled correct and now documented at the `find()`, plus a case pinning it so the tempting "fix" (two entries current for one URL) goes red |
| 18 | a docblock described a `prefetch={false}` prop that does not exist | restated to match `TeamsIndexRegion`'s accurate account of the same deliberate choice |
| 19 | **decision:** `/players` and `/teams` shells vs D5b's headings clause | **re-ruled by Juan.** `EXPERIENCE.md:417` amended to scope the clause to headings that are not artifact-derived, naming the two routes and stating why; D5b in this story and the code comment updated to cite it |
| 20 | **missed in triage:** both index routes shipped a crash headline naming results and standings | `players.crashed*` / `teams.crashed*` minted route-scoped, on the precedent `es.ts:812-815` already records |
