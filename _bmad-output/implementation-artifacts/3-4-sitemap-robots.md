...
- [ ] **1.1** Record `git rev-parse HEAD` as this story's baseline commit in the Dev Agent Record.
- [ ] **1.2** **A3 file-ownership probe.** Run `git status --porcelain` and confirm each path this
      story OWNS is clean:
...
- [ ] **1.3** Confirm the concurrent-session file list has not grown into this story's three paths.
...
- [ ] **1.4** **Re-measure the test baseline yourself.** It has moved repeatedly this epic
...
- [ ] **1.5** **Re-measure the route baseline.**
...
- [ ] **1.6** Confirm the manifest counts by reading `data/index/tournament.json` — expected
...
- [ ] **2.1** Write a **minimal** `app/src/app/sitemap.ts` returning two hard-coded entries and a
      minimal `app/src/app/robots.ts`. Run `npm run build`.
- [ ] **2.2** **Verify the flat-file claim empirically.** Assert `app/out/sitemap.xml` and
...
- [ ] **2.3** Record the route count Next prints (expect the previous count plus `/sitemap.xml` and
      `/robots.txt`). Record the number rather than predicting it.
- [ ] **2.4** Confirm no `export const dynamic` directive was needed. If the build bails to dynamic,
      add `export const dynamic = "force-static"` and record why — §D5.
- [ ] Throw the two-entry bodies away once 2.2 passes. They exist to de-risk, not to ship.

### Task 3 — `sitemap.ts` (AC: 2, 3, 4)

- [ ] **3.1** Import `SITE_ORIGIN` from `@/lib/site-origin` and `readTournament` from
      `@/lib/build-data`. **No origin literal, no `data/index/tournament.json` path literal.**
- [ ] **3.2** Build the entity URLs by mapping the manifest 1:1, exactly as the three
...
- [ ] **3.3** Discover the non-entity routes from the **app route tree**, per §D4: walk
      ...
      entity-only sitemap.
- [ ] **3.4** Compose each entry as `{ url }` **only**. **Omit `lastModified`, `changeFrequency` and
      `priority`** — §D3.
- [ ] **3.5** Type the default export `MetadataRoute.Sitemap`
      (`import type { MetadataRoute } from "next"`).
- [ ] **3.6** Docblock it in this repo's register: state that the origin comes from the one
...
- [ ] **4.1** Return `{ rules: { userAgent: "*", allow: "/" }, sitemap: <SITE_ORIGIN>/sitemap.xml }`,
      typed `MetadataRoute.Robots`. No origin literal.
- [ ] **4.2** Docblock the **absence** of `Disallow: /data/` with §D6's reason. The next reader will
      otherwise "fix" it.
...
- [ ] **5.1 — Layer 1, manifest bijection (runs with no build; THIS is the A1 gate).**
...
- [ ] **5.2 — Layer 2, shape (runs with no build).** Every URL: starts with `SITE_ORIGIN`, ends with
...
- [ ] **5.3 — Layer 3, export bijection (`describe.skipIf(!anyBuilt)`).**
...
- [ ] **5.4 — robots (`skipIf`).** `out/robots.txt` exists as a file; contains exactly one `Sitemap:`
      line and it equals `${SITE_ORIGIN}/sitemap.xml`; contains `Disallow:` zero times (AC7).
- [ ] **5.5** A2: pin every path relatively (`fileURLToPath(new URL("../../out/", import.meta.url))`,
...
- [ ] **6.1 — Direction 1, a manifest entry omitted from the sitemap.** In `sitemap.ts`, `.slice(1)`
      the players map. Run `npx vitest run src/app/sitemap.test.ts`. Layer 1 must fail and **name the
      dropped playerId**. Revert.
- [ ] **6.2 — Direction 2, a sitemap entry absent from the manifest.** In `sitemap.ts`, append a
      phantom `${SITE_ORIGIN}/players/phantom-player/`. Same command. Layer 1 must fail and name
      `phantom-player`. Revert.
- [ ] **6.3 — Layer 2 red.** Drop the trailing slash from the team URLs; assert the shape layer
      fails. Revert.
- [ ] **6.4 — Layer 3 red.** With a build present, delete one emitted route directory from `out/`
      (e.g. `out/teams/algeria/`) and re-run; the export bijection must fail naming it. Restore by
      rebuilding, not by hand.
- [ ] **6.5 — AC7 red.** Add `disallow: "/data/"` to `robots.ts`, rebuild, assert 5.4 fails. Revert.
- [ ] **6.6** Confirm the whole file is green again and that the pre-existing suite is untouched.

### Task 7 — AC6: the origin gate over the real export (AC: 6)

- [ ] **7.1** Full `npm run build` (it chains lint → typecheck → assert:schema-version → next build →
      copy-data → assert:no-external-origins). Record `BUILD_EXIT`.
- [ ] **7.2** Record the gate's stdout verbatim: the `scanned` count (must be non-zero; 12,683 was
      3.1's figure and this story adds two assets), the `MENTIONED` line, and the absence of any
      `EXTERNAL SUBRESOURCE(S)` block.
- [ ] **7.3** Confirm `https://mundial-stats.juancr.dev` appears **nowhere** in the gate output, and
      that `www.sitemaps.org` **does** appear on the `MENTIONED` line. §D2 predicts both; verify.
- [ ] **7.4** If the 3.1 review lands a further gate patch mid-story, re-run 7.1–7.3 against it
      before committing. Do not edit the gate.

### Task 8 — Verify, commit, hand off (AC: all, A4)

- [ ] **8.1** Full chain green: `npm run lint` (0 under `--max-warnings 0`), `npm run typecheck` (0),
...
- [ ] **8.2** If the shared tree is dirty enough to contaminate the figures, verify in an isolated
      worktree per §D9 and say so — 3.1 and 3.5 both did, and both were right to.
- [ ] **8.3** **A4 commit.** Stage **only**:
...
- [ ] **8.4** Append to `sprint-status.yaml` — **append only, never regenerate.** Flip
      `3-4-sitemap-robots` to `review` and add a journal entry with the measured figures.
- [ ] **8.5** Update the ledger entry at `deferred-work.md:4663-4677`, which names **this story** as
      ...
      gate", and say why.
- [ ] **8.6** **Surface AC8 to Juan as a manual task and stop.** Do not attempt a Search Console
