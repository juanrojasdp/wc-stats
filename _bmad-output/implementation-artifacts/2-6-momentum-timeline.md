...
| 2026-08-03 | Adversarial code review (Blind Hunter / Edge Case Hunter / Acceptance Auditor over the 2.6-scoped diff `c645cfe..HEAD`). 2 decisions ruled, **24 patches applied**, 7 items deferred, 4 dismissed. The Acceptance Auditor verified 24 of 27 ruled decisions as correctly implemented and both declared departures as truthfully described. **Headline finding, found independently by two layers: tap-to-position mapped `clientX` through the wrong plot origin.** recharts computes `offset.left = margin.left + leftAxesOffset`, and `selectLeftAxesOffset` adds the y-axis's own `width` — pinned here at 34, same as the margin — so the true plot origin is 68, not 34. A left-edge tap landed ~17 samples late on m074 at mobile width, decaying to exactly zero at the right edge, which is why Task 11.4's "plot-left -> 0, mid -> 50, plot-right -> 100" spot-check passed it; `indexAtOffset` was correct in isolation and the unit test handed it the caller's own wrong numbers. Fixed by moving the box geometry into a tested pure `momentumPlotBox`. **Second headline: the `next/dynamic` code-split was defeated by a value import** — `CHART_HEIGHT_CLASS` was imported from `MomentumChart` by `MomentumSection`, a static module-graph edge into the deferred module; the Completion Notes had attributed the resulting eager `<script async>` to "Next's static export". The constant moved to the pure model module, and the 316 kB recharts chunk is now referenced by **no route's HTML at all** (verified in `out/`), including m002, which has no chart. Also corrected: recharts 3.4+ z-index portals mean declaration order is NOT the paint-order lever decisions 24/25 reasoned from (outcome was right by library default; the comments now say so); axis titles reached no surface because `name` is tooltip-only and the `<Tooltip>` is banned, now `<Label>`; the data table rendered the missing-data em dash for a known "no goal", now `viz.table.no`; `marker.exact` was computed and consumed by nobody, now speaks an "approximate position" qualifier; the read guards accepted NaN/Infinity stoppage minutes and negative/fractional counts; the cursor handle's 44x44 hit rect made an activated goal marker permanently un-tappable; overlapping markers hit-tested in reverse of decision 26, now resolved by capping hit widths so they cannot overlap; and the whole model pipeline re-ran on every arrow keypress, now memoised. Ruled: markers keep `role="button"` and gain a real `onClick` (AT activation was a no-op); `momentumYTicks` now always labels the peak (m074 read `[-10, 0, 10]` against a peak of 17). Suite 358/16 -> **364/16**; lint, typecheck, `assert:schema-version` and `next build` all green. |

## Review Findings

Adversarial code review, 2026-08-03 — three parallel layers (Blind Hunter, Edge Case Hunter,
Acceptance Auditor) over the 2.6-scoped diff `c645cfe..HEAD` (10 files, +2182/-4). Every finding
below was re-verified against the shipped tree and, where it concerns library behaviour, against
`node_modules/recharts@3.10.1` source before triage. Build chain re-run independently and green:
lint 0, `tsc --noEmit` 0, vitest **358 passed / 16 files**.

Two layers converged independently on the tap-to-position defect.

### Decisions ruled

Both raised decisions were ruled by Juan on 2026-08-03 and converted to patches; the rulings are
recorded inline below so they are not re-litigated.

- [x] [Review][Decision] **RULED — keep the buttons, fix all three consequences.** Departure 1 stands: markers remain `role="button"` and continue to move the cursor. Three patches follow from this ruling: (a) add `onClick` to `GoalMarkerShape` so assistive-technology activation works; (b) **drop the 44x44 transparent hit rect from `CursorHandleShape`** — the slider is keyboard-driven and the plot area owns the tap target, so the handle does not need one, and removing it dissolves the occlusion band entirely; (c) paint the goal markers in reverse while leaving DOM/tab order chronological, so the earlier goal wins the pointer per decision 26. *Original finding:* **The goal-marker interaction model needs one ruling, and it governs three findings** — Departure 1 shipped `onActivate={() => onIndexChange(marker.index)}` (`MomentumChart.tsx:516`). Three consequences were not anticipated. (a) **Occlusion:** `GoalMarkerShape` and `CursorHandleShape` both draw a transparent `MIN_HIT_PX` 44x44 hit rect centred on `cx`/`cy`, both are `ReferenceDot`s at `y={0}` (recharts z-layer `scatter: 600`, so within-layer declaration order decides), and the handle is declared last (`:508-549`). Activating a marker parks the handle exactly on top of it, so **that marker can never be tapped again**; more generally every marker within +/-22px of the cursor (+/-9 samples at `<md`) is pointer-dead. (b) **AT activation is a no-op:** the `<g role="button">` wires `onKeyDown` and `onPointerDown` but **no `onClick`** (`:141-152`), and screen readers activate a button by synthesizing `click` — so the very user departure 1 was written to protect still gets nothing from NVDA/JAWS/VoiceOver. (c) **Marker-vs-marker order is inverted:** decision 26 ruled "first-in-DOM-order wins" and `momentum-model.test.ts` pins first-in-array, but SVG paints the later-declared marker on top, so the *later* goal takes the pointer. Reachable on real corpus data — any two goals within ~9 minutes at mobile width. The story itself offers the alternative: "the fix is one line (drop `onActivate`) plus a role change from `button` to a non-interactive focusable."
- [x] [Review][Decision] **RULED — include the peak in the tick set; no `CartesianGrid`.** `momentumYTicks` extends so the peak is always labelled alongside the round steps (`momentumYTicks(17)` → `[-17, -10, 0, 10, 17]`), making the curve's maximum readable. The property test over peaks 1-40 and both fixture literals must be updated with it. No gridline is introduced — Task 8.2's undecided gridline token stays undecided, and that call moves to the later chart stories. *Original finding:* **The y-axis never labels the peak, so values cannot be read off the chart** — `momentumYTicks(17)` returns `[-10, 0, 10]` (pinned at `momentum-model.test.ts:1864`). On m074 the domain is `[-17, 17]`, so the tallest labelled value covers 59% of the domain and the curve's actual peak sits above any label. With `axisLine={false}`, `tickLine={false}` and **no `<CartesianGrid>`**, the axis is three floating numbers with nothing connecting them to the curve — and decision 6's sign-stripping makes the top and bottom both read "10". The entire value-reading burden falls on the cursor chip. Options: extend the tick set to include +/-peak, add a grid, or accept and file.

### Patches

- [x] [Review][Patch] Tap-to-position maps `clientX` through the wrong plot origin AND the wrong width — recharts computes `offset.left = margin.left + leftAxesOffset` (`selectChartOffsetInternal.js:62`), and `selectLeftAxesOffset` adds the `YAxis`'s own `width`, pinned here at `width={MARGIN.left}` (34). True plot area is `[68, rect.width-10]`, not `[34, rect.width-10]`. Error is maximal at the left edge (~17 samples at `<md` on m074) and decays to zero at the right edge, which is why a right-edge spot-check passed. Falsifies the Completion Note "Tap-to-position itself is exact: plot-left -> 0, mid -> 50, plot-right -> 100" [app/src/components/MomentumChart.tsx:366]
- [x] [Review][Patch] The unit test bakes in the caller's wrong assumption, so it can never catch the tap-to-position defect — `indexAtOffset` is correct in isolation; the test hands it invented `plotLeft`/`plotWidth` values matching the caller's error, and no test exercises the caller [app/src/viz/momentum-model.test.ts:510]
- [x] [Review][Patch] `CHART_HEIGHT_CLASS` is a **value** import from the dynamically-imported module, creating a static module-graph edge that defeats decision 21's code split — the Completion Notes attribute the eager `<script async>` load to "Next's static export"; this line is the more likely cause. Move the const to `@/viz/momentum-model` so only the type crosses the boundary, then re-measure [app/src/components/MomentumSection.tsx:6]
- [x] [Review][Patch] The x- and y-axis titles never render — recharts' `name` prop feeds tooltip/legend payloads only, and decision 23 bans `<Tooltip>`, so `viz.momentum.axisMinute` / `axisEntries` reach no surface at all and the y-axis ships as bare numbers with no unit. Violates decision 13's enumerated "the y-axis label". The mechanism is `<Label>` as an axis child [app/src/components/MomentumChart.tsx:418]
- [x] [Review][Patch] The data table renders an em dash (`viz.table.unknown`, the missing-data glyph) for rows where it is definitively known that no goal fell — `viz.table.no` ("No") already exists at `es.ts:257` and is unused. 136 of m074's 138 rows assert absence of information where the information exists [app/src/components/MomentumSection.tsx:290]
- [x] [Review][Patch] `marker.exact` is computed, documented, tested — and consumed by nothing. On the fallback path the dot is drawn at one row while `markerNames` announces the goal's true stamp, with no "approximate" qualifier: the chart, the table and the spoken name assert three different things. This is precisely the data lie the field's own docblock says it exists to prevent [app/src/viz/momentum-model.ts:260]
- [x] [Review][Patch] The read guards are half a validator, on a module whose header explicitly refuses to trust the `as`-cast: `readStamp` finite-checks `minute` but **not** `stoppageMinute` (NaN/Infinity pass, collapsing every off-grid goal onto row 0 and rendering "45+NaN"); `readCount` accepts negative and non-integer counts (a negative `away` is flipped **positive** by `away === 0 ? 0 : -away` and prints "-5" in the table); `stampRank`'s x100 packing collides once `stoppageMinute >= 100`, silently reordering markers and therefore tab order [app/src/viz/momentum-model.ts:119]
- [x] [Review][Patch] `momentum: undefined` bypasses both absence branches and crashes — `MomentumSection.tsx:172` tests `=== null` and `tactical-sections.ts:120` tests `!== null`, so an absent key classifies "ready", reaches `momentumRows`, and throws into `TacticalErrorBoundary`. That is the "crash instead of an absence is strictly worse" outcome the guard's own comment names, and the dedicated empty state (the whole point of decision 12) never renders. **Only the `MomentumSection` line is in scope** — `tactical-sections.ts` is on the do-not-touch list, so its half is filed below [app/src/components/MomentumSection.tsx:172]
- [x] [Review][Patch] Nothing is memoized: one arrow keypress re-runs `momentumRows` (re-validating and re-allocating 138 sample objects through the read guards), `goalMarkers`, `momentumTableRows`, `momentumTickIndices`, `markerNames` and `tickLabels`, **and** rebuilds the full 138-row table element tree even while the disclosure is closed. The new `rows` identity also forces recharts to recompute both area paths [app/src/components/MomentumSection.tsx:181]
- [x] [Review][Patch] `onPointerDown` filters neither `button` nor `isPrimary`, so a right-click, a second touch point, or the `pointerdown` that begins a vertical page scroll over a full-width 122-170px target all move the reading cursor with no undo [app/src/components/MomentumChart.tsx:366]
- [x] [Review][Patch] The slider key handler matches on `event.key` alone and calls `preventDefault()` unconditionally, swallowing Cmd/Ctrl/Alt/Shift chords — Alt+Left (browser Back), macOS Cmd+Left/Right and screen-reader pass-through are reinterpreted as +/-1 sample [app/src/components/MomentumChart.tsx:329]
- [x] [Review][Patch] The cursor chip clips and mis-anchors — `nearRight` flips at `cx > chartWidth * 0.65`, a magic fraction unrelated to the chip's ~140px rendered width, leaving a band (~cx 176-212 on a 326px chart) where a start-anchored chip is truncated by the viewport; and because `useElementWidth` measures after paint, the first desktop frame uses the 326px fallback and anchors `end` at `x ~ 26`, running text off the **left** edge, for which there is no counterpart clamp [app/src/components/MomentumChart.tsx:273]
- [x] [Review][Patch] *(from decision 1a)* The goal marker's `<g role="button">` has no `onClick`, so assistive-technology activation is a no-op — screen readers activate a button by synthesizing `click`, not `keydown` or `pointerdown` [app/src/components/MomentumChart.tsx:141]
- [x] [Review][Patch] *(from decision 1b)* Remove the 44x44 transparent hit rect from `CursorHandleShape` — the slider is keyboard-driven and the plot area already owns the tap target, so the handle needs no pointer surface of its own. This is what dissolves the occlusion band that makes an activated marker permanently un-tappable [app/src/components/MomentumChart.tsx:242]
- [x] [Review][Patch] *(from decision 1c)* Paint the goal markers in reverse while leaving DOM/tab order chronological, so that where two markers' hit boxes overlap the **earlier** goal takes the pointer, as ruled decision 26 requires and `momentum-model.test.ts` already pins [app/src/components/MomentumChart.tsx:508]
- [x] [Review][Patch] *(from decision 2)* Extend `momentumYTicks` so the tick set always includes +/-peak alongside the round steps (`momentumYTicks(17)` -> `[-17, -10, 0, 10, 17]`), and update the property test over peaks 1-40 plus both fixture literals to match [app/src/viz/momentum-model.ts:349]
- [x] [Review][Patch] The data-table column heads never name the metric — numeric columns are headed `MEX`/`RSA` under a "Linea de momentum" caption, so "MEX 4" reads as a momentum index. Decision 13's final enumerated site; `metricNote` is available and unused here [app/src/components/MomentumSection.tsx:270]
- [x] [Review][Patch] `recharts` is the one loosely-pinned dependency (`^3.10.1`) while `react`/`react-dom` are pinned exactly — and this module's correctness rests on undocumented internals (the z-index portal system introduced in 3.4, `YAxis.width` feeding `selectLeftAxesOffset`, `ReferenceDot` `shape` receiving resolved `cx`/`cy`). With no jsdom and no rendering tests, a caret minor bump has no failure detector. Pin exact, per AD-13 [app/package.json:27]
- [x] [Review][Patch] A single-sample series — contract-legal at `@minItems 1`, and accepted by `momentumRows` — yields `lastIndex = 0` and a degenerate `domain={[0, 0]}` that recharts cannot scale. `momentumPeak` has an explicit `Math.max(1, peak)` floor for exactly this degeneracy on the y axis; the x axis has no equivalent [app/src/components/MomentumChart.tsx:310]
- [x] [Review][Patch] A container narrower than the fixed margins (`rect.width <= 44` — a `display:none` ancestor, a collapsed flex parent, a print stylesheet) makes `indexAtOffset` return a *value* where a no-op is correct, so any tap silently resets the cursor to minute 1 [app/src/viz/momentum-model.ts:389]
- [x] [Review][Patch] An absent or non-string `scorerName` is coerced to an empty string rather than falling back to `viz.marker.unknownPlayer` (which exists and is used by the other viz panels), producing the marker name "Gol de , minuto 8" [app/src/viz/momentum-model.ts:276]
- [x] [Review][Patch] `markerNames[position]` can be `undefined` — the two props carry no length invariant — rendering a `role="button" tabIndex={0}` node with `aria-label={undefined}` and no text content: a focusable element with no accessible name. The sibling `tickLabels[position] ?? ""` at `:375` has the fallback this one lacks [app/src/components/MomentumChart.tsx:512]
- [x] [Review][Patch] `momentumTickIndices` seeds `ticks` with index 0 unconditionally, skipping the stoppage and `% stepMinutes` tests every other tick must pass, so the axis can carry a stoppage-slot label at the origin [app/src/viz/momentum-model.ts:339]
- [x] [Review][Patch] The paint-order comments are false for the installed recharts and tell the next maintainer the opposite of how it works — `DefaultZIndexes` (`area: 100`, `line: 400`, `axis: 500`, `scatter: 600`) portals each element type into its own group, so the midline paints over the fills because 400 > 100, **not** because it is declared last, and the tab-order guarantee holds only because every focusable node here happens to be a `ReferenceDot`. Correct the comments and note the `zIndex` prop [app/src/components/MomentumChart.tsx:53]

### Deferred

- [x] [Review][Defer] A malformed momentum series takes down all eleven Tactical sections — `TacticalErrorBoundary` wraps the whole layer, and decision 8 deliberately moved the throw earlier to guarantee it fires on load. Boundary granularity is pre-existing architecture, not this story's [app/src/components/TacticalErrorBoundary.tsx:42] — deferred, pre-existing
- [x] [Review][Defer] `tactical-sections.ts:120`'s `!== null` half of the `undefined` guard — on this story's explicit do-not-touch list; needs a waiver or an owning story [app/src/lib/tactical-sections.ts:120] — deferred, out of scope by ruling
- [x] [Review][Defer] An all-zero series renders as a visually empty card with no zero state — `viz.momentum` has no `zero` key while `viz.passNetwork`/`shotMap`/`crossMap` all do. Needs UX copy [app/src/components/MomentumChart.tsx:392] — deferred, needs UX
- [x] [Review][Defer] **Undeclared departure:** series labels ship as `ReferenceDot` + custom text, not decision 24's `<Label position="right">` on each `<Area>`, and land at pixel ~76 and 60% of the domain rather than the mockup's `x=8` / `y=20`. The spec contradicts itself here (decision 24 says `position="right"`, decision 9a says flush left), so the swap is defensible — but the Dev Agent Record asserts "All 27 ruled decisions implemented as written except the two departures declared below" [app/src/components/MomentumChart.tsx:492] — deferred, declare it
- [x] [Review][Defer] `cursorLabel` is an operating instruction ("mueve con las flechas...") used as an `aria-label`, so a command is re-read on every value change; instructions belong in `aria-describedby` [app/src/locales/es.ts] — deferred, UX copy call
- [x] [Review][Defer] Minute digits bypass locale formatting (`formatGoalMinute`'s raw template) while the counts in adjacent cells use `formatInteger(value, locale)` — two number-rendering policies in one table row. Pre-existing helper the story required importing [app/src/lib/match-hero.ts:72] — deferred, pre-existing
- [x] [Review][Defer] **Process:** 2.6's `deferred-work.md` and `sprint-status.yaml` edits landed in commit `5344fac` (Story 1.14, a pipeline-only story) — verified, that commit already contains "RESOLVED by Story 2.6". The story's File List claims both as modified by 2.6, and Task 10.4 required them staged with this story with any co-commit disclosed. The content is present and correct; the co-commit is undisclosed, in the reverse direction from the one that was [_bmad-output/implementation-artifacts/deferred-work.md] — deferred, record it

### Dismissed (4)

The `rows.length === 0` empty-figure branch (unreachable — `momentumRows` throws first); the "dead" `--color-shot-goal-canvas` Tailwind token (mandated by Task 8.1 / decision 10); the no-op `cn()` at `MomentumChart.tsx:574` (trivial); Story 2.8's locale and dispatch lines appearing in the diff (known, disclosed co-commit, scoped out at review time).
