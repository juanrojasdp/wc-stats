...
- [x] **Task 3: `TacticalSection` shell — disclosure semantics (AC: 1)**
  - [x] 3.1 Create `app/src/components/TacticalSection.tsx` (`"use client"`). Markup contract (ruled decision 6):
    ...
    ```
    `CHEVRON` is a module const, not a JSX literal (the gate flags bare glyph text). The summary `<p>` renders **only** in the collapsible (`<lg`) presentation — the desktop mockup shows no summary lines. The content region wrapper is rendered whenever the section is open, **including** for always-expanded sections, so anchor navigation always has a focus target. No `scroll-mt-*` on the section: Task 8's `scroll-padding-top` on the scrollport covers both fragment navigation and `scrollIntoView()`.
  ...
  - [x] 3.3 On open, move focus to the revealed content region (`tabIndex={-1}` + `role="region"` + `aria-labelledby` the heading). Ruled decision 7 records why this, and not the heading itself: the heading sits inside the always-visible trigger, so focusing it would not move focus into what was revealed. Do this only on a *user* toggle and on anchor auto-expand — never on the ≥lg initial render.
...
- [x] **Task 4: `TacticalLayer` — order, rhythm, breakpoints, anchors (AC: 1)**
  ...
  - [x] 4.3 Disclosure policy, in precedence order:
    ...
    3. everything else → collapsible: expanded at `≥lg`, collapsed at `<lg`.
...
7. **Focus on expand goes to the revealed content region**, not the heading: the heading lives inside the always-visible trigger, so focusing it would not move focus into what was revealed. The region carries `role="region"` + `aria-labelledby` the heading, so the announcement still names the section (EXPERIENCE's intent).
...
- **10.3 — ≥1024 px (1920 px actual).** 11 sections, **0** disclosure triggers, **0** summary lines (matches the desktop mockup, which draws plain `<h2>`s), 11 content regions; key stats in a two-column grid (`546px 546px`) of **19** rows; `margin-top` between every section pair **48 px** (`section-gap`); `border-top: 1px rgb(42,49,56)` (`border-hairline`); 10 pending panels; `scroll-padding-top: 72px`; no horizontal scroll.
- **10.4 — Keyboard at `<lg`.** All **9** triggers are 44 px tall and tabbable. Enter on `#shot-maps` → `aria-expanded` `false→true`, content region mounts, focus lands on `#shot-maps-content` (`role="region"`), computed `outline-width` **2px** (global `:focus-visible --ring`). Space → `true→false`, content **unmounts** (lazy mount confirmed), focus stays on the trigger. `aria-controls="shot-maps-content"`, `aria-describedby="shot-maps-summary"`.
- **10.5 — Deep links.** `/matches/m001-mexico-south-africa/#defensive-actions` at `<lg`: after the bundle resolves the section auto-expands, section top **72 px** vs sticky-header bottom **57 px** (clears it), focus on `#defensive-actions-content`. Setting `#pressing` (hashchange): expands, top 134 px, focus on `#pressing-content`. Same behaviour re-verified at ≥lg (72 px / 105 px).
...
- **Deviation from the story's literal Task 4.3 wording, resolved toward AC 1 and the mockups:** at `≥lg` a collapsible section renders in the **non-collapsible presentation** (plain `<h2>`, no trigger, no summary line), not as an open disclosure. The desktop mockup's only `aria-expanded` is the Hero line-ups disclosure, Task 3.1 states the summary `<p>` renders only in the `<lg` presentation, and AC 1 scopes Accordion semantics to `<lg`. Flag in review if the other reading was intended.
- **Focus-on-expand is nonce-driven, not `open`-driven.** `TacticalSection` moves focus into the revealed region on an explicit request from the layer (user toggle or anchor), so a viewport change that opens all nine sections at `≥lg` — and the initial `≥lg` render — never steal focus. Ruled decision 7's target is the region (`role="region"` + `aria-labelledby`), not the heading.
