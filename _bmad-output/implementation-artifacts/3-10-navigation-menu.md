...
- [ ] [Review][Patch] **`trailingSlash: true` is not accounted for, so `aria-current` is dead and every nav link is a redirect** [app/src/lib/nav-destinations.ts:76-141,161] — `high`. `next.config.ts:9` sets `trailingSlash: true`; Next 16 derives `usePathname()` from `location.href` with no normalisation (`app-router.js:116-122` returns `url.pathname`; `create-initial-router-state.js:30` seeds `canonicalUrl` from `location.href`). The live pathname is `/compare/`, `/glossary/`, `/about/` — never the slash-less `route` values compared at `:161` with `===`. Only `home` (`/`) survives, so on three of the four shipped destinations `aria-current="page"`, the inline underline and the sheet's lime marker are all silently absent. Both suites miss it because they mock the hook with slash-less literals (`SiteNav.test.tsx:118-120`, `HeaderSearch.test.tsx:110-111`) — the gate asserts against an input the app never produces. **Same root cause, second symptom:** every `href` is slash-less too, against this repo's own ruled convention — `compare-url.ts:48` (`COMPARE_PATH = "/compare/"`, "the slash is mandatory"), `PlayerHero.tsx:185` ("a slash-less `/compare?…` is a REDIRECT rather than a link"). `"/tournament#results"` additionally breaks the ruled fragment form (`PlayerMatchesSection.tsx:156`: "THE TRAILING SLASH BEFORE `#` IS MANDATORY"), so the fragment rides through a 301. Fix: slash both `href` and `route`, and add a gate — `nav-destinations.test.ts` currently guards neither.
