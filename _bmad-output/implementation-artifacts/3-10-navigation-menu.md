...
- 🔴 **AC 7's PREMISE IS FALSE, AND THE RETROSPECTIVE DID NOT RUN.** AC 7 opens "Given the epic
  closes with this story". It does not: **At `d073575`, 3-2, 3-3 and 3-9 were `backlog` and 3-4 was `in-progress`.** (Corrected by the 2026-08-26 code review: the story, its Completion Notes, `sprint-status.yaml` and the commit message all said "3-2, 3-3, 3-4 and 3-9 are all still `backlog`", and this story's own disclosed 3-4 hunk is what made that false. The conclusion is unaffected — A6 triggers at epic close and `epic-3` is `in-progress` — but the evidence as written was wrong.) This is
...
Three adversarial layers (Blind Hunter, Edge Case Hunter, Acceptance Auditor) over commit
`d073575`. Every finding below was re-verified against `git show d073575:<path>` before being
rated — the working tree holds three other sessions' changes and was not trusted.

**Four findings are `high`, and three of them are regressions against shipped behaviour rather
than gaps in new work.**

### Decisions needed

- [x] [Review][Decision] **The sheet is pinned to the left edge from 387 px to 1279 px** — `ui/dialog.tsx:103` ships `fixed inset-x-0 top-0 … w-full`; the call site adds `max-w-[386px]` with no `mx-auto` (`SiteNav.tsx:452`). `left:0; right:0; width:386px` is over-constrained, so LTR drops `right` and the sheet renders as a 386 px column hard against the left edge while the trigger sits at the other end of the row. Below 387 px it is correct (full width). The `md`→`xl` move handed this entire band a presentation the mockup never drew — `key-navigation.html` has phone frames only — and the Task 9 matrix samples 195/320/390/1280, skipping every width in the band. Centre it (`mx-auto`), right-align it under the trigger, or keep it full-width until `xl`? **RULED 2026-08-26 (Juan, at review): right-align it under the trigger** — the sheet opens from the edge its button lives on, which is also the edge the trigger moves to once the alignment patch lands. Now a patch: add `ml-auto mr-0` (or equivalent) alongside `max-w-[386px]`, and add a matrix cell in the 387–1279 px band so the placement is measured rather than assumed.
- [x] [Review][Decision] **Below `xl` with the sheet closed there is no `<nav>` landmark on any route** — the only two `<nav>` elements are the inline one (`SiteNav.tsx:391`, `hidden xl:flex`, so `display:none` and out of the accessibility tree at every phone/tablet width) and the sheet's (`:503`, mounted only while open). A screen-reader user rotoring by landmark on a phone finds `banner` and `main` and no `navigation`, on all 1,406 routes. This may be the intended consequence of UX-DR24 collapsing nav behind one trigger, but AC 5 was signed off as "measured, not asserted" and the landmark list is not among the things Task 9.9 checked. Accept as ruled, or wrap the trigger in a labelled `<nav>`? **RULED 2026-08-26 (Juan, at review): wrap the trigger in a labelled `<nav>`** — below `xl` the trigger IS the navigation, so it belongs inside the landmark. Now a patch: wrap the `xl:hidden` trigger block in `<nav aria-label={t("nav.landmark")}>`, and assert the landmark's presence at both widths so the rotor gap cannot reopen. Zero visual cost.

### Patches

- [x] [Review][Patch] **`stickyInsetPx()` parses `rem` as `px`, collapsing the observer inset from 104 px to ~7 px** [app/src/components/CompareChartsSection.tsx:247-254] — `high`, and a regression: the diff replaces the shipped literal `rootMargin: "-104px 0px 0px 0px"` with a template reading `` -${stickyInsetPx()}px ``. Custom properties are unregistered here (`globals.css` has no `@property`), so `getComputedStyle(document.documentElement).getPropertyValue("--header-h")` returns the substituted token stream `"3.875rem"`, not a used length. `Number.parseFloat` strips the unit to `3.875`, which is finite and greater than zero, so the `Number.isFinite(parsed) && parsed > 0` guard passes and the fallback is unreachable. The inset becomes `-6.875px` (or `-10.375px` wrapped) against a correct ~110 px. A figure sitting entirely behind both sticky bars counts as intersecting, so the mini-header names the wrong side — the one question that bar exists to answer, and the direction the function's own docblock calls unsafe. The story's Task 9.3b log prints `--header-h=7.375rem (118px)`: the measurement harness converted the unit, the shipped function does not. Fix: multiply by the root `font-size`, or read the length off a resolved property. **Secondary, same site:** `stickyInsetPx()` is evaluated once inside the observer effect whose deps are `[isNarrow, pair]`, so crossing the 215 px `--header-h` boundary leaves a stale inset until remount.
- [x] [Review][Patch] **`trailingSlash: true` is not accounted for, so `aria-current` is dead and every nav link is a redirect** [app/src/lib/nav-destinations.ts:76-141,161] — `high`. `next.config.ts:9` sets `trailingSlash: true`; Next 16 derives `usePathname()` from `location.href` with no normalisation (`app-router.js:116-122` returns `url.pathname`; `create-initial-router-state.js:30` seeds `canonicalUrl` from `location.href`). The live pathname is `/compare/`, `/glossary/`, `/about/` — never the slash-less `route` values compared at `:161` with `===`. Only `home` (`/`) survives, so on three of the four shipped destinations `aria-current="page"`, the inline underline and the sheet's lime marker are all silently absent. Both suites miss it because they mock the hook with slash-less literals (`SiteNav.test.tsx:118-120`, `HeaderSearch.test.tsx:110-111`) — the gate asserts against an input the app never produces. **CORRECTED AT APPLY TIME — the `href` half of this finding was overstated.** The review claimed every slash-less `href` shipped a 301 hop. It did not: checked against the exported HTML in `app/out`, Next's `<Link>` NORMALISES the href at render, so a source `"/glossary"` was already emitted as `href="/glossary/"` in all 1,406 documents. No redirect ever reached a reader. The `href` fields are slashed anyway — the source should say what ships, and this repo rules the form explicitly (`compare-url.ts:48` "the slash is mandatory"; `PlayerMatchesSection.tsx:156` "THE TRAILING SLASH BEFORE `#` IS MANDATORY") — but it is consistency, not a defect fix. **The `route` half is unaffected and was CONFIRMED IN THE SHIPPED ARTIFACT:** `route` is a plain runtime string compared against `usePathname()`, which Next never touches. `app/out/glossary/index.html` carries ZERO `aria-current="page"` and `app/out/index.html` carries exactly ONE — home matching by accident and every other destination unmarked, exactly as predicted. Fix: slash both `href` and `route`, and add a gate — `nav-destinations.test.ts` currently guards neither.
- [x] [Review][Patch] **The sheet never closes when a destination is activated** [app/src/components/SiteNav.tsx:256-262] — `high`. `SiteHeader` lives in the root layout (`layout.tsx:48`), so client navigation does not unmount `SiteNav`; the `<Link>` carries no `onClick`, there is no effect on `pathname`, and the click is *inside* the dialog so Radix's outside-dismiss never fires. Every `setSheetOpen(false)` site is Esc, `DialogClose`, the `xl` effect, or the overlay registry. Tap *Glosario* on any phone: the route changes behind a still-open modal that Radix keeps inert and body-scroll-locked, and the reader sees the menu rather than the page they chose. Tapping the current destination navigates nowhere and still leaves it up. This is the feature's primary interaction on every viewport below `xl`.
- [x] [Review][Patch] **The menu trigger is no longer right-aligned on any phone or tablet** [app/src/components/HeaderSearch.tsx:340, app/src/components/SiteNav.tsx:407] — `high`, on all 1,406 routes. The header row is unchanged (`SiteHeader.tsx:120`: `flex min-h-14 max-w-6xl flex-wrap items-center gap-tile-gap`, no `justify-between`), and its only growing child was the search slot, which went from `min-w-0 flex-1` to `hidden min-w-0 flex-1 xl:flex` — `display:none`, so it is not laid out and its `flex-1` is inert below `xl`. The identity block is `flex min-w-0 flex-col` with no grow (`SiteHeader.tsx:161`), and the trigger's wrapper has `justify-end` but no `flex-1`/`ml-auto`, so `justify-end` has no free space to distribute. Both children pack at flex-start: wordmark, then hamburger, then an empty bar. The mockup is explicit — `key-navigation.html:68` defines a `.spacer{flex:1 1 auto;min-width:0}` used in frames A and B immediately before the trigger — and D10 predicted exactly this ("the header row. **This one changes**"), but it was declined. Invisible to Task 9, which recorded heights, `document.scrollWidth` and tab order; no cell measured the trigger's x-position. Fix: `ml-auto` on the wrapper, or `justify-between` on the row.
- [x] [Review][Patch] **`--header-h`'s breakpoint is `px`, so text-only zoom reopens the 46 px defect this story closed** [app/src/app/globals.css:538] — `medium`. `@media (min-width: 215px)` encodes the viewport width at which the header stops wrapping *at a 16 px root font*, but the row wraps on content and all of that content is `rem`-sized. A reader on a 24 px default font (WCAG 1.4.4 text resize — an AT setting, so the layout viewport does not shrink) wraps the header at roughly 320 px while the query still reports one row and reserves 62 px against a 118 px bar. Anchored headings and the `#main-content` skip-link target land 46 px behind the bar again — the identical WCAG 2.4.11 failure the ledger entry is being marked DONE for. Page zoom was tested (the 195 px cell); text zoom was not. Fix: make the query `rem`-based so it tracks the default font size.
- [x] [Review][Patch] **`scroll-padding-top` and `/compare`'s `scroll-mt` both count `--header-h`, and they are additive** [app/src/app/globals.css:535, app/src/components/CompareChartsSection.tsx:343] — `medium`. A fragment jump rests the target at `scroll-padding-top + scroll-margin-top` from the scrollport top, i.e. `(header + 16) + (header + 48)` — the header height reserved twice, 188 px against 110 px of actual bars, so an anchored figure lands ~78 px below the mini-header. The pre-token `72 + 112` had the same shape, but the rewritten comment now claims the sum is correct, which it is not. Fix: `scroll-mt` needs only the mini-header, since `scroll-padding-top` already reserves the header.
- [x] [Review][Patch] **`--spacing-compare-mini-h: 3rem` canonises a number the file it serves says is wrong** [app/src/app/globals.css:366] — `medium`. `CompareChartsSection.tsx:210-211` survives this change still reading "re-measure this bar rather than trusting the `~48 px` the scroll-mt figure was built on — the ledger measured 54", and `:309` spells out "so 112 needs to be 116". The story's stated purpose was to stop hardcoding the "56 header + 48 mini" arithmetic; it derived the wrong half and promoted the other wrong half into `globals.css` and `DESIGN.md:191`. Under-reserves by ~6 px. Task 9.7 measured the mini-header's `top`, never its height. Fix: re-measure, and correct the token and `DESIGN.md` together.
- [x] [Review][Patch] **AC 7 / Task 11.3: ledger entry L1465 was never marked closed** [_bmad-output/implementation-artifacts/deferred-work.md:4180] — `medium`. Task 11.3 ("Confirm L1465 is recorded adopted by story 3-7's contract. **Mark it closed.**") is ticked `[x]` and the Completion Notes say "the ledger walked", but `grep L1465` at `d073575` returns exactly one hit: the original Partition C re-deferral row, successor still "**A copy/UX pass**", untouched. Compare L1553/L1886, which story 3-8 closed with an explicit block at `:4494-4521` — that is the shape 11.3 asked for. The *confirm* half holds (`EXPERIENCE.md:473` carries the adoption); the *mark it closed* half was not performed. Unlike the retrospective half, this clause does not depend on the epic closing, so it is this story's own unfinished work.
- [x] [Review][Patch] **The `DONE` marker in the ledger is an orphan bullet detached from the entry it closes** [_bmad-output/implementation-artifacts/deferred-work.md:4468] — `medium`. It is inserted as a top-level `- status: **DONE — closed by story 3.10…**` item immediately *before* `- source_spec: spec-sign-the-project.md`. Every other entry in the file uses `- source_spec:` as the item head with `status:`/`summary:`/`evidence:` indented under it, so the spec-sign-the-project entry still carries no `status:` field and parses as open, with a detached status bullet floating above it. `bmad-loop-sweep` partitions this ledger by entry and will re-bundle the header-height entry as unresolved. Fix: indent it as a field of the entry it belongs to.
- [x] [Review][Patch] **Nav links ship in `ink-secondary` where the contract rules `ink-primary`** [app/src/components/SiteNav.tsx:265-278] — `medium`. `DESIGN.md:254` (`link-color: '{colors.ink-primary}'`), `:408` ("nav links are {colors.ink-primary}") and `:409` ("links in {typography.body} {colors.ink-primary}", "Inline links at `≥xl` use the same type and color") all rule one colour for both states; the mockup agrees (`.inlinenav a{color:var(--ink-primary)}`). Current-vs-not is meant to be carried by the underline, the lime marker and the weight change. Both variants instead give non-current links `text-ink-secondary`, adding an unruled second colour axis. Not an a11y failure (ink-secondary computes at or above 7:1), but the story's own framing is that the contract wins.
- [x] [Review][Patch] **`CompareChartsSection`'s docblock now contradicts itself in consecutive paragraphs** [app/src/components/CompareChartsSection.tsx docblock] — `medium`. The added text ends "**Both halves are tokens now.**" and is immediately followed by surviving context reading "⚠️ **BOTH HALVES OF THAT ARITHMETIC ARE NOW STALE** (2026-08-26 code review)…". Given the two findings above, the stale paragraph is closer to the truth than the new one, so this is more than cosmetic — the next editor cannot tell which is current.
- [x] [Review][Patch] **Focus is dropped when the `≥xl` sync effect closes the sheet** [app/src/components/SiteNav.tsx:346-364] — `medium`. `setSheetOpen(false)` unmounts `DialogContent`; Radix's `onCloseAutoFocus` calls `.focus()` on the trigger, but at that instant the trigger's ancestor matches `xl:hidden` (`:407`), i.e. `display:none`, so the call is a no-op and focus resets to `<body>`. A keyboard user who opens the sheet at 1000 px and then rotates a tablet or drags past 1280 px loses their place: the next Tab restarts from the top of the document (WCAG 2.4.3). Task 9.8's keyboard walk covered Esc, not the resize-close. Needs a deliberate focus target at `≥xl` — the wordmark link or the first inline destination.
- [x] [Review][Patch] **`resetAnnouncement()` runs on only one of the three close paths, against the invariant the code states** [app/src/components/SiteNav.tsx:322-327,355] — `low`. The registry closer and the `xl` effect both call `setSheetOpen(false)` directly, bypassing `handleOpenChange` where the reset lives (`:379`) — while `:371-378` asserts the reset happens on "BOTH edges". Type in the sheet's search (a 400 ms `announceTimer` pending), then cross to `≥xl` or open a glossary popover inside that window: the timer survives and fires against a torn-down region. The guarantee holds only via the reset on the *next* open.
- [x] [Review][Patch] **The exported-markup destination assertion samples one route while claiming 1,406** [app/src/app/static-output.test.ts:886] — `low`. It opens `const { html } = everyRouteHtml()[0];` where the sibling case three above iterates every route, and the failure message reads "a link to a 404 on every one of 1,406 routes". A per-route divergence in the nav passes.
- [x] [Review][Patch] **The same test's exact-zero half keys on labels the dictionary reuses** [app/src/app/static-output.test.ts:889] — `low`. The split matches bare anchor text for `Jugadores`, `Equipos`, `Partidos`, `Comparar` — strings that also live in five other `es.ts` namespaces (`:1041`, `:2396`, `:2402-2403`, `:2589`, `:3016-3023`). Green today only because those currently render inside buttons and tabs; the first component that wraps one in a `<Link>` reds this case with a message blaming `nav-destinations.ts`. The collision is already conceded for `Glosario` (the footer link) and worked around only on the available half. Scope the match to the nav's own markup.
- [x] [Review][Patch] **The underline word-boundary regex is satisfied by `underline-offset-[5px]` alone** [app/src/components/SiteNav.test.tsx:428] — `low`. The hyphen is a non-word character, so the boundary matches (verified). Deleting `underline` while keeping `underline-offset-[5px]` leaves the 1.4.1 assertion green. This gate was not among the five driven RED in Task 10.
- [x] [Review][Patch] **The `key` to `labelKey` pairing is never asserted** [app/src/lib/nav-destinations.test.ts] — `low`. The test only checks that `labelKey` starts with `nav.destinations.`, and `NavDestination.key` is a bare `string`, so casting `destination.key` to a dictionary key is unchecked in three places. A row pairing `key: "teams"` with `labelKey: "nav.destinations.players"` renders "Jugadores" pointing at `/teams`, and the static-output guard then looks up `es.nav.destinations.teams`, counts zero and passes. Type `key` as a union, or assert that `labelKey` equals `nav.destinations.` plus the key.
- [x] [Review][Patch] **`availableDestinations()` is exported, never called, and re-implemented at its only call site** [app/src/lib/nav-destinations.ts:147] — `low`. `git grep` at `d073575` returns the definition alone, while `SiteNav.tsx:252` writes the same `.filter(...)` inline. The module's whole argument is "one table, no drift", and it ships the single-filter helper alongside a call site that duplicates it.
- [x] [Review][Patch] **`SiteSignature.test.tsx` was not extended, though Task 8.4 required it and is ticked** [app/src/components/SiteSignature.test.tsx] — `low`. The File List reasons that nothing was weakened because nothing was touched — true of *don't weaken*, false of *extend for the new composition*. The file renders `<SiteHeader />` with no `next/navigation` mock, so it now exercises `SiteNav` with `usePathname()` returning `null` outside a router context, feeding `currentDestinationKey(pathname: string)` a non-string. It does not throw only because `find` misses — luck, not design. The file's own header comment ("mounts `HeaderSearch`, which fetches the tournament index") is also now false.
- [x] [Review][Patch] **"3-2, 3-3, 3-4 and 3-9 are all still `backlog`" is false, and this commit is what made it false** [_bmad-output/implementation-artifacts/3-10-navigation-menu.md, sprint-status.yaml] — `low`. The claim is repeated verbatim in the story file, the Completion Notes, `sprint-status.yaml` and the commit message. At `d073575` the statuses are `3-2: backlog`, `3-3: backlog`, `3-4: **in-progress**`, `3-9: backlog` — flipped by the disclosed 3-4 hunk that rode along in this same commit. The AC 7 conclusion survives (A6's trigger is epic close, and `epic-3` is `in-progress`); the stated evidence does not. Correct the sentence in all four places.
- [x] [Review][Patch] **`es.ts` reasons from a locale key this same change deletes** [app/src/locales/es.ts] — `low`. The new `nav.sheetTitle` comment says "on `search.sheetTitle`'s precedent it must NOT merely repeat the trigger" while the same diff deletes `search.sheetTitle`. State the rule rather than citing a key that no longer resolves.
- [x] [Review][Patch] **The new module mock was spliced between a docblock and the function it documents** [app/src/components/HeaderSearch.test.tsx:105-115] — `low`. The comment "jsdom is missing three APIs Radix Dialog calls unconditionally" is now immediately followed by the `next/navigation` mock, with `installDialogStubs()` after it, so the comment documents the wrong thing. This file's convention is that every stub carries its reason.
- [x] [Review][Patch] **Two `useSearchIndex()` instances now exist behind a docblock still promising "one corpus"** [app/src/components/HeaderSearch.tsx:138, app/src/components/SiteNav.tsx:246] — `low`. The retained JSDoc reads "one corpus, one live region, two presentations". `loadTournamentIndex()` dedupes the request — which is what the `SiteNav` comment claims — but not `searchEntities(tournament, labels)`, the `useMemo` that materialises the corpus from a 409 KB payload. Open the sheet on a tablet, widen past `xl`, focus the inline input: both engage and the corpus is built and retained twice. Correct the docblock at minimum.
- [x] [Review][Patch] **Story 3.9 is told "no change to any component", which by DESIGN.md's own rule invalidates `--header-h`** [app/src/lib/nav-destinations.ts:25-28] — `low`. `DESIGN.md:374` rules that *any change to the header's composition changes this token*. Flipping the four flags adds five inline links to the `≥xl` row, and the story's own 9.5 table shows the search input already squeezed from 511 px to 158 px in `es` at 1280 px. The bijection gate binds the flag to the filesystem; nothing binds it to a re-measurement of the token. Add the caveat to the comment 3.9 will read.

### Deferred

- [x] [Review][Defer] **`epics.md:143` still names the `header-h-zoom` token this change deletes** [_bmad-output/planning-artifacts/epics.md:143] — deferred, pre-existing scope boundary: not this story's file. UX-DR24 as written rules four tokens (`header-h-oneline`/`-wrapped`/`-zoom` plus `scroll-clearance`); `globals.css` and `DESIGN.md` now ship three plus a new, unruled `compare-mini-h`. The deletion's measurement is genuine (195 px reports 117.8 px, identical to every other wrapped width) and D9 authorises re-measuring *values*, but this changed the token *set*, so two planning artifacts now disagree about what UX-DR24 rules.
- [x] [Review][Defer] **The bijection gate has a route-group blind spot** [app/src/lib/nav-destinations.test.ts] — deferred, robustness gap rather than a defect. Direction 2 resolves a route to `src/app{route}/page.tsx` literally, so a route minted at `src/app/(marketing)/tournament/page.tsx` or as `page.jsx` would not fire it, and the nav would stay four wide beside a shipped, unlinked route. Direction 1 and the dynamic-segment rejection are unaffected and well-tested.
- [x] [Review][Defer] **The sheet ships square bottom corners against a ruled `{rounded.md}`** [app/src/components/ui/dialog.tsx:103] — deferred, pre-existing. Inherited from Story 2.14 and correctly not fixed here (D4: "`ui/dialog.tsx` needs no change", and the house rule against editing a primitive for one consumer). The mockup draws a 10 px bottom radius. The deviation is now UX-DR24's as well as 2.14's and was recorded nowhere.
- [x] [Review][Defer] **Task 9.2 is ticked against a bar it did not clear** [_bmad-output/implementation-artifacts/3-10-navigation-menu.md] — deferred, already filed in the ledger. The bar was "zero horizontal overflow, every cell"; 4 of 96 cells overflowed (`/` at 195 px, `doc=208`). The attribution is correct (`LeaderboardsRegion.tsx:191,193`, `w-48` is a fixed 192 px plus 16 px of gutter), the file is outside this story's declared paths, and it is honestly recorded — but the subtask reads complete against an unmet bar. Does not touch AC 3, whose bar is 320 px and is clean.

### Dismissed as noise

- `data-slot="site-nav-trigger"` replacing the primitive's `data-slot="dialog-trigger"` — intentional, the tests scope on it, no defect.
- "This commit *silently* flips another story's sprint status" — the commit message discloses the 3-4 hunk explicitly and its content is accurate. The residue worth fixing is the "all still backlog" sentence, kept as a patch above.
- The AC 7 retrospective deferral itself — A6's trigger is epic close, `epic-3` is `in-progress`, and four Epic 3 stories remain open. The reasoning holds; only its stated evidence was wrong.

### What the layers confirmed as correct

Recorded so it is not re-reviewed: the availability bijection is real and fallible in both
directions, resolved by relative path, with dynamic segments rejected before resolution; AC 2's
"no second mechanism" pin is genuinely two-sided (data gate plus a source-level guard on
`match-anchors` / `use-anchor-nonce` / `hashchange`); `aria-controls` is conditional and the
sheet carries the matching `id`, with the closed-state absence asserted; AC 6 holds byte-for-byte
(the caption is still a sibling of the wordmark link and the reflow-guard needle is untouched);
AC 3's guarded class string survived and 320 px is clean in both locales and themes; the two live
regions cannot both be live, by structure rather than by convention; the three deleted locale keys
are genuinely dead; D11's implicit-grid trap is real and was avoided correctly; and the
`calc()` inside the arbitrary `scroll-mt` is valid under Tailwind v4.3.3.
| 2026-08-26 | **Code review complete: `review` -> `done`.** Three adversarial layers over `d073575`; 26 patches applied at `4133195`, 4 deferred to the ledger, 3 dismissed. Four `high` findings, three of them regressions: the `rootMargin` that read an unregistered custom property and ran at -6.875px against the -104px literal it replaced; `trailingSlash: true` unaccounted for, so `aria-current` was dead on three of four destinations (confirmed in the export, then re-confirmed fixed); the sheet that never closed when a destination was activated; and the trigger that packed flush against the wordmark on every phone once the search slot stopped being laid out. **One review claim was overstated and corrected against the build:** slash-less `href`s did NOT ship a 301 -- Next's `<Link>` normalises them -- so that half was consistency, not a defect. Five new gates, each driven RED first. AC 7's ledger clause discharged (L1465 recorded CLOSED); its retrospective clause remains correctly unrun, since A6 triggers at epic close and `epic-3` is `in-progress`. 1,504/1,504 tests, 0 skipped; 1,406 routes. |
