...
- 🔴 **AC 7's PREMISE IS FALSE, AND THE RETROSPECTIVE DID NOT RUN.** AC 7 opens "Given the epic
  closes with this story". It does not: **3-2, 3-3 and 3-9 are `backlog` and 3-4 is `in-progress`.** (Corrected by the 2026-08-26 code review: the story, its Completion Notes, `sprint-status.yaml` and the commit message all said "3-2, 3-3, 3-4 and 3-9 are all still `backlog`", and this story's own disclosed 3-4 hunk is what made that false. The conclusion is unaffected — A6 triggers at epic close and `epic-3` is `in-progress` — but the evidence as written was wrong.) This is
