...
Adversarial code review 2026-07-23 (Blind Hunter + Edge Case Hunter + Acceptance Auditor over `d60827c..e6cfa7e`):

- [x] [Review][Decision] Outcome cross-check runs as a pair-formation filter, not the spec'd post-link demotion (`pipeline/markers/linking.py:202-212`) — RESOLVED by user 2026-07-23: assignment-constraint design **accepted** (it resolves overlapping-marker label swaps that post-hoc demotion would wrongly kill); divergence from Task 3's letter is signed off. Follow-up became a patch item below (pin the reroute-to-next-nearest path with a test).
- [x] [Review][Patch] Add an assignment-level test pinning the ratified reroute behavior — a marker whose nearest glyph is outcome-incompatible links to the next-nearest *compatible* glyph within radius (and unlinks when none exists) [pipeline/tests/test_markers_linking.py]
- [x] [Review][Decision] `DETAIL_COMPATIBLE_OUTCOMES` accepts both `incomplete` and `on-target` for `deflected-on-target-defensive-event`, ahead of AD-14 (`pipeline/markers/attempts.py:125-128`) — RESOLVED without user input: already adjudicated by the Story 2.3 sign-off (2026-07-23), CR-2 filed and batched into CS-1; the interim code behavior matches the ratified decision. No action for this story.
- [x] [Review][Decision] Staging records carry two non-contract `ShotOutcomeDetail` values (`incomplete`/`on-target`, `pipeline/markers/attempts.py:59-61`) — RESOLVED without user input: already adjudicated by the Story 2.3 sign-off (2026-07-23), CR-1 filed and batched into CS-1; Story 1.16 remains blocked pending CS-1 as the ledger records. No action for this story.
- [x] [Review][Patch] Story record understates the commit's true contents — `19816fc` bundles Story 1.6's non-additive Domain A rework (`_domain_a_memo`/`_domain_a_uncached`, `domain_a_checks` one-arg signature, `UnknownMinuteGlyphError` → `unknown-rgb` category remap in `pipeline/validate/checks.py`; import rework in `pipeline/ingest/extract_report.py`; timestamp-whitelist tightening `.endswith(".kickoff")` → exact path in `pipeline/tests/test_ingest_record.py`) which also silently repaired a 4-commit `ImportError` on main (`aggregate_self_validation` moved by 0643843). The Debug Log discloses some of this but the File List does not. Fix: amend File List + Completion Notes to credit the 1.6 repairs and the broken-main window. [high]
- [x] [Review][Patch] Row-count "divergence is a parser bug" assertion is a tautology — `parse_attempt_rows` compares its row count against `attempts_table_count`, which recomputes the identical pure functions on the identical page; `f(x) != f(x)` can never fire, and it doubles the words/clustering work per table page while docstrings sell it as a defensive net [pipeline/markers/attempts.py:240-248]
- [x] [Review][Patch] Rescue pass can re-claim a released whole glyph, undoing collision resolution — the `claimed_ordinals` guard in `greedy` only blocks *split* parts, so a pass-1 duplicate-ordinal claimant released by the keep-nearest step can re-claim the same whole word in pass 2, recreating the duplicate and unlinking the kept claimant too (loud/safe direction, but the documented keep-nearest mechanism self-defeats) [pipeline/markers/linking.py:227]
- [x] [Review][Patch] Merged-word rescue has zero assignment-level tests — split readings are tested only at `collect_digit_glyphs` level; no test drives `link_markers` through whole-claim vs split-rescue, interval-conflict rejection, or release-then-rescue, though 10 corpus reports depend on the path and CI auto-skips real PDFs [pipeline/tests/test_markers_linking.py]
- [x] [Review][Patch] `exclude_legend_rows` is production-dead — `parse_shots` inlines the legend predicate, so the tested public filter-chain function no longer guards the production path and the exclusion rule lives in two places [pipeline/markers/shots.py:98-102]
- [x] [Review][Patch] `_word_readings` partition enumeration is unbounded — a pathological long digit word (e.g. all 1s with row_count ≥ 11) grows readings Fibonacci-style before flattening to parts; cap merged-label length or derive parts without enumerating full readings [pipeline/markers/linking.py:74-100]
- [x] [Review][Patch] `format_summary` selects the link-rate template by key-sniffing (`"unlinked" in check`) instead of `check.get("check") == "shots-link-rate"` — a future check family carrying an `unlinked` key silently adopts the wrong rendering [pipeline/ingest/batch.py:440]
- [x] [Review][Patch] Weak assertion in `test_a_corrupted_out_of_range_label_unlinks_its_marker` — asserts the *set* `{True, False}` without checking which marker unlinked; a swapped outcome would still pass [pipeline/tests/test_markers_linking.py]
- [x] [Review][Patch] "Full suite green" checked without a recorded final tally (Git-intelligence requires exact numbers; baseline had a waved-off transient fail) — run the full suite and record the count [story Dev Agent Record]
- [x] [Review][Patch] No drift guard ensuring every `OUTCOME_LABEL_TO_DETAIL` value is a `DETAIL_COMPATIBLE_OUTCOMES` key — today it holds by construction; a drift would surface as a bare `KeyError` inside `link_markers`, violating its nothing-raises docstring; add a one-line coverage test [pipeline/markers/attempts.py:125]
- [x] [Review][Defer] `_domain_a_memo` holds a module-scope strong reference to the last `pymupdf.Document` and caches arbitrary exceptions for identical re-raise [pipeline/validate/checks.py:311-331] — deferred, copies the pre-existing `_parse_memo` pattern; single-process pipeline; revisit with Domain A ownership

All 11 patches applied 2026-07-23 and verified: full suite **663 passed + 1 skipped**; full batch **104/104 extracted, RUN RESULT: PASS, 0 self-validation failures**; gate **PASS, 0 deviations across 16 sampled reports**. Patch notes: the tautological row-count assert was REMOVED (post-factor-out both sides computed the same functions on the same input; docstrings in `attempts.py`/`shots.py` now state the shared-admission-rule guarantee honestly — the Implementation Plan's "divergence assert kept as a defensive net" line describes the pre-review state); `parse_shots` calls `exclude_legend_rows` again; `_word_readings` capped at `_MAX_MERGED_WORD_DIGITS = 8`; `format_summary` dispatches on the check id; the pass-2 whole-word re-claim loophole was kept (it IS the mechanism delivering Task 7's duplicate-ordinal both-unlink semantics) with the collision-resolution comment rewritten to say so, and the merged-word rescue geometry is now pinned by four new assignment-level tests plus the ratified reroute test.
...
- **Commit-scope disclosure (added by the 2026-07-23 code review):** the 1.6 review session (0643843) committed `pipeline/extract/domain_a.py` with `aggregate_self_validation` removed and `domain_a_checks` reduced to one argument, but never committed the matching consumer edits — so at 0643843, 18f2c50, 2276256 and d60827c the committed tree raised `ImportError` on `from pipeline.extract.domain_a import aggregate_self_validation`. Commit 19816fc (this story's) is what landed the repairs: the `checks.py` Domain A rework (`_domain_a_memo`/`_domain_a_uncached`, one-arg `domain_a_checks(payload)`, `UnknownMinuteGlyphError` re-bucketed `probe-failure` → `unknown-rgb`, specifics format `f"{type(exc).__name__}: {exc.reason}"`), the `extract_report.py` import rework (`aggregate_self_validation` now from `pipeline.extract`), and the `test_ingest_record.py` timestamp-purity whitelist tightening (`.endswith(".kickoff")` → the exact `.domains.match_metadata.kickoff` path). The same commit also carried Story 1.7's tracking move in `sprint-status.yaml`. None of this is Story 1.5 work; it rode along uncredited and is recorded here so `git log -p 19816fc` reads truthfully.
...
- `pipeline/ingest/extract_report.py` — `link_rate_checks` appended to Self-Validation (additive); ALSO carries the 1.6 repair: `aggregate_self_validation` import moved to `pipeline.extract`, `domain_a_checks` called one-arg (see Debug Log commit-scope disclosure)
- `pipeline/ingest/batch.py` — `format_summary`: link-rate failure rendering branch (additive)
- `pipeline/validate/checks.py` — `marker-event-link-rate` check registered (append-only), docstring roster updated; ALSO carries the 1.6 repair: `_domain_a_memo`/`_domain_a_uncached`, `_domain_a_payload` reworked for the one-arg `domain_a_checks`, `UnknownMinuteGlyphError` re-bucketed to `unknown-rgb` (see Debug Log commit-scope disclosure)
...
- `pipeline/tests/test_ingest_record.py` — new independent `shots-link-rate` test; ALSO carries the 1.6-owned timestamp-purity whitelist tightening (`.endswith(".kickoff")` → exact path; see Debug Log commit-scope disclosure)
...
- 2026-07-23: Adversarial code review (Blind Hunter + Edge Case Hunter + Acceptance Auditor over `d60827c..e6cfa7e`): 19 unique findings — 3 decisions (2 already adjudicated by the Story 2.3 sign-off CR-1/CR-2; the cross-check-as-assignment-constraint design ratified by user), 11 patches applied, 1 deferred to the ledger, 5 dismissed. Re-verified: suite 663+1, batch 104/104 PASS, gate PASS 0 deviations. Status review → done.
