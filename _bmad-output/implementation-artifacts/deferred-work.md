...
## L1465 — CLOSED (story 3.10, recorded by its code review 2026-08-26)

**L1465 is CLOSED.** Task 11.3 of story 3-10 required two things of this entry — confirm it is
recorded as adopted by story 3-7's contract, and **mark it closed**. The subtask was ticked `[x]`
and the story's Completion Notes state "AC 7 — the ledger walked", but only the first half was
performed: at `d073575` the ledger carried exactly one mention of L1465, the original Partition C
re-deferral row, successor still `**A copy/UX pass**`, untouched. Compare L1553/L1886, which story
3-8 genuinely closed by appending an explicit block; that is the shape 11.3 asked for and did not
produce. Recorded here by the code review rather than left as a false tick.

**The adoption is real and verifiable.** `EXPERIENCE.md:473` carries it in the shipped UX contract:
"The Expert table at 390 px — ledger L1465, ruled." The entry's substance — two data columns at
390 px in the Expert table, measured at 55.7 px of data columns with the `<md` escape hatch buying
88 px — was resolved by story 3-7's ruled abbreviations lever (EXPERIENCE.md:139) rather than by a
reflow change, which is why it never produced a code diff to point at. 320 and 390 both pass with
every disclosure open.

**Successor: none.** The "copy/UX pass" the original row named as its successor was the ruling, and
the ruling landed in 3-7's contract. Nothing further is owed.

**Note on AC 7's other half:** the Epic 3 retrospective (A6) remains correctly UNRUN. A6 triggers
at epic close and `epic-3` is `in-progress` — 3-2, 3-3 and 3-9 are `backlog` and 3-4 is
`in-progress` — so the premise of AC 7's "Given the epic closes with this story" is false. Story
3.10 was right to say so rather than tick it. That half of AC 7 is not this story's to discharge;
this half was, and now is.

## Deferred from: code review of 3-4-sitemap-robots (2026-08-26)

Appended, never regenerated: every line above this heading is unchanged. Five entries, all verified
against `git show 5754522:<path>` and the emitted `out/` tree rather than the working tree, which
held a concurrent session's uncommitted `layout.tsx` (story 3.2) while this review ran.

- source_spec: `3-4-sitemap-robots.md` (code review)
  status: OPEN — design ruling needed before the first nested static route exists.
  summary: `discoverStaticRoutes` cannot see a static route nested beneath a dynamic segment.
  evidence: `sitemap.ts:108` skips bracketed directories *before recursing*, so `src/app/teams/[slug]/squad/page.tsx` — an ordinary Next route — would never be discovered and would silently miss the sitemap. The docblock at `:91-92` claims the function returns "Every non-dynamic route path"; it returns every non-dynamic route path not nested beneath a dynamic one. Deliberately NOT patched in this review: the correct behaviour is a real design question, because such a route must be expanded once per entity (1,248 `/players/<id>/stats/` entries, say), and that is a mapping story 3.4 did not rule. The module's defensive posture is asymmetric today — unmappable segment *kinds* throw loudly at `:82`, unmappable dynamic *families* vanish silently. **Owner: whoever adds the first static route under a dynamic segment.** Fix at that time: recurse into bracketed directories and expand the tail per manifest entity, or extend the throw to cover the case so it cannot ship silently.

- source_spec: `3-4-sitemap-robots.md` (code review)
  status: OPEN — pre-existing, shared with `static-output.test.ts`; inherited by 3.4, not introduced by it.
  summary: The export-grounded test layers silently skip when no build exists, and a stale `out/` is undetectable.
  evidence: `sitemap.test.ts:45` is a bare `existsSync(OUT_DIR)` — no build-id check, no mtime comparison against `src/`. Nine assertions across Layers 3 and 4 (the only layers that can support AC5's "no listed URL 404s" claim, and the only coverage `robots.ts` has at all) therefore run only when a human happened to build first. Nothing in the repo enforces that: `package.json` `"test"` is a standalone `vitest run`, `"build"` runs no tests, there is no `.github/` directory, and `netlify.toml` runs `npm run build` alone at deploy. The story's header at `:36` warns "`npm run build` MUST precede `npm test` or the export layers silently skip" and the completion notes record hitting exactly this class of failure ("after Task 3 the module was correct but `out/` still held the two-entry probe"), yet the response was still `existsSync`. The file's own skip-guard reasoning at `:38-42` — that keying on `out/sitemap.xml` would report a missing sitemap as a green skip — is correct and one level too low: a fresh clone reports nine green skips for the identical reason. **Owner: whoever next hardens the export-grounded suites, or introduces CI.** Fix: stamp the build id / a manifest hash into `out/` and assert freshness, or add a `pretest` build step, or run the suite in CI after a build.

- source_spec: `3-4-sitemap-robots.md` (code review)
  status: OPEN — verification debt on shipped gates; one of them was concealing a live defect.
  summary: A1 was driven against five gates plus one extra, but seven shipped assertions were never proven red.
  evidence: Task 6's "Six gates driven RED" is accurate as a count and the evidence is verbatim and genuine. Never driven red: AC3's self-origin test (`sitemap.test.ts:127-133`), AC4's `/compare` test (`:165-171`), D3's decoration test (`:157-163`), the duplicate-`<loc>` test (`:151-155`), the query/fragment test (`:143-149`), and both AC7 shape tests (`:283-292`). The last is the whole argument for this entry: the natural red case for "allows every user agent" is a `Disallow: /` robots.txt, and running it would have surfaced immediately that `/Allow:\s*\//i` is unanchored and **matches `Disallow: /` as a substring** — the HIGH finding patched in this review. A1 as practised selected the gates the AC named explicitly and left the shape assertions unproven. **Owner: standing Epic 3 A1 practice.** Fix: treat "every assertion in a new test file" as A1's scope, not "every gate an AC names".

- source_spec: `3-4-sitemap-robots.md` (code review)
  status: OPEN — robustness; the cited precedent already solved this one level away.
  summary: A manifest failure takes down the robots.txt layer, which does not read the manifest.
  evidence: `sitemap.test.ts:47` calls `sitemap()` at module scope, so a `readTournament()` throw kills collection for the entire file — including Layer 4, whose subject is `robots.ts` and which touches no manifest. `teams/static-output.test.ts:36-50`, quoted approvingly by this file's own header, records precisely this lesson from a prior code review: a collection-scope read that "threw during COLLECTION and took the whole file down with it, including … the message written to report exactly this." The fix applied there (a guarded accessor returning `""`) was applied here to the export reads (`:237-241`, `:276`) but not to the module call. Low impact today — a `readTournament()` throw means the build is already broken — but it costs the robots diagnostics at exactly the moment they would be read. **Owner: whoever next edits `sitemap.test.ts`.** Fix: make `entries`/`urls` lazy accessors rather than module-scope constants.

- source_spec: `3-4-sitemap-robots.md` (code review)
  status: OPEN — record accuracy; bookkeeping, not code.
  summary: Task 6.6 is ticked `[x]` against a different demonstration than the one it specifies.
  evidence: Subtask 6.6 (spec:240) reads "Confirm the whole file is green again and that the pre-existing suite is untouched". The completion notes' "6.6" (spec:653) is the unplanned `out/sitemap.xml`-deleted skip-guard red — a genuinely valuable demonstration, and the right thing to have recorded, but not that subtask. The substance of the real 6.6 appears only indirectly, in Task 8.1's chain figures (59 files / 1,488 tests / 0 skipped). Same shape as the 3.10 finding logged above: a subtask reading complete against a bar it did not literally measure. **Owner: bookkeeping pass on 3-4.** Fix: renumber the skip-guard red as 6.7 and point 6.6 at the Task 8.1 chain figures.

## Deferred from: code review of 3-2-metadatabase-canonical-urls (2026-08-26)

Appended, never regenerated: every line above this heading is unchanged. Four entries, verified against
`git show 30905dc:<path>` and the emitted `out/` tree rather than the working tree, which held a
concurrent session's uncommitted `sitemap.ts`/`robots.ts`/`assert-no-external-origins.*` (story 3.4)
while this review ran.

- source_spec: `3-2-metadatabase-canonical-urls.md` (code review)
  status: OPEN — **extends the identical entry filed by 3.4's review above to a second file.** Same class, same owner.
  summary: The canonical guard never runs on any automated path, and a stale `out/` scores the same green as a fresh one.
  evidence: `canonical-output.test.ts:203-206` gates all eight cases behind `describe.skipIf(!anyBuilt)` keyed on `existsSync(out/index.html) || existsSync(out/404.html)`. `app/.gitignore:3` ignores `out/`, so on any fresh clone the entire AC2/AC3/AC4 gate reports green having asserted nothing. Nothing runs it regardless: `package.json:16` `"build"` is `lint && typecheck && assert:schema-version && next build && copy-data && assert:no-external-origins` with no `vitest`; `"test"` is a standalone `vitest run` that never builds; `netlify.toml:8` runs `npm run build` alone; there is no `.github/`. Worse than the skip, the freshness hole: nothing correlates `out/` with `src/`. Confirmed by running `npx vitest run src/app/canonical-output.test.ts` with no build of any kind — 8 passed. Delete `alternates: { canonical: "./" }` from `layout.tsx` and re-run without rebuilding and all eight still pass, which is precisely the failure the file exists to prevent. The docblock at `:80-84` claims to have closed "the `scanned === 0` lesson", but `REQUIRED_DOCUMENTS` only fires *after* `skipIf` has let the run through. Four shipped source comments (`page.tsx:79`, `matches/[slug]/page.tsx:54`, `players/[slug]/page.tsx:85`, `teams/[slug]/page.tsx:110`) each state verbatim "`canonical-output.test.ts` is the gate that catches it" — true only if a human remembers to build, then test. **Owner: whoever next hardens the export-grounded suites, or introduces CI** (same owner as 3.4's entry). Fix: stamp the build id / a manifest hash into `out/` and assert freshness, or add a `pretest` build step, or run the suite in CI after a build. Note this now covers `sitemap.test.ts`, `static-output.test.ts` *and* `canonical-output.test.ts` — three files and ~17 assertions.

- source_spec: `3-2-metadatabase-canonical-urls.md` (code review)
  status: OPEN — latent; fails in the LOUD direction, which is why it is deferred rather than patched.
  summary: `servedUrl` builds expected canonicals from raw filesystem bytes with no percent-encoding.
  evidence: `canonical-output.test.ts:186-192` composes `${SITE_ORIGIN}/${relativePath.slice(...)}` by template interpolation only — no `encodeURI`, no `new URL(path, SITE_ORIGIN).href` normalisation. Next resolves `metadataBase` + pathname through `new URL(...)`, which percent-encodes non-ASCII. All 1,400 entity directories are ASCII today (`find players teams matches -maxdepth 1 -mindepth 1 -type d | grep -P '[^\x00-\x7F]'` returns nothing), which is the only reason it passes. Should a slug generator change or a name like `türkiye` survive un-transliterated, Next emits `href=".../teams/t%C3%BCrkiye/"` while `servedUrl` computes `.../teams/türkiye/`: two cases go red naming a canonical that is entirely correct, sending the reader after Next rather than after the test. The function's own comment claims derivation "from its own relative path and nothing else", which is exactly the assumption that breaks. **Owner: whoever introduces the first non-ASCII slug.** Fix: normalise through `new URL()` on both sides before comparing.

- source_spec: `3-2-metadatabase-canonical-urls.md` (code review)
  status: OPEN — pre-existing and already self-declared in `robots.ts`; this review's gate walks past it by construction.
  summary: 11,235 crawlable `.txt` RSC payloads duplicate every page's text and carry no canonical.
  evidence: `canonical-output.test.ts:142` walks `.html` only. The export also ships `index.txt`, `__next.*.txt` and per-route `.txt` flight payloads — 11,235 files outside `out/data` and `out/_next` — which are `text/plain`, absent from `sitemap.xml`, allowed by `robots.txt`'s `Allow: /`, and structurally incapable of carrying a `<link rel="canonical">` (that would need an HTTP `Link:` header, and `netlify.toml` sets none). AC3's "every indexable route emits a canonical" is therefore satisfied only under an `.html`-shaped reading of "route". `robots.ts:41-49` already names this as known and defers it to "a future story", so 3.2 neither introduced nor worsened it. **Owner: the future story `robots.ts:41-49` already names.** Fix at that time: a `X-Robots-Tag: noindex` header for `*.txt` in `netlify.toml`, or a `Disallow` rule, whichever survives the pure-static constraint.

- source_spec: `3-2-metadatabase-canonical-urls.md` (code review)
  status: OPEN — record accuracy in a file this story may not touch (standing AC A3: held by the live 3.4 session).
  summary: A shipped test asserts `hreflang` is "the other tag 3.2 emits". 3.2 emits none.
  evidence: `assert-no-external-origins.test.ts:228-231` carries the case name `"PASSES a self-origin <link rel=\"alternate\" hreflang> — the other tag 3.2 emits"` with a fixture `<link rel="alternate" hreflang="en" href="${SITE_ORIGIN}/en/players/quinones/">`. Story 3.1 wrote that in anticipation; AC4/§D7 (D17, upheld by D20) forbid it, and the export carries **0** `hreflang` across all 1,407 documents. The fixture is not a second-origin or D17 breach — it is the only `hreflang` string in `src/` and it never reaches the export — but AC4 is now contradicted in prose by a green test, and nobody recorded it. Not patched here because it is 3.1's file, not 3.2's. It was also **modified and uncommitted by the concurrent 3.4 session** for the duration of this review; that session committed at `8e07471` before this review closed, so the A3 block has lifted and the case name survived it unchanged — verified at `8e07471`. Ownership is now the only thing deferring it. **Owner: whoever next edits `assert-no-external-origins.test.ts`.** Fix: rename the case to say the tag is one the gate must *tolerate*, not one 3.2 emits.

## Deferred from: code review of 3-3-og-image-twitter-card (2026-08-27)

Appended, never regenerated: every line above this heading is unchanged. Two entries, both verified
against the working tree at `1c19b25` and the emitted `out/` tree (1,407 documents) rather than read
off the story record. The first EXTENDS an existing open entry rather than duplicating it.

- source_spec: `3-3-og-image-twitter-card.md` (code review)
  status: OPEN — **extends the `skipIf(!anyBuilt)` entry filed by 3.2's review above to a fourth assertion.** Same class, same file, same owner.
  summary: The new AC5 same-origin card guard inherits the skip, so on any tree without `out/` the "only thing holding the same-origin line" passes having asserted nothing.
  evidence: Story 3.3 adds `it("emits ONE same-origin og:image on every exported document (3.3 AC 5)")` at `canonical-output.test.ts:405` — inside the same `describe.skipIf(!anyBuilt)` at `:295` that 3.2's review already filed, keyed on `existsSync(out/index.html) || existsSync(out/404.html)`. Its two per-route twins sit behind the same construct (`players/static-output.test.ts:86`, `teams/static-output.test.ts:116`). `app/.gitignore:3` ignores `out/`, so on a fresh clone or a cleaned worktree all three card assertions vanish and the run reports green. Nothing runs them on an automated path: `package.json:16` `"build"` carries no `vitest`, `"test"` never builds, `netlify.toml:8` runs `npm run build` alone, and there is no `.github/`. What 3.3 adds to the existing entry is the PROSE RISK: three shipped source comments now state that these assertions are "the only thing holding the same-origin line" (`players/static-output.test.ts:696-699`, `teams/static-output.test.ts:802-805`, `matches/[slug]/page.tsx:493-497`) with no mention that they are conditionally inert — so a future reader trusts a guard that may never have run. **Owner: whoever next hardens the export-grounded suites, or introduces CI** (unchanged from the 3.2 entry). Fix unchanged: stamp a build id / manifest hash into `out/` and assert freshness, or add a `pretest` build step, or run the suite in CI after a build. This now covers `sitemap.test.ts`, `static-output.test.ts` (both copies) and `canonical-output.test.ts` — and, with 3.3, roughly 20 assertions.
