...
Adversarial code review 2026-08-03 over `c645cfe..5344fac` (3 layers: Blind Hunter, Edge Case Hunter, Acceptance Auditor). 32 raw findings deduped to 22; 12 dismissed on verification, 2 decisions ruled by Juan. Every finding below was re-read against the code before rating — the reviewers' own severities were discarded.

**DECISION 1 — RULED: record only, do not repair history.** Commit `5344fac` does not import: `pipeline/ingest/extract_report.py:57,337` import and call `domain_e_goalkeeper_warnings`, which does not exist anywhere in the tree at that commit — its definition lands two commits later at `7f9b959:pipeline/extract/domain_e.py:1418`. `import pipeline.ingest.extract_report` therefore raises `ImportError` at `5344fac`, and the whole pipeline plus every test module in the change fails at collection. The same commit's `checks.py` `_domain_e_uncached` also passes `lineups=None` into a `extract_domain_e` that at `5344fac` still declares `lineups: dict` and dereferences it unconditionally. Both are resolved at HEAD by `7f9b959`, so this is a **history-integrity defect, not a live one** — but `git bisect` across `5344fac` and any checkout of it are broken, and that is now recorded rather than repaired. Ruled: the breakage and the undisclosed co-commit are written into the Completion Notes and sprint-status; history stands as published.

**DECISION 2 — RULED: leave as is.** `pass-network-row-bound` failing when a matrix player has no Domain G row (`pipeline/extract/pass_network.py:713-718`) is intended: a matrix player absent from Domain G is a genuine cross-domain inconsistency and is worth failing on, even though it also surfaces as a `count-mismatch` deviation under `pass-network-counts`. No patch.

- [x] [Review][Patch] Both BOUND checks discard their delta census on any failing report, against Tasks 4.3/4.4's "record the delta on every report" [pipeline/extract/pass_network.py:733-741, :759-763] — and for `pass-network-total-bound` a passing side's delta is dropped because the *other* side failed. Same shape discards `top5_totals` at `:692`.
- [x] [Review][Patch] `pipeline/README.md:1101-1103` claims the printed Top-5 panel "validates the matrix total — and therefore every cell"; all three shipped checks are invariant under any permutation of the matrix's off-diagonal cells [pipeline/extract/pass_network.py:670-687] — `volumes = sorted(...)` uses no positional operand, and `matrix_total` and the row sums are permutation-invariant too. No shipped check corroborates the geometric column assignment; the only evidence is the uncommitted prototype comparison and the prose mutation check.
- [x] [Review][Patch] `test_the_tolerance_is_the_printed_half_ulp_and_admits_exactly_it` does not exercise the tolerance [pipeline/tests/test_extract_pass_network.py:700-712] — it mutates the printed value to `32.1 + 0.05` against a computed `32.1428…`, a delta of **0.0071**, not 0.05. Combined with the clean fixture (delta 0.0429) and the out-of-tolerance test (`40.0`, delta 7.86), `TOP_RANKED_PCT_TOLERANCE` could be set anywhere in ≈`[0.043, 7.85]` with the suite fully green.
- [x] [Review][Patch] The half-ulp comparison can false-fail on float representation [pipeline/extract/pass_network.py:682] — `delta > TOP_RANKED_PCT_TOLERANCE`. The three worst corpus cases land at `0.04999999999999982` (below), but a symmetric case landing at `0.050000000000000044` fails a faithful page. Needs an explicit epsilon, not a changed constant.
- [x] [Review][Patch] Sibling payloads are read with bare subscripting, so a shape change raises an untyped `KeyError` [pipeline/extract/pass_network.py:711, :720, :749] — this is exactly the defect the Story 1.9 review patched out of `_goalkeepers`. At the `extract_report` seam it aborts the report with a non-typed error; at the gate `runner.py:85-87` catches broad `Exception`, so it degrades to a deviation rather than a crash.
- [x] [Review][Patch] The synthetic fixture's column 0 is degenerate [pipeline/tests/conftest.py:2142-2155] — `_pass_network_base(i, 0) == 0` for every `i`, and `_pass_network_spike_cells` only ever targets odd columns, so `colsum[0] == 0` at every size (verified 11–18). The first player's `passes_received` is always 0 and column 0 is indistinguishable from a parser writing zeros there — the exact uniformity risk the function's own docstring exists to prevent. `test_the_fixture_makes_all_three_refuted_relations_false_by_construction`'s `assert any(delta < 0)` [pipeline/tests/test_extract_report_pass_network.py:201] rests entirely on it.
- [x] [Review][Patch] Eight typed raise branches have no test, including the story's own declared departure [pipeline/extract/pass_network.py:222, :295, :309, :318, :326, :335, :353, :673] — verified zero matches in both test modules for `not contiguous` (the declared header-contiguity tightening), `outside every header cell`, `wrapped row label`, `not a 1-2 digit`, `assembled an empty player name`, `carries no matrix rows`, `rows against` (the N-rows-vs-M-columns census) and the `len(largest) < len(printed)` branch. The contiguity test needs a conftest hook that drops a single header cell.
- [x] [Review][Patch] The tail-pinning anti-pattern this very commit repaired is re-introduced in its own tests [pipeline/tests/test_extract_report_pass_network.py:163, :170] — `record["warnings"][-1] == warning` and `ids[-3:] == list(PASS_NETWORK_CHECK_IDS)` both break for the next story that appends, which is precisely why `test_extract_report_domains_ef.py`'s `ids[-7:]` had to be fixed here.
- [x] [Review][Patch] Real-PDF ground truth pins one row of sixteen and no away-side cell [pipeline/tests/test_extract_report_pass_network.py] — `RANGEL_ROW` covers row 0 only; the other 15 home rows and the entire away matrix are verified only through `matrix_total`, which is permutation-invariant. Task 8.7's floor is met, but a column-assignment slip on rows 1–15 has no ground-truth coverage.
- [x] [Review][Patch] Undisclosed co-committed Story 1.9 work in four shared files, and the File List omits `pipeline/tests/test_extract_report_domains_ef.py` entirely — `checks.py:1409-1451` rewrites `_domain_e_payload`/`_domain_e_uncached` behaviour (non-additive); `extract_report.py:57,337` wires the goalkeeper warnings; `conftest.py` adds `gk_distribution_spots` / `gk_distribution_donut_xs`; `test_extract_report_domains_ef.py` wires `clean_registry` into six previously-unwired tests. The File List describes `checks.py` as carrying only pass-network content.
- [x] [Review][Patch] The do-not-ship test is a substring filter, weaker than the Completion Notes claim [pipeline/tests/test_extract_pass_network.py:797] — `if "offer" in check_id or "col" in check_id`. A future check named e.g. `pass-network-received-bound` shipping exactly the forbidden relation passes it green. Assert the exact id set instead.
- [x] [Review][Patch] `_assert_no_pitch_and_no_markers` accepts a non-raising empty return where Task 2.12, its own docstring and `pipeline/README.md:1266` all say `detect_pitch_frames` is REQUIRED to raise [pipeline/extract/pass_network.py:159-163] — `except PitchFrameError: rects = []` then `if rects:` makes the two outcomes equivalent. Benign today (`filter_chain` never returns empty) but it is an undeclared *loosening*, unlike the two declared departures, both of which verified as real tightenings.
- [x] [Review][Patch] `_matrix`'s out-of-range column guard is unreachable [pipeline/extract/pass_network.py:364] — keys are built as `index - 2` for `2 <= index < len(cells)` and `column_count == len(cells) - 2`, so the key range is exactly `0..column_count-1`.
- [x] [Review][Patch] `top5_totals` prints a tautological ratio [pipeline/extract/pass_network.py:687] — `f"{side} {len(printed)}/{len(printed)}"` can only ever read `5/5`, while reading like a verified-of-total count.
- [x] [Review][Patch] A stale forward reference shipped in the commit that resolves it [pipeline/ingest/extract_report.py:425] — "Domains A-G plus set plays are filled; the remaining ones land with 1.14-1.15" names the current story as future work, two lines above the `pass_network` key it just added.
- [x] [Review][Patch] Task 2.6's `y-6 .. y+12` row-label band is not implemented and the deviation is undeclared [pipeline/extract/pass_network.py:285-341] — the parser takes the whole body below the header, groups it with `lines.group_rows` and recovers shirt/name from header-cell buckets 0 and 1. Arguably better (fully geometric, consistent with Task 2.4), but it is a third undeclared departure.
- [x] [Review][Patch] `pass_network_columns` has no upper-size guard, unlike every other invariant in the same factory [pipeline/tests/conftest.py:2255-2263] — at 18 columns the cycling widths carry the grid to x=786.75, past `PASS_NETWORK_PANEL_X0 = 760.5`, silently breaking both the panel filter and `body_limit`. `default_pass_network_block` raises loudly for `size < 6` with an explanatory message; the symmetric overflow gives an opaque parse failure.
- [x] [Review][Patch] `make_report`'s comment advertises a case it cannot reach [pipeline/tests/conftest.py:2707] — `pass_network_cell_text` is documented as doctoring "a fullwidth digit", but `cell_fonts` (`:2276`) is not a `make_report` kwarg and `emit_pass_network_page` (`:3466`) does not forward it, so a fullwidth digit routed that way renders as U+FFFD. The real coverage lives in `test_extract_pass_network.py:472`, which builds its own document.
- [x] [Review][Patch] `test_the_header_band_geometry_matches_the_corpus_template` asserts constants against themselves [pipeline/tests/test_extract_pass_network.py:813-819] — `columns[0][0] == PASS_NETWORK_FIRST_COLUMN_X0` (returned by construction from that constant), `PASS_NETWORK_HEADER_Y0 == 90.75`, and widths against `PASS_NETWORK_COLUMN_WIDTHS[:3]`. Nothing in it is derived from the real page, contrary to its docstring.

- [x] [Review][Defer] `_parse_rows` silently skips any body row carrying no shirt span and no name span [pipeline/extract/pass_network.py:301-315] — deferred, making it loud risks a corpus-wide regression and needs a full batch re-run to validate.
- [x] [Review][Defer] `_filled_bezier_rects` is narrower than the negative the AD-14 filing rests on [pipeline/extract/pass_network.py:130-146] — deferred, it flags only all-Bézier fills, so a re-scoped template drawing nodes as filled `"re"` squares, mixed `c`+`l` paths or a raster would pass both standing negatives and keep publishing `node_positions: null`. Widening the predicate is beyond this story's ruling.

**Verification after the patches — and what is still OUTSTANDING.** Every module the patches can reach was re-run and is green: 647 tests over `test_ingest_record`, `test_runner`, `test_checks_registry`, `test_extract_report_{domains_ef, domains_bc, domain_a, domain_g, momentum, pass_network}`, `test_extract_pass_network`, `test_ingest_batch`, `test_extract_domain_{f,g}` and all six `test_markers_*` modules. One failure was found and fixed mid-review: the new quiet-column guard picked `size - 1`, which collides with a Top-5 spike at size 10 (`(2k+1) % 10` lands on 9), breaking `test_a_count_inconsistency_lands_in_the_count_mismatch_bucket`; the quiet column is now the rightmost column the spikes do not occupy, and the construction is verified to hold at every size from 6 to 17. `test_extract_domain_e.py` was deliberately NOT run — another session had it dirty mid-edit and any result would have been unattributable.

**Tasks 9.2 and 9.3 were RE-RUN after the patches, and all three acceptance runs reproduce the ruled baseline.** They had to be: the `specifics` patch changes the content of every staged record, and every `pipeline/**/*.py` edit changes `code_version()`, so all 104 records were correctly invalidated by this review.

*9.2 batch.* `python -m pipeline.ingest.batch --input-dir pmsr-corpus --expect-reports 104 --force` → **`extracted 104 | failed 0`**, and exactly **2** self-validation failures — `PMSR-M19-ARG-V-ALG` and `PMSR-M58-TUN-V-NED`, both `defensive-actions-marker-count`, the ledgered pair. **No third, and zero `pass-network-*` failures over all 104 reports** (grep for `pass-network-` in the failure block returns 0). Non-zero exit by design, not asserted against.

*9.2 idempotence.* Re-run without `--force` → **`extracted 0 | failed 0 | skipped-unchanged 104`**. The tree stayed quiet across the interval, so this did NOT reproduce the false alarm the original Dev Record hit. (`test_re_extraction_is_byte_identical` covers the stronger determinism claim at unit level and passes.)

*9.3 gate.* `python -m pipeline.validate.verify --input-dir pmsr-corpus --expect-reports 104` → **`GATE RESULT: PASS (0 deviation(s) across 16 sampled report(s), 0 corpus gap(s))`**, all four categories at 0, 104 probed with 0 probe failures, and both `pass-network-completeness` and `pass-network-counts` present in `checks_run`.

**One observation for the NEXT story, not this one.** `checks_run` now also carries `identity-completeness` and `identity-pinning` — Story 1.15's gate checks, registered by a concurrent session while this review was running. `test_runner.py` pins that list exactly, so 1.15 owes it the same forced repair 1.14 made (and that file is already dirty in this tree from that session). Recorded here only because this review's gate output is the first place the two new ids appear.

**Dismissed on verification (12):** the claimed `deferred-work.md:205` citation drift (verified correct — the 1.13 entry is at `:205` and the 1.12 entry at `:171` in the file as committed); an N outside 13–17 staging silently (the contiguity, census, row/column-order and lineup-join guards all fire); `matrix_total == 0` (guarded at `:680`; the Top-5 check then fails loudly); a non-`PipelineError` crashing the gate (`runner.py:85-87` catches broad `Exception`); a comma decimal separator (parses to a wrong value that the Top-5 check rejects loudly); the standing negative being skipped at the gate on a Domain A failure (`extract_report` runs it on all 104 extractions; the gate samples 16); header-band y1 / gutter / straddling-span micro-cases (all surface as loud failures); a shirt/value row y-mismatch (fails the census loudly); U+2010/U+2011 dash normalization (fails the row/column-order check loudly); `test_ingest_record.py`'s warnings restructure (documented, and stronger where it matters); and the `checks.py` "Later stories add" line rewrite (the `1.10+` text it replaced was stale).
...
**CODE REVIEW CORRECTION (2026-08-03) — commit `5344fac` does not import, and it carries four files' worth of undisclosed Story 1.9 work.** Task 9.6 asks for exactly this disclosure and it was not made. What `5344fac` actually contains beyond this story: `pipeline/ingest/extract_report.py:57,337` import and call `domain_e_goalkeeper_warnings`, **whose definition does not exist anywhere in the tree at that commit** — it lands two commits later at `7f9b959:pipeline/extract/domain_e.py:1418` — so `import pipeline.ingest.extract_report` raises `ImportError` at `5344fac` and the whole pipeline, including every test module in this change, fails at collection; `pipeline/validate/checks.py:1409-1451` rewrites `_domain_e_payload` / `_domain_e_uncached` so a Domain A failure no longer skips Domain E, a **non-additive** behaviour change that at `5344fac` also passes `lineups=None` into an `extract_domain_e` still declaring `lineups: dict`; `pipeline/tests/conftest.py` adds the `gk_distribution_spots` / `gk_distribution_donut_xs` kwargs; and `pipeline/tests/test_extract_report_domains_ef.py` wires `clean_registry` into six tests. Both defects are resolved at HEAD by `7f9b959`, so nothing shipped is broken — this is a **history-integrity defect**: `git bisect` across `5344fac` and any checkout of it fail. **Ruled by Juan: record it, do not repair history.** The published log stands; this paragraph is the record. Note the mirror-image case already in the ledger — Story 2.6's ledger and sprint-status edits also landed inside `5344fac`.

**A THIRD departure the Completion Notes did not declare, found by the review and now corrected in code.** Task 2.12 says the parser must "call `detect_pitch_frames(page)` and **require it to raise** `PitchFrameError`". As shipped, `_assert_no_pitch_and_no_markers` caught `PitchFrameError` into `rects = []` and then raised only `if rects:` — which accepts a non-raising empty return as equally valid. Behaviourally identical today (`filter_chain.detect_pitch_frames` never returns empty; it raises), but it is a **loosening**, unlike the two departures the record does declare, both of which the review verified as genuine tightenings. The raise is now the assertion: any return at all, empty or not, fails the page.

**A FOURTH undeclared deviation, recorded not changed.** Task 2.6 pins the row label as read "from the span region left of the first player column, `y-6 .. y+12`". The shipped parser has no such band: it takes the whole body below the header, groups it with `lines.group_rows`, and recovers shirt and name from header-cell buckets 0 and 1. The `repr()` and `join_spans` halves of 2.6 are met. The implemented form is arguably better — it is the same geometric rule Task 2.4 mandates for the value columns, rather than a second, y-based rule beside it — and it is left as built; it should simply have been declared.
...
- `pipeline/tests/test_extract_report_domains_ef.py` — **omitted from this list until the code review.** Two changes, and only the first is this story's: the forced repair described in Task 9.1 (the E/F block located by index rather than pinned as `ids[-7:]`), and — co-committed from the concurrent Story 1.9 review session — a docstring on the `clean_registry` fixture plus its wiring into six previously-unwired tests
