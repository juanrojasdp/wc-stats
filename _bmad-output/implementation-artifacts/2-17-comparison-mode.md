...
- [~] 4.5 `<md` sheet behaviour reused from 2.14; **CSS `hidden`/`md:hidden`, no `useMediaQuery` in the picker**. — **PARTIAL, and the departure is the shipped answer.** No `<md` sheet is mounted: `ComparePicker`'s docblock argues it at length (the picker owns a full-width page region with room the header bar did not have, and two modal sheets would spend the page's single depth-1 overlay slot on a field that already fits at 320 px). The second half — no `useMediaQuery` in the picker — IS satisfied. Box corrected from `[x]` at code review 2026-08-07: the note argued the departure, the checkbox denied it.
...
- [x] 7.1 `src/components/CompareCharts.tsx` — `{ colorVar, hatch, … }`. Copy the Recharts Contract exactly. — the bar leaf ships; **no `CompareLineChart` was built**, because no shipped comparison plots a line: `type=players` plots five speed bands, `type=teams` eight phase rates, `type=matches` a two-team distribution. Filed in the ledger rather than built speculatively.
- [~] 7.2 The D4 hatch for side B's bars; `TEAM_B_DASH_ARRAY` for side B's lines. Per-instance `useId()` pattern ids. — **PARTIAL.** The hatch and the per-instance `useId()` pattern ids ship. `TEAM_B_DASH_ARRAY` does not exist anywhere, and correctly so: D4's own binding conclusion is that a dashed stroke **cannot work on a filled bar**, and 7.1's line chart was never built for it to apply to. Box corrected from `[x]` at code review 2026-08-07.
...
- [x] 11.6 Append `EXPERIENCE.md` policy rows under its own heading — **appended, never renumbered**. Include D12's analysis. — the five rows landed with D12's analysis, but **under 2.16's heading rather than their own**; the "Rows appended by Story 2.17" heading and its own table header were added at code review 2026-08-07.
...
| 2026-08-09 | Review resumed after an interruption. The findings record above had never been committed and four of its patches sat in a dirty tree; committed as `5eaa654` before any further edit, because a concurrent session has swept this story's files into another story's commit once already (D3a). |
| 2026-08-09 | Remaining patches applied — every `[ ]` in the list above is now `[x]`, with the outcome written on the item rather than in a summary. D3a closed as MOOT: all four files were already in `HEAD` via `82888b4`, verified per-path with `git log -1`. |
| 2026-08-09 | `seriesLabelIndex` and its eight tests moved out of `TacticalCharts` into `viz/phases-model`; `TacticalCharts.test.ts` deleted. This is the only structural move in the review and it returns `TacticalCharts.tsx` to D9's declared "only edit". |
| 2026-08-09 | Verification: lint 0, tsc 0, `next build` green (13 static pages), **1,238 tests across 45 files, all passing**. Vendor chunk re-measured on the built export with the story's own three-token classifier: **exactly 1, at 361.9 KB** — unchanged by the second `dynamic()` handle, which was the risk that change carried. |
| 2026-08-09 | One residual gap FILED rather than fixed: two entities sharing a display name still render byte-identical captions, unreachable on the 2-player fixture and owned by 2.19's real-data swap. |
...
- [x] [Review][Patch] **[from D1a]** Add `direction: "higher" | "lower" | "none"` to `TeamCompareField`; mark `lost` and `goalsAgainst` as `"lower"`, `drawn` and `pressingIntensity` as `"none"`, the rest `"higher"`. `teamCompareRows` resolves the leader through that direction — inverting for `"lower"` and emitting `"tie"` for `"none"` — so no row ever asserts a leader the metric does not have. Add a distinct-team test that would go red on the current code [`compare-model.ts:255-366, :368-382`] — **APPLIED.** `CompareDirection` + `directedLeader` in `compare-model.ts`; `resolveLeader` is reached through it and never re-minted (D14 held). The distinct-team test synthesises a `rival` by spread and pins `lost` / `goalsAgainst` to `"home"` and `drawn` / `pressingIntensity` to `"tie"` on UNEQUAL values — every one of those four reads the other way without the fix.
- [x] [Review][Patch] **[from D2a]** Give the `type=matches` mirrored rows a head row naming home and away by team code, so nineteen rows of unattributed pairs become attributable; and re-rule the row accent inside a match block so side B's values do not paint in side A's accent under side B's border [`CompareRegion.tsx:941-948`, `CompareRows.tsx:112,154-161`, `compare-model.ts:491-505`] — **APPLIED.** `ColumnHeads` in `CompareRows.tsx`, fed by `matchHeads()` and rendered only for `type=matches`, where the side header names the MATCH rather than the pair. The heads carry their own accents, which states the local meaning of `viz-team-a`/`viz-team-b` where the reader meets it — the same way the charts discharge D5's corollary.
- [x] [Review][Patch] **[from D3a]** Land `i18n.test.ts`, `teams/static-output.test.ts`, `TeamHero.tsx` and `team-profile.ts` as a 2.17 follow-up commit, so Tasks 2.4 / 11.3 / 11.4 are reproducible from a commit rather than from a dirty tree — **MOOT, AND THE REASON IS THE FINDING RESTATED.** All four were already committed when this patch was written — by `82888b4`, the *2.16* code-review session, which swept them in while this review was being drafted. They are reproducible from a commit; that commit simply belongs to another story. Nothing was landed here, and the split is recorded rather than rewritten: re-committing files already in `HEAD` would produce an empty diff, and rewriting `82888b4` to re-attribute them is not worth a history edit. Verified 2026-08-09: `git log -1 -- <each path>` returns `82888b4` for all four.

**From the review layers:**

- [x] [Review][Patch] `/compare` ships no `<h1>` — the only route in the app without one; the route's top heading is an `sr-only <h2>`, and both sibling suites carry an `h1`-count gate this one omits [`ComparePicker.tsx:150`, `compare/static-output.test.ts:107`] — **APPLIED.** Promoted in place, in `ComparePicker` rather than `CompareRegion`, because the picker is the one block that renders in all four states and therefore the only one present in `out/compare/index.html`. Both sibling assertions added: exactly one `<h1>`, and it is `sr-only`.
- [x] [Review][Patch] Side headers ship `<h3>` against Route Composition's ruled `<h2>`, so the outline runs h2 → h3 → h2 with no h1 [`CompareRows.tsx:90`] — **APPLIED**, and the docblock that argued for `<h3>` is replaced with the reason the ruled outline is deliberately flat: the two sides and the two sections are four peers under one `<h1>`, not a hierarchy. A `not.toContain("<h3")` gate holds it.
- [x] [Review][Patch] A pasted URL naming one or two sides renders a blank, unannounced region for the whole index fetch — every branch is gated on `chosenCount === 0`, `indexReady` or `bothListed`, and none can be true first. This is AC 6's own path [`CompareRegion.tsx:756,770,791`] — **APPLIED.** New `chosenCount > 0 && indexStatus === "loading"` branch rendering the extracted `ComparisonSkeleton` under `aria-busy`. Not focused, unlike the retry path: no panel owning the caret was unmounted, so moving focus would steal it from a reader already in the picker.
- [x] [Review][Patch] The lazy-chart fallback is pinned to `compareBarChartHeightClass(5)` while `TeamFigures` mounts at `(8)`, so every `type=teams` comparison shifts ~110 px per chart, ×2 — the exact CLS defect the Recharts Contract rule exists to prevent [`CompareChartsSection.tsx:86` vs `:547`] — **APPLIED.** `next/dynamic`'s `loading` takes no props, so one handle cannot serve two heights: `lazyCompareBarChart(count)` mints one per mounted count. Both point at the SAME import specifier, so the export keeps exactly one recharts vendor chunk.
- [x] [Review][Patch] `?a=X&b=X` is reachable in two clicks — nothing filters the other side's pick out of the scoped corpus and `handlePick` does not dedup, producing duplicate React keys and byte-identical figure captions, table names and mini-header names [`CompareRegion.tsx:600-608`, `ComparePicker.tsx:132`, `CompareChartsSection.tsx:434,514,605`] — **APPLIED IN THREE PLACES**, each covering what the others cannot: the picker filters each side's pick out of the other's corpus (`scopedForA`/`scopedForB`), the URL cleanup drops a duplicate `b`, and `idA !== idB` joins `bothListed` so the degenerate pair cannot render in the one frame before that write lands. Pinned by a new source gate.
- [x] [Review][Patch] Team codes render lowercase on `/compare` (`mex`/`rsa`) and uppercase everywhere else — `ChartSeries.teamCode` is documented as *"Already uppercased at the TacticalLayer prop boundary"* and `TacticalLayer.tsx` uppercases at 22 sites [`CompareChartsSection.tsx:626,634,653,654`, `CompareRegion.tsx:351,356`] — **APPLIED** (in `5eaa654`): one `displayTeamCode` helper at the prop boundary, named so a reviewer can grep it, rather than six scattered `.toUpperCase()` calls.
- [x] [Review][Patch] A bad `type` param discards a valid slug and blames the reader for it — `droppedSides` never consults `params.droppedType`, so `?type=team&a=mexico` rewrites to `?type=players` AND shows "No encontramos mexico" [`CompareRegion.tsx:143-156`, `:438-458`] — **APPLIED.** `droppedSides` returns `[]` under `droppedType`, so the accusation is suppressed; the cleanup effect drops BOTH slugs rather than keeping one, because neither was ever validated against the type it was written for and keeping one would silently reinterpret it as a player id.
- [x] [Review][Patch] The polite live region concatenates its two children with no separator ("5 resultadosComparación cargada.") and re-reads the stale load confirmation on every settled keystroke; the rejected-slug, index-error and index-invalid panels are not announced at all [`CompareRegion.tsx:681-686`] — **APPLIED.** Two separate polite regions — the picker's counts and the region's outcome answer to different events, and sharing one made each re-read the other. The outcome sentence is composed in a `const` from a `SENTENCE_JOIN` separator and now covers the rejected slugs, the index error and the index-invalid state as well as the three data states.
- [x] [Review][Patch] `seriesLabelIndex` is a **value** import crossing into a chart module, against the Recharts Contract's *"Only `import type` may cross into a chart module"*, and adding its `export` was a second edit to `TacticalCharts.tsx` beyond D9's "only edit" [`CompareCharts.tsx:14`] — **APPLIED BY MOVING THE FUNCTION, not the import.** It now lives in `viz/phases-model.ts` beside `wrapAxisLabel` and `distributionChartHeightClass`, which both leaves already import from the pure layer; its eight tests moved to `phases-model.test.ts` and `TacticalCharts.test.ts` is deleted. `TacticalCharts.tsx` is back to D9's declared "only edit".
- [x] [Review][Patch] The AR-10 "no component state holds the comparison" gate is defeated by renaming a variable — the regex misses `const [compareType,`, `const [chosenA,`, `useRef`/`useReducer` — and `toContain("replaceUrlQuery")` is satisfied by the import line or by comment prose [`compare/static-output.test.ts:166-169`] — **APPLIED AS AN ALLOW-LIST.** The seven legal `useState` slots and the two legal `useRef`s are enumerated, so ANY new state on the route goes red until someone writes down what it is; the picker is pinned at zero of both. The writer assertion matches the CALL (`/replaceUrlQuery\s*\(/`), not the mention.
- [x] [Review][Patch] The combobox-count gate cannot detect the failure it names: `toBeGreaterThanOrEqual(2)` still passes with one `SearchField` deleted. Measured on the export — `out/compare/index.html` = 3, `out/about/index.html` = 1 (header only) [`compare/static-output.test.ts:966-974`] — **APPLIED.** Measured as a DIFFERENCE against `/about`, the header-only baseline, so the picker's own contribution is pinned at exactly two and the SiteHeader's one is subtracted rather than absorbed into a floor.
- [x] [Review][Patch] Every team-model test compares México with itself, so `expect(model.a).toEqual(model.b)` is tautological and `every(leader === "tie")` masks the inverted-metric bug above. The file already synthesises a second profile by spread one test earlier, so a distinct-team case cost one line [`compare-model.test.ts:4491-4541` in diff] — **APPLIED.** A `rival` fixture by spread, worse on the lower-is-better metrics, plus a second test giving `teamChartModel` two DIFFERENT peak profiles — AC 4's "identical scales" claim is unfalsifiable against a team compared with itself, where one peak is trivially both.
- [x] [Review][Patch] The caption-inventory extension is distinct by construction and excludes `a === b`, the one input that actually breaks caption distinctness on this route — while asserting *"the route guarantees `a !== b`"*, which it does not [`i18n.test.ts` compare captions block] — **APPLIED, AND THE CLAIM IS NOW TRUE RATHER THAN ASSERTED.** The comment is rewritten to record that it was false when written, to name the three mechanisms that now enforce it, and to point at the new gate that pins them. A RESIDUAL GAP IS FILED rather than papered over: distinct ids are still not distinct NAMES, and the caption prefix is the name — `search-model.ts` records "Emiliano MARTINEZ occurs twice in the real corpus", so two different players can still produce two identical captions. Not reachable on the 2-player fixture; filed to the ledger.
- [x] [Review][Patch] `seriesCode` falls back to the entity's full display name, painted into a 34 px gutter with no truncation and silently clipped by the SVG viewport [`CompareChartsSection.tsx:550`, `CompareCharts.tsx` `COMPARE_MARGIN`] — **APPLIED.** `seriesCode` is `string | null` and a null SUPPRESSES the label through the same `-1` sentinel D9 already established, so there is one guard rather than two. Nothing is lost: `CompareFigure` prints the entity name directly above the chart and the figure summary names it again.
- [x] [Review][Patch] Copy and prop defects: the swap button's accessible name is `"Intercambiar lados México / Brasil"` while its docblock two lines above advertises `"Intercambiar lados: México / Brasil"`; `fieldLabel` overloads the listbox's own name so the input and its listbox both announce as "Lado A"; `className=""` is passed at both `ValueCell` call sites; `row.unit === null` should be `== null` to survive an out-of-union metric code [`ComparePicker.tsx:141-144`, `HeaderSearch.tsx` `listLabel`, `CompareRows.tsx:157,202,212`] — **ALL FOUR APPLIED.** `SIDE_JOIN` carries the colon the docblock advertised; `listLabel` is decoupled from `fieldLabel` and keeps `search.listLabel` on every call site (two listboxes may share a name — the single-open registry means only one is ever in the accessibility tree); the dead `className` prop was dropped in `5eaa654`; `== null` covers the `undefined` an out-of-union metric code produces.
- [x] [Review][Patch] Checkbox and traceability integrity: Tasks 7.1/7.2 are `[x]` with no `CompareLineChart` and no `TEAM_B_DASH_ARRAY` anywhere; Task 4.5 is `[x]` with no `<md` sheet. Both departures are argued in the Completion Notes and filed in the ledger — the boxes contradict the notes. Task 11.6's policy rows also lack their own "Rows appended by Story 2.17" heading, landing instead under 2.16's [`EXPERIENCE.md`] — **APPLIED.** Tasks 4.5 and 7.2 are re-marked `[~]` with the departure stated on the line itself; 7.1 keeps `[x]` with the missing line chart named. `EXPERIENCE.md` gets its "Rows appended by **Story 2.17**" heading and its own table header, so the table no longer reads as though 2.16 ruled the comparison vocabulary.
- [x] [Review][Patch] Test title says *"the compare route's three fetches"* and asserts `toHaveLength(4)` [`app/src/app/static-output.test.ts:906-910`] — **APPLIED.** Retitled to four artifact PATHS, with both numbers explained: three fetches per load, but `type` is a URL parameter so all three branches of `fetchSide` are reachable from the entry point and the allow-list must carry all four.
