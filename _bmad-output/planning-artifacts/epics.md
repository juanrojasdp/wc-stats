...
NFR-4: Shareability & discoverability — stable, human-readable URLs for every match, player, team; meaningful `<title>`/OG meta for link previews; absolute canonical URLs from a configured `metadataBase`; a same-origin `og:image` card and a `summary_large_image` Twitter card; `sitemap.xml` and `robots.txt` emitted at build. **Metadata is emitted ONCE PER ROUTE in the canonical locale (Spanish)** — a static export yields one document per URL, and per-locale metadata is not reachable without per-locale URLs (D17; upheld by D20, 2026-08-26).
...
- AR-11 (AD-11): Rendering split — build-time filesystem reads (`generateStaticParams` from the route manifest) for static params, `<title>`/OG meta, and pre-rendered Hero content from `storyStats`; client fetch for everything below the Hero; no inlining full bundles; `output: 'export'`, `images: { unoptimized: true }`; fonts (Archivo + Inter) self-hosted via `next/font`, zero external requests. **The "zero external requests" clause scopes to THIRD-PARTY ORIGINS, not to assets as such (clarified by D20, 2026-08-26): same-origin static assets under the export are in bounds, and a URL in a `<meta>` tag (`og:image`, `twitter:image`) is not a request the page makes at all — it is a hint a crawler may fetch, off-page and off-session. The mechanical enforcement is `app/scripts/assert-no-external-origins.mjs`, whose `FETCHING_POSITIONS` list is the operative definition of "a request".**
...
- UX-DR24: **Home IA & navigation (ruled 2026-08-26 by `bmad-ux` story 3.7; supersedes UX-DR4's "no primary nav")** — `/` becomes a **Landing** surface (identity lede + an 8-badge feature grid, `/compare` emphasised by size/position/border, never color); the Tournament Hub moves to `/tournament`, the 36 leaderboard boards to `/tops`, and `/players` + `/teams` index routes are minted so no badge resolves to a fragment. **Route count 1,406 → 1,410.** Navigation is one 9-destination set in two presentations: `<xl` a single trigger absorbing nav + search + `ES|EN` + theme into a **modal** sheet (document inert, focus trapped, scrim, `Esc` returns focus, `aria-controls` conditional on the portalled sheet being in the DOM); `≥xl` inline links with the chrome inline. **The nav LOWERS header min-content below the 237 px R2/D8 figure** — the 320 px floor gets easier, at the stated price of language and theme becoming two taps on phones. `lg` was measured and rejected: 976 px usable against ~1,060–1,080 px, failing invisibly because the search ships `min-w-0 flex-1`. Adds a **header-height token** (`header-h-oneline/-wrapped/-zoom` + `scroll-clearance`) replacing the 4.5rem `scroll-padding-top` constant, which `globals.css:446` records as wrong by 46 px at wrapped widths. Deep-link fragments are **surface** (scroll only — `#results`, `#standings`) or **leaf** (open through every closed layer), declared per anchor, never inferred. Adopts ledger L1553/L1886 and L1465. Full contract: `EXPERIENCE.md` → Information Architecture / The Landing Page / The Player and Team Indexes / Navigation / Deep-Link Fragment Grammar, and `DESIGN.md` → Components.
...
**Then** it enumerates 104 matches + 1,248 players + 48 teams + `/`, `/about`, `/glossary`, `/compare` **and, once UX-DR24's routes land, `/tournament`, `/tops`, `/players`, `/teams`** — every entry trailing-slashed. If 3.4 ships before 3.9, the bijection assertion picks the four up automatically when they appear, which is why it is written against the manifest rather than a literal count.
...
**Then** Lighthouse mobile is **≥ 68 on `/tournament`**, which is where UX-DR24 moves the 6,025 nodes the 68 was measured over — the guard follows the CONTENT, not the route. `/`, `/tops` and `/players` are new surfaces: each records its own first median as its own floor. Holding the near-empty `/` to ≥68 would be a gate that cannot go red (A1/NFR-12). The refactor is free to reshape and not free to regress. SM-5 stays closed; this story does not design toward 90.
...
**Then** the route count is **1,410** — UX-DR24 mints `/tournament`, `/tops`, `/players` and `/teams`, deliberately superseding this AC's original "stays 1,406" (the clause guards against accidental route explosion and per-locale duplication, neither of which four ruled routes is) — the build is green, and no test is newly skipped.
