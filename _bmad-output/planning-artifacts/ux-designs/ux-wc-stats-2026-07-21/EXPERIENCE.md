...
| player profile — section headings | translate | Perfil físico / Evolución por partido / Totales del torneo / Partido por partido | **PROPOSED — Juan to confirm or overturn at review.** The route's four `<h2>`s (`player.sections.*.title`), in the ruled disclosure-grammar order. No spine names them: the IA table names the surface "Player Profile", which is a document label rather than reader-facing copy. "Totales del torneo" rather than "Agregados" — the artifact's word is `aggregates`, but *agregado* in LatAm Spanish reads as "added on", not "aggregate". Story 2.15 Task 10.2; en "Physical profile / Match by match trend / Tournament totals / Match by match" |
| player profile — per-match column heads | translate | Fecha / Rival / Fase / Titular / Minutos | **PROPOSED — Juan to confirm or overturn at review.** The five `PlayerMatchRow` fields with no shipped label (`player.column.*`). Every OTHER head on that table is **reused**: metric terms from `enums.leaderboardMetric.*`, and `attemptsAtGoal` / `passesAttempted` from `expert.field.*` — Story 1.18 established that neither is a `MetricCode`, so neither has a metric label to take |
| player profile — transposed table heads | translate | Métrica / Valor | **PROPOSED — Juan to confirm or overturn at review.** `player.column.metric` / `.value`. These exist nowhere yet because every shipped table is metric-per-column; the aggregates table is TRANSPOSED (eighteen metrics sharing one value column), which is also why its unit rides the row-header label rather than the column head — a scoped, recorded departure from 2.13's head-side unit rule, which presumes one metric per column |
| player profile — started boolean | translate | Sí / No | **PROPOSED — Juan to confirm or overturn at review.** `player.started.*`. A boolean is never printed raw. Deliberately NOT "Titular"/"Suplente": the column is already headed "Titular", so repeating the noun in every cell says the same word twice and loses the answer |
| player profile — appearances line | translate | Partidos / Titular / Suplente / Minutos | **PROPOSED — Juan to confirm or overturn at review.** `player.appearances.*`, composed LABEL-FIRST (`Partidos: 5`) rather than count-first. t() has no plural machinery, so a count-first form would need eight keys for four counters and would still read "1 partidos" the day one is missed; label-first is correct at every count, including the 209 players (16.7 %) whose every counter is 0 |
| player profile — empty states | translate | Sin evolución por partido. / Sin partidos en el torneo. | **PROPOSED — Juan to confirm or overturn at review.** `player.empty.*`. `useEmptyHeadline()` COULD NOT BE REUSED: it composes "Sin datos de {sección} **para este partido**", which is false on a route that is not a match — the Hub hit the same wall and answered it the same way. Neither sentence may imply the page is broken: 209 players never appeared, and a goalkeeper's profile legitimately carries no goalkeeping data (Story 1.18 R1(A), ruled by Juan), so the copy states the fact and stops |
| player profile — region states | translate | Cargando los datos del jugador / Datos del jugador cargados. / No pudimos cargar los datos del jugador… / Los datos de este jugador no coinciden con esta versión del sitio. | **PROPOSED — Juan to confirm or overturn at review.** `player.region.*`, mirroring `MatchBundleRegion`'s four-state machine with profile-scoped copy. `invalid` carries no retry: a payload that arrived intact and failed the schemaVersion gate cannot be fixed by re-fetching the identical artifact |
| player profile — trend selector | translate | Métrica de la evolución | **PROPOSED — Juan to confirm or overturn at review.** `player.trendSelector`, the `ToggleGroup`'s `aria-label`. Radix gives `type="single"` groups radiogroup semantics, so this names a `role="radiogroup"`, not a tablist |
