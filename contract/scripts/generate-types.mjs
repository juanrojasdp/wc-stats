...
function stripCrossReferenceStanza(text) {
  const kept = text
    ...
    .filter(
      (line) =>
        !/^\s*\*\s*This interface was referenced by /.test(line) &&
        !/^\s*\*\s*via the `[^`]+` "/.test(line)
    );
...
function assertFidelity(source) {
  const problems = [];
  ...
  const suffixed = source.match(/^export (?:type|interface) \w*[A-Za-z]\d+\b.*$/gm);
  if (suffixed) {
    problems.push(
...
async function main() {
  ...
  if (!Number.isInteger(version.schemaVersion)) {
    throw new Error(
      `schemaVersion must be an integer, got ${JSON.stringify(version.schemaVersion)}`
    );
  }
